...
```
platform 把任务指派给我后，flow 监听到事件，给我发了一条任务消息：
  {
    type: "task_assign",
    task_id: "TASK-003",
...
> 工单不只是 flow 分配的，还有多种创建来源。工单创建后存在 platform，由 platform 驱动状态流转和指派，flow 监听事件后向 Agent 发送执行指令。
...
内心独白（我的视角，后端开发-1）：
  "测试-1 发现了我的代码的 BUG。
   我在写代码时确实没考虑空值校验。
   等 platform 指派修复任务给我，flow 发 FlowMessage 告诉我怎么修。"
```

...
| delivery 推进 | 功能闭环 | P0 | scheduled delivery 可推进到 dispatched / completed / failed 并保留 history | 不承诺 exactly-once |
...
| AC-FUNC-003 | 订阅与 delivery 推进 | P0 | scheduled delivery 可经默认后端路径推进到 dispatched / completed / failed;history append;批处理成功失败隔离 | delivery 卡在不可解释状态;completed 被 reopen;batch failure 污染成功项;backend unavailable 无失败或重试证据 | `TC-BUS-DLV-001`~`004`;`EV-BUS-DLV-*` |
...
| AC-FUNC-007 | Outbox relay 边界承接 | P0-min | committed outbox fact 可被消费并形成 publication acceptance;source ref 唯一;duplicate / source ack failure 不生成重复 acceptance | 未提交 fact 推进为 bus truth;duplicate 生成新 acceptance;source ack failure 后重放破坏幂等 | `TC-BUS-OBX-001`~`002`;`EV-BUS-OBX-*` |

...
> 本文件是 `projects/L0-bus/03-详细设计.md` 的 SOP 校准工作台。
> 它记录每个 Step 的中间产物位置、状态和回填章节。
> 本目录中的内容是中间产物,不替代正式 `03-详细设计.md`。
>
> 本轮状态说明:
> - 当前正式 `03-详细设计.md` 是 2026-05-17 的旧版草案,只作为问题诊断材料。
> - 旧版 `03-详细设计.md` 仍围绕 envelope / routing / callback schema 等旧口径展开,与新版 `02-概要设计.md` 的 publication / delivery / feedback / recovery / read-only output 主线不一致。
...
| 当前需求文档 | `projects/L0-bus/00-需求文档.md` v0.2.0 |
| 当前架构设计 | `projects/L0-bus/01-架构设计.md` v0.2.0 |
| 当前概要设计 | `projects/L0-bus/02-概要设计.md` v0.2.0 |
| 待校准详细设计 | `projects/L0-bus/03-详细设计.md` v0.1.0 |
...
本轮不是继续扩写旧版 `03-详细设计.md`,而是在已经稳定的需求、架构和概要设计边界下,把 `L0-bus` 详细设计重校准成可以 1:1 指导实现的代码实现契约。
...
| 样例 | 可参考内容 | 不可机械套用内容 |
|---|---|---|
| `projects/L0-core/design-calibration/03_ddd_*` | L0 底座仓如何按 19 Step 形成详细设计中间产物 | L0-core 的 contract definition、baseline、snapshot、compatibility 对象不能搬到 L0-bus |
| `projects/L3-method-library/design-calibration/03_ddd_*` | 重 Step 如何按模块、对象、trait、协议、处理流和状态机分批形成 | method content、definition、fingerprint、snapshot、qualification 等业务结论不能搬到 L0-bus |
| `projects/L0-bus/02-概要设计.md` | 本仓代码主体框架、主要组成部分、对象、接口、处理流、状态机和配置影响 | 概要设计的轮廓不能直接当成完整实现契约 |

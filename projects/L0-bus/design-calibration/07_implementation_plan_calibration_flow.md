...
> 本文件是 `projects/L0-bus/07-实施计划.md` 的 SOP 校准工作台。
> 它记录每个 Step 的中间产物位置、状态、回填章节和当前处理状态。
> 本目录中的内容是中间产物,不替代正式 `07-实施计划.md`。
>
> 本轮状态说明:
> - 当前 `projects/L0-bus/07-实施计划.md` 尚不存在。
...
| 当前验收标准 | `projects/L0-bus/06-验收标准.md` |
...
- 允许参考已收稳的 `L0-core`、`L1-identity`、`L3-method-library` 和 `L0-bus` 前序中间产物,但不能机械搬运其他子项目的实施阶段。

...
|---|---|---|---|---|---|
| EV-CI-FMT-001 | 格式 / 静态检查报告 | `fmt_lint_suite` | `artifacts/test/<run_id>/suites/ci-fmt-lint/` | 全局 | 代码质量门禁 |
| EV-UNIT-001 | 单元测试报告 | `unit_domain_suite` | `artifacts/test/<run_id>/suites/unit-domain/` | TC-CMD-*、TC-IDEM-* | 领域不变量 |
| EV-SVC-001 | 应用服务测试报告 | `service_command_query_suite` | `artifacts/test/<run_id>/suites/service-command-query/` | TC-CMD-*、TC-QUERY-*、TC-IDEM-* | P0 主线 |
| EV-CONTRACT-001 | DTO / schema roundtrip 报告 | `dto_schema_contract_suite` | `artifacts/test/<run_id>/suites/contract-schema/` | TC-DTO-001 | 契约兼容 |
| EV-CONTRACT-002 | CloudEvent / relay boundary 报告 | `outbox_relay_boundary_suite` | `artifacts/test/<run_id>/suites/contract-outbox/` | TC-EVENT-001、TC-OUTBOX-* | 事实传播边界 |
| EV-CONC-001 | 并发冲突报告 | `integration_persistence_suite` / `nightly_fault_recovery_suite` | `artifacts/test/<run_id>/suites/concurrency/` | TC-CONC-001 | 并发一致性 |
| EV-INT-001 | repository / transaction / projection 集成报告 | `integration_persistence_suite` | `artifacts/test/<run_id>/suites/integration-persistence/` | TC-OUTBOX-001、TC-TXN-001、TC-QUERY-002 | 一致性 |
| EV-WORKER-001 | job / worker 报告 | `worker_job_suite` | `artifacts/test/<run_id>/suites/worker-job/` | TC-JOB-* | 后台承接 |
| EV-CONFIG-001 | 配置测试报告 | `config_smoke_suite` + config failure gate | `artifacts/test/<run_id>/suites/config-smoke/` | TC-CONFIG-* | 配置门禁 |
| EV-AUDIT-001 | 审计证据报告 | audit fixture / trace query | `artifacts/test/<run_id>/suites/audit/` | TC-AUDIT-001 | 可追溯 |
| EV-TRACE-001 | trace 传播报告 | log / audit / event scan | `artifacts/test/<run_id>/suites/trace/` | TC-AUDIT-001、TC-E2E-001 | 可观测 |
| EV-SEC-001 | 禁止正文入仓扫描报告 | negative fixture scan | `artifacts/test/<run_id>/suites/security-body-boundary/` | TC-SCOPE-002 | 安全边界 |
| EV-SEC-002 | raw secret 扫描报告 | config / log / audit scan | `artifacts/test/<run_id>/suites/security-secret-boundary/` | TC-CONFIG-003 | 密钥边界 |
| EV-SCOPE-001 | 范围边界负向报告 | scope policy / API surface scan | `artifacts/test/<run_id>/suites/scope-boundary/` | TC-SCOPE-001、TC-SCOPE-002 | 职责边界 |
| EV-NFR-001 | 性能 / job / query baseline 报告 | `nfr_baseline_suite` | `artifacts/test/<run_id>/suites/nfr-baseline/` | TC-NFR-001 | 非功能 |
| EV-NFR-002 | 查询 / 追溯 benchmark 报告 | `nfr_baseline_suite` | `artifacts/test/<run_id>/suites/nfr-query-trace/` | TC-NFR-001 | 非功能 |
| EV-E2E-001 | 最小闭环报告 | `e2e_minimal_loop_suite` | `artifacts/test/<run_id>/suites/e2e-minimal-loop/` | TC-E2E-001 | 发布候选 |
| EV-NIGHTLY-001 | nightly 恢复 / 并发报告 | `nightly_fault_recovery_suite` | `artifacts/test/<run_id>/suites/nightly-fault-recovery/` | TC-CONC-*、TC-JOB-*、TC-OUTBOX-* | 残余风险 |
...
artifacts/test/<run_id>/
  机器原始证据、JSON report、stdout/stderr log
        |
        | scripts/reports/*
        v
reports/
  人类可读报告、验收交接、风险接受和审查意见
```

| 报告 | 来源 artifact | 生成脚本 | 输出位置 | 人 / Agent 审查要求 |
|---|---|---|---|---|
| run summary | `artifacts/test/<run_id>/evidence-index.json` | `scripts/reports/generate_reports.sh` | `reports/runs/<run_id>/summary.md` | 检查 P0 suite 结论和失败说明 |
...
| gate results | `artifacts/test/<run_id>/suites/*/report.json` | `scripts/reports/generate_reports.sh` | `reports/runs/<run_id>/gate-results.md` | 检查 release gate 是否完整 |
| redaction check | artifacts + reports | `scripts/checks/check_redaction.sh` | `reports/runs/<run_id>/redaction-check.md` | 检查 raw secret / raw body 未泄露 |
| acceptance handoff | `reports/runs/<run_id>/*` | `scripts/reports/generate_acceptance_handoff.sh` | `reports/acceptance/handoff.md` | 补充送验范围、残余风险和交付说明 |
| veto checklist | `reports/runs/<run_id>/*` | `scripts/reports/generate_acceptance_handoff.sh` | `reports/acceptance/veto-checklist.md` | 人 / Agent 确认所有一票否决项 |

正式测试和验收不得引用 `latest`，也不得使用 `artifacts/test/l0-core/<run_id>` 或 `reports/l0-core`。

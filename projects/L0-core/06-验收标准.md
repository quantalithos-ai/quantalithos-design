...
| 原始证据基线 | `artifacts/test/<run_id>/...` | 待测试执行补齐 | 机器原始证据、JSON report、stdout/stderr log |
| 运行报告基线 | `reports/runs/<run_id>/...` | 待测试执行补齐 | 人类可读 suite 摘要、EV 索引、门禁结果和脱敏检查 |
| 验收交接基线 | `reports/acceptance/...` | 待送验补齐 | handoff、veto checklist、risk acceptance 和 open issues |
...
| report run 变化 | 更新 `reports/runs/<run_id>` 和 `reports/acceptance/*` 引用 | 是 |
...
- [ ] `reports/runs/<run_id>/summary.md`、`evidence-index.md`、`gate-results.md` 和 `redaction-check.md` 已从 `artifacts/test/<run_id>` 生成。
- [ ] `reports/acceptance/handoff.md` 和 `reports/acceptance/veto-checklist.md` 已由人 / Agent 审查；如果存在有条件通过，`reports/acceptance/risk-acceptance.md` 已补齐接受人和后续动作。
...
- [ ] 正式验收引用固定 `<run_id>`，不引用 `latest`、`artifacts/test/l0-core/<run_id>` 或 `reports/l0-core`。
...
| AC-EVID-011 | 人类可读报告归档 | `reports/runs/<run_id>/summary.md`、`evidence-index.md`、`gate-results.md`、`redaction-check.md` | reports 可回指 `artifacts/test/<run_id>`，且不引用 `latest` | reports 缺失、使用 `latest` 或无法回指 artifact |
| AC-EVID-012 | 验收交接报告 | `reports/acceptance/handoff.md`、`veto-checklist.md`、必要时 `risk-acceptance.md` | handoff 和 veto checklist 已审查；有条件通过时风险接受完整 | 交接缺失、VETO 未覆盖或风险接受无接受人 |

...
| 上游契约消费与派生视图 | `DerivedBindingView`、`LanguageBindingView`、`SnapshotFreshnessState` | upstream truth boundary rule | contract / bus snapshot projection | `UpstreamVersionRef`、`CoreContractRef`、`TransportSemanticId` | `DerivedBindingView`、`LanguageBindingView`、`UpstreamVersionRef`、`SnapshotFreshnessState` |
...
| 事件客户端视图 | `BusEventClientView` | event semantic consistency rule | event capability projection | `TransportSemanticId`、`EventBoundaryRef` | `BusEventClientView`、`EventSemanticMapping` |
...
| `DerivedBindingView` | projection / value object | 表达从 core / bus / formal API 派生出的 SDK binding 视图 | `view_id`、`source_refs`、`freshness_state`、`capability_symbols` | `assert_fresh()`、`contains_capability(...)`、`from_upstream_refs_and_symbols(...)` |
...
| `BusEventClientView` | projection / view object | 表达 SDK 对 `L0-bus` 语义的事件 client 视图 | `event_view_id`、`mapping_set`、`freshness_state`、`supported_operations` | `supports_operation(EventClientOperation operation)`、`assert_transport_semantic_aligned(...)` |
| `EventSemanticMapping` | value object | 连接 SDK 事件表达与 bus transport semantic | `sdk_event_name`、`transport_semantic_id`、`operation`、`mapping_version` | `matches_sdk_event(SdkEventName sdk_event_name)`、`assert_transport_semantic(...)` |
...
| `CoreContractRef` / `TransportSemanticId` / `FakeBoundaryRef` / `LanguageRuntimeRef` / `PackageArtifactRef` / `RunnerRef` | 当前作为字段类型或引用类型，不独立承担对象责任 |
...
| Outbound Event | `SdkSnapshotFreshnessChangedEvent` | `DerivedBindingView` / `LanguageBindingView` freshness changed | 通知 candidate generator、maintainer、reports | 使用 `DerivedViewId` 和 `affected_languages`，只传播 PH-02 派生视图 freshness |
| Outbound Event | `SdkClientViewFreshnessChangedEvent` | `ServiceClientView` / `BusEventClientView` freshness changed | 通知 candidate generator、maintainer、reports | 使用 `ServiceViewId` 或 `EventViewId`，不携带 `affected_languages` |
...
| `RefreshDerivedBindingView` | entry -> `ContractConsumptionService.refresh_views(...)` -> source ports -> application extraction -> `DerivedBindingView.from_upstream_refs_and_symbols(...)` -> `SnapshotFreshnessState.mark_fresh(...)` | 上游 snapshot 先在 application 层提取 `UpstreamVersionRef` / `CapabilitySymbol`，再形成派生视图和 freshness，不复制上游 truth |

...
|---|---|---|---|---|---|---|
| `TC-SDK-CONTRACT-001` | 上游 snapshot 正常派生 | P0 | `RefreshDerivedBindingView` with current semantic baseline / concept map | view refreshed | `SnapshotFreshnessState::Fresh`；derived / language view 保存；language view 使用 current concept map | contract + service |
...
| `TC-SDK-CONTRACT-003` | upstream changed 标记 stale | P0 | consume changed event | affected views stale | `SnapshotFreshnessState::Stale`；core changed 产出 `SdkSnapshotFreshnessChangedEvent`，bus / formal changed 产出 `SdkClientViewFreshnessChangedEvent` | service + integration |
...
| `TC-SDK-BOUNDARY-001` | service capability 最小调用 | P0 | `InvokeServiceCapability` 携带 `ServiceCapabilityRefId` | result ref / diagnostic ref | lookup 完整 `ServiceCapabilityRef`；SDK domain truth 写入 0；runtime 幂等记录可审计 | service + integration |
...
| `TC-SDK-EVENT-001` | event semantic mapping 正常 | P0 | `PublishBusEvent` 携带 `EventSemanticMappingRef` | boundary 返回 publish ref | lookup 完整 `EventSemanticMapping`；不生成 bus delivery truth；runtime 幂等记录可审计 | service + contract |
...
| `TC-SDK-CANDIDATE-002` | build language packages | P0 | `BuildLanguagePackages` | artifact metadata attached with source `language_view_id` | `Built` 不是状态；artifact 缺来源 language view | integration |
...
| `DS-SDK-BOUNDARY` | service / fake boundary | fake adapter seed + `ServiceCapabilityRefId` | `run_id + capability_id` | reset adapter state | `TC-SDK-BOUNDARY-*` |
| `DS-SDK-EVENT` | bus event mapping | fixture semantic mapping + `EventSemanticMappingRef` | `run_id + event_name` | reset event boundary | `TC-SDK-EVENT-*` |

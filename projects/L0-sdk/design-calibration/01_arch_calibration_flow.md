...
> 本文件是 `projects/L0-sdk/01-架构设计.md` 的 SOP 校准工作台。
...
| 待校准架构设计 | `projects/L0-sdk/01-架构设计.md` |
...
本轮不是扩写旧版 `01-架构设计.md`,而是基于已收稳的 `00-需求文档.md` 重新校准 `L0-sdk` 的架构边界。

目标输出:

```text
1. 明确 L0-sdk 是三语言官方客户端接入层,不是 binding-only 仓、服务端 gateway、auth provider、UI 组件库或 runtime 框架。
2. 明确 L0-sdk 对 L0-core / L0-bus 的编译期依赖,以及对 L1/L2/L3/L4 正式 API 的运行期封装边界。
3. 明确 Rust / Python / TypeScript 三语言一致性、版本兼容、trace、error mapping、redaction 和文档示例的架构约束。
4. 清除旧版公共注册表 P0、完整 MCP / REST / GraphQL / REPL 混入主线、三语言目录提前固化等过期口径。
...
- 架构阶段不得写 Rust struct、Python class、TypeScript interface、handler、repository、测试用例或实施命令。

...
> 本文件是 `projects/L0-sdk/02-概要设计.md` 的 SOP 校准工作台。
...
| 待校准概要设计 | `projects/L0-sdk/02-概要设计.md` |
...
本轮不是继续扩写旧版 `02-概要设计.md`,而是在已经稳定的需求和架构边界下,把 `L0-sdk` 概要设计重校准成可支撑 `03-详细设计.md` 的代码主体骨架。

目标输出:

```text
1. 02 只承接需求与架构结论,不重写需求、架构取舍或旧 SDK 草案。
2. 02 按最新 14 步 SOP 和 14 章正式主链组织。
3. 02 把官方三语言客户端接入层落到代码主体框架、主要组成部分、关键对象、接口骨架、处理流、状态机和配置影响轮廓。
4. 02 清除旧版 binding / wrapper / public registry 主线,改为上游契约派生视图、官方语义封装、formal API / fake boundary adapter、event client view、package candidate 和横切默认主线。
...
| 样例 | 可参考内容 | 不可机械套用内容 |
|---|---|---|
| `projects/L0-core/02-概要设计.md` | L0 底座仓如何从需求 / 架构落到代码主体骨架 | L0-core 的契约真相、发布基线、兼容性对象不能搬到 L0-sdk |
| `projects/L0-bus/02-概要设计.md` | L0 协作仓如何组织主要组成部分、关键对象、接口、处理流和状态机 | publication、delivery、feedback、recovery 等 bus 业务结论不能搬到 L0-sdk |
| `projects/L3-method-library/02-概要设计.md` | 代码主体框架、关键对象、接口骨架、处理流和状态机的组织方式 | method content、definition、snapshot、fingerprint、qualification 等业务结论不能搬到 L0-sdk |
| `projects/L0-sdk/00-需求文档.md` | 本仓定位、目标、核心能力闭环、功能需求、数据归属、接口依赖、验收和风险 | 需求章节本身不能在概要设计中重复展开 |
| `projects/L0-sdk/01-架构设计.md` | 职责边界、系统上下文、限界上下文、容器、依赖方向、数据所有权、交互方式、技术选择和风险 | 架构级取舍不能在概要设计中重新打开 |

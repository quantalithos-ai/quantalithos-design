# L1-artifact 03 详细设计校准工作台

> 对应 SOP: `standards/document/详细设计讨论流程_SOP.md`
> 中间产物规范: `standards/document/设计文档讨论中间产物规范.md`
> 可落码性标准: `standards/document/设计真相源闭环与可落码性标准.md`
> 目标正式文档: `projects/L1-artifact/03-详细设计.md`
> 创建日期: 2026-07-03
> 当前状态: Step 1~19 已完成;03 详细设计已装配完成
...
现有 `03-详细设计.md` 是 2026-05-16 旧草稿,带有旧版五部分主线、旧对象命名和旧结构组织,只能作为问题诊断输入。正式详细设计必须由本轮 `03_ddd_step_*` 中间产物逐步装配,不得从旧文档直接继承未重新校准的对象、接口、流程、状态或目录结构。
...
| 输入 | 权威级别 | 用途 |
|---|---|---|
| `projects/L1-artifact/00-需求文档.md` | 正式上游 | 仓定位、需求边界、业务规则、数据归属和验收红线 |
| `projects/L1-artifact/01-架构设计.md` | 正式上游 | 系统边界、依赖方向、数据所有权、一致性、交互方式和技术中立边界 |
| `projects/L1-artifact/02-概要设计.md` | 直接输入 | 代码主体框架、10 个主要组成部分、关键对象轮廓、接口骨架、处理流、状态机、异常边界和配置影响 |
| `projects/L1-artifact/design-calibration/02_hld_*` | 解释性输入 | 理解概要设计结论来源;若与正式 `02` 冲突,以正式 `02` 为准 |
| `projects/L1-artifact/03-详细设计.md` | 旧草稿 / 问题诊断输入 | 识别不得继承的旧主线、旧结构和旧对象口径 |
...
- 对象契约、协议契约、处理流、状态机、幂等、Query view、metadata / idempotency 和 phase boundary 必须执行 `设计真相源闭环与可落码性标准.md`。
- 若正式 `03` 与对应 Step 冲突,以正式 `03` 为准;若正式 `03` 只写摘要且无法直接落码,继续读取对应 Step 中间产物;读取后仍不清楚时暂停回 Step 19 修正文档,不得交给实现者自行选边。
- 旧 `03-详细设计.md` 中仍适用的事实必须通过新版 `00/01/02` 或本轮 `03_ddd_step_*` 中间产物重新进入,不得直接继承。
- 每次用户确认只推进一个当前 Step;当前 Step 完成后暂停,等待用户确认进入下一 Step。

---

## 5. 下一步

当前已完成 Step 19。正式 `03-详细设计.md` 已由 Step 1~18 中间产物装配完成。

```text
等待用户审查后进入后续正式 `04/05/06/07` 文档链
```

...
| 输出文件 | `projects/L1-artifact/design-calibration/03_ddd_step_18_risks_open_questions.md` |
...
- 目标实现仓 `/home/aris/Projects/quantalithos-artifact` 在 Step 3 / Step 4 检查时未发现,这不阻塞设计 SOP,但阻塞正式代码开工。
...
| 哪些问题会阻塞实现,哪些只影响后续优化? | Step 19 正式 `03` 未装配、`07` 未生成并完成逐 boundary 审计、目标实现仓未确认会阻塞正式实现开工。durable DB / bus / metric backend / DLQ / archive /真实相邻仓 adapter 未定不阻塞 P0 fake / in-memory 闭环,但阻塞生产化 adapter 和真实验收。 |
...
| 议题 | 方案 | 取舍 |
|---|---|---|
| 是否把所有历史 open item 继续列入风险 | A. 全部保留;B. 只保留仍未关闭且影响实现 / 移交的事项 | 采用 B。Step 6 的后续闭合项已由 Step 7~17 承接,不重复列为未关闭风险 |
...
```text
L1-artifact detailed design risks
  |
  ...
  |     +-- old ArtifactRequest / Exception / RiskAcceptance must not re-enter
  |
...
| Query response / page / marker 不闭合 | Step 8 / Step 9 / Step 17 §12.4 | Query 的 response surface、empty / not visible / degraded / stale / failed 口径已预复核 |
| 状态机闭合缺口 | Step 10 / Step 16 / Step 17 §12.5 | 状态集合、迁移和测试入口已对齐 |
| port / persistence 读取面缺口 | Step 7 / Step 11 | repository、snapshot、projection、relay、stored result 和 UoW 读取 / 写入面已定义 |
...
| `/home/aris/Projects/quantalithos-artifact` 当前未发现 | 无法创建 Rust workspace、运行实现测试或提交实现仓 commit | 阻塞代码写入 | `07` PH-01 前置检查确认或创建目标仓 | 实施计划维护者 / 实现 agent |
...
| Step 8 Command 数量文字漂移 | 正式 `03`、`07`、测试计划可能出现 22 / 23 双口径 | 阻塞正式 `03` 装配通过 | Step 19 统一为正式总表口径 | 详细设计维护者 |
| 旧 `ArtifactRequest / Exception / RiskAcceptance` 主线残留 | 实现者可能按旧草稿恢复旧对象模型 | 阻塞正式 `03` 装配通过 | Step 19 明确旧 `03` 只作诊断,正式对象以 Step 6 / 8 / 10 为准 | 详细设计维护者 |
...
| 外部下游导出目标和格式未锁定 | 外部下游导出 job 只能按 marker / port 语义实现,无法绑定具体产品 | 不阻塞 P0 disabled / fake;阻塞真实导出 | 默认 disabled / fake;真实 target 由配置和 adapter design 固定 | downstream export owner / 实施计划维护者 |
...
| 目标实现仓路径和创建方式 | `/home/aris/Projects/quantalithos-artifact` 当前未发现 | 实施计划维护者 / 实现 agent | PH-01 前暂停代码写入 |
...
| 外部下游导出目标 | 影响 export job 的真实 adapter 和格式 | downstream export owner / 实施计划维护者 | 默认 disabled / fake,不写死产品格式 |
...
| 外部下游导出目标未定 | 保持 disabled / fake target,不得把外部产品文档格式写进 core truth |
...
- 必须标明 Step 8 Command 数量漂移由 Step 19 统一修正为正式总表口径。

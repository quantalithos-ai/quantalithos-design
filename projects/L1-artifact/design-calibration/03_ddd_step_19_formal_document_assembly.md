...
> 回填章节: `projects/L1-artifact/03-详细设计.md`
...
| 输出文件 | `projects/L1-artifact/design-calibration/03_ddd_step_19_formal_document_assembly.md`;`projects/L1-artifact/03-详细设计.md` |
| 停审方式 | 完成本 Step 后暂停,由用户审查正式 `03` 后再进入后续文档 |

## 2. 本步目标

本 Step 将 Step 1~18 已确认的详细设计校准结论装配为正式 `03-详细设计.md`。正式文档作为实现者的正式入口,字段级对象契约、trait 签名、DTO schema、逐接口 flow、状态矩阵和持久化语义保留在对应 `design-calibration/03_ddd_step_*.md` 中。

正式 `03` 不复制 Step 6~9 的全部字段级对象契约、trait 签名、DTO schema 和逐接口 flow。字段级真相仍保留在对应 Step 文件中。正式 `03` 的职责是:
...
| 7. API / Command / Query / Event / Job 协议契约 | Step 8 | 使用正式 protocol 总表口径 |
...
| 目标实现仓 | 继续记录目标实现仓在 Step 3 / Step 4 检查时未发现,作为实现开工前置门禁 |
...
| 条件 | 状态 | 说明 |
|---|---|---|
| 正式 `03` 已按 18 章主链装配 | 通过 | 见 `projects/L1-artifact/03-详细设计.md` |

...
| 输出文件 | `projects/L1-artifact/design-calibration/06_acceptance_step_04_entry_exit.md` |
| 停审方式 | 完成本 Step 后暂停,由用户审查后再进入 Step 5 |

## 2. 本步目标

定义正式验收何时可以开始、何时可以结束、何时必须暂停、何时不可裁决。
...
| `05-测试方案.md` §13 | 已完成 | 提供 `EV-CAND-ART-*`、artifact/report/acceptance/review 证据结构 |
...
| 哪些缺陷会阻断进入验收? | 未关闭 S 级缺陷、未复验 VF-ART 相关缺陷、redaction / dependency / evidence integrity 缺陷、P0 profile 无法装配、缺少 raw artifact/report pairing、实现或设计基线未固定均阻断进入。 |
...
| 旧 `06-验收标准.md` §3 | 退出条件只列 P0 验收项、少数高风险链路和 S 级缺陷 | 新版退出条件增加 VETO、EV-CAND、artifact/report pairing、redaction/dependency/report audit、risk acceptance |
...
- [ ] `EV-CAND-ART-*` evidence index 无 orphan EV、无静态造证据、无缺 artifact digest。
- [ ] `VF-ART-001~004` 均有真实 evidence 支撑未触发,且 `veto-checklist.md` 未静态默认 passed。
...
| P0 suite / report / evidence | `05` §9 / §13 | blocking suite、`EV-CAND-ART-*`、artifact/report pairing |
| 缺陷 / 复验 / 风险接受 | `05` §11 | S/A/B/R 和复验要求 |
| residual / P1/P2 | Step 2;`05` §14 | P1/P2 不阻断 P0,但必须记录 |
| VETO / 红线 | `00` §14;Step 2;后续 Step 11 | `VF-ART-001~004` 不可风险接受 |
...
- 退出验收必须满足所有 P0 验收项有裁决、`EV-CAND-ART-*`、`VF-ART-*`、`TC-ART-*` 可追溯、VETO 未触发、S=0、A 级缺陷修复或接受、residual 有接受人。

...
| `03-详细设计.md` | `ConversationRuntimeConfig`、`ConfigLoader`、`ConfigValidator`、runtime builder、adapter、store、job 和报告路径绑定点 | JSON 配置项、加载校验、敏感引用、失效模式和下游承接 |
...
本文不重新定义需求、架构、概要和详细设计中的代码契约。凡是会改变 `ConversationRuntimeConfig` 字段、loader / validator / builder 函数签名、adapter constructor、trait、error、event、job receipt 或 runtime graph 机制的配置结论，必须先回写 `03-详细设计.md`。

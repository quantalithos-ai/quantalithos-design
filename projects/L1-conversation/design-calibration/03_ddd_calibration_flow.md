# L1-conversation 03-详细设计校准流程

> 本文件是 `projects/L1-conversation/03-详细设计.md` 的 SOP 校准工作台。
> 它记录每个 Step 的中间产物位置、状态和回填章节。
> 本目录中的内容是中间产物,不替代正式 `03-详细设计.md`。
>
> 本轮状态说明:
> - 当前正式 `03-详细设计.md` 是旧版草案,只作为问题诊断材料。
> - 旧版 `03-详细设计.md` 仍围绕 Conversation / Turn / StreamEvents / AG-UI / event-to-turn mapping 旧口径展开,与新版 `02-概要设计.md` 的 Conversation truth center、fact、scope、manifestation、trace、projection、outbox 和 handoff 主线不一致。
> - 本轮详细设计直接承接已经重建的 `00-需求文档.md`、`01-架构设计.md` 和 `02-概要设计.md`。
...
| 待校准详细设计 | `projects/L1-conversation/03-详细设计.md` |
...
本轮不是继续扩写旧版 `03-详细设计.md`,而是在已经稳定的需求、架构和概要设计边界下,把 `L1-conversation` 详细设计重校准成可以 1:1 指导实现的代码实现契约。
...
| 样例 | 可参考内容 | 不可机械套用内容 |
|---|---|---|
| `projects/L0-core/design-calibration/03_ddd_*` | L0 底座仓如何按 19 Step 形成详细设计中间产物 | L0-core 的 contract definition、baseline、snapshot、compatibility 对象不能搬到 L1-conversation |
| `projects/L0-bus/design-calibration/03_ddd_*` | L0 协作仓如何做跨文档闭环复核、状态名一致性和 phase boundary | bus publication、delivery、feedback、recovery 业务对象不能搬到 L1-conversation |
| `projects/L0-sdk/design-calibration/03_ddd_*` | SDK 仓如何处理 client boundary、candidate、evidence 和跨文档真相源冲突 | SDK semantic baseline、binding view、language package 和 candidate 对象不能搬到 L1-conversation |
| `projects/L1-identity/design-calibration/03_ddd_*` | L1 仓如何定义 actor / member 引用和身份视角相关契约 | Identity lifecycle、member truth、role assignment 不能搬到 L1-conversation |
| `projects/L1-conversation/02-概要设计.md` | 本仓代码主体框架、主要组成部分、对象、接口、处理流、状态机和配置影响 | 概要设计的轮廓不能直接当成完整实现契约 |

...
> 本文件是 `projects/L1-conversation/04-配置设计.md` 的 Step 6 中间产物。
...
- 回填章节: `projects/L1-conversation/04-配置设计.md` §6 环境、部署 profile 与配置矩阵
...
| `03-详细设计.md` §13 / §15 | 配置引用、外部依赖绑定、脚本和报告目录 | 将 profile 差异映射到既有配置控制面和报告路径 |

已确认结论:

```text
L1-conversation 的 profile 是配置矩阵分类,不是新的详细设计代码契约。
...
| 环境 | 是否适用 | 当前口径 |
|---|---|---|
| local | 适用,P0 | 使用 in-memory store、fake resolver、fake publisher、fake handoff 和 local reports 跑通默认可验证路径 |
...
| operations-replay | 适用,P0 | 用于 outbox、projection、snapshot、handoff、consistency 和 cursor cleanup 的恢复 / 重跑验证 |
...
P0 local-dev 和 ci-test 不依赖真实外部服务。integration-like 可以接入 controlled fake / configured adapters,但不能要求真实生产 endpoint。operations-replay 依赖本地或脱敏的历史状态、outbox、projection、snapshot、handoff 和 report root。

真实 durable database、real event bus、real identity / work / governance / artifact / runtime / bridge adapter、real observability / archive handoff 和 secret provider 属于 P1/P2 接入,不作为 P0 通过前置。
...
| fake vs configured resolver | unresolved、digest mismatch、source unavailable 和 degraded marker 测试 |
| fake vs configured publisher | outbox publish、retry、failed、suppressed 和 fake marker 验收 |
...
| operations-replay | rerun、partial failure、idempotency window 和 consistency diagnostic 验收 |

## 4. 当前文档问题诊断

| 位置 | 当前问题 | 影响 |
|---|---|---|
| `04-配置设计.md` | 尚未存在 profile 配置矩阵 | Step 7 无法判断默认值和环境差异 |
...
| 当前旧 `05/06` | 未按新版 profile 重校准 | 后续测试验收必须按本步重写 |
...
| 测试验收承接 | 未定位差异项 | 明确 store、resolver、publisher、handoff、report、redaction、replay 的测试影响 | 支撑 `05/06` 重校准 |
...
- L1-conversation 既有 api / worker / jobs,也有 outbox、projection、snapshot、handoff 和 consistency job,只写 dev / test / prod 不足以指导测试。
...
| operations-replay | replay、rerun、partial failure、idempotency window、consistency report | 恢复不产生第二 truth,证据路径稳定 |

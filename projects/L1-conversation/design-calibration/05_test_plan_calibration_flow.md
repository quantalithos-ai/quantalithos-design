# L1-conversation 05-测试方案校准流程

> 本文件是 `projects/L1-conversation/05-测试方案.md` 的 SOP 校准工作台。
> 它记录每个 Step 的中间产物位置、状态、回填章节和当前处理状态。
> 本目录中的内容是中间产物，不替代正式 `05-测试方案.md`。
>
> 本轮状态说明:
> - 当前 `projects/L1-conversation/05-测试方案.md` 是 2026-05-16 旧版草案。
> - 旧版 `05` 仍围绕 Conversation / Turn / StreamEvents / projection 的旧主线展开，未承接新版 `00~04` 中的 conversation truth center、authorization scope、cross-domain manifestation、handoff、configuration、reports / artifacts 和详细设计 14 组状态机。
...
| 当前旧测试方案 | `projects/L1-conversation/05-测试方案.md` |
...
本轮不是修补旧版 `05-测试方案.md`，而是按新版 `00~04` 重新生成 L1-conversation 测试方案。
...
3. 05 从 conversation truth center、space / scope、fact append、authorization scope、cross-domain manifestation、projection、outbox、handoff、configuration、observability 和 reports / artifacts 主线抽取测试对象。
...
- 允许参考已收稳的 `L0-core`、`L0-bus`、`L0-sdk`、`L1-identity` 和 `L3-method-library` 前序中间产物，但不能机械搬运其他子项目的测试用例。

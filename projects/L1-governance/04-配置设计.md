...
### 5.1 Jobs entry-local 参数 schema

Jobs crate binaries share one entry-local schema. Entry-local 参数只影响当前 binary invocation,不进入 public protocol DTO,不写 domain truth,不覆盖 startup invariant。

| 参数 | CLI flag | Env key | 默认 / 缺失处理 | 适用 binary | DTO 承载边界 |
|---|---|---|---|---|---|
| config source selector | `--config <path>` | `GOVERNANCE_CONFIG` | 缺失使用默认 config discovery;flag/env 同时存在且值不同则 entry rejected | all jobs bins | local `GovernanceJobsEntryLocalArgs.config_path` |
| profile selector | `--profile <profile>` | `GOVERNANCE_PROFILE` | 缺失使用 config/default profile;unknown profile rejected | all jobs bins | local `GovernanceJobsEntryLocalArgs.profile` |
| job request file | `--job-request <path>` | `GOVERNANCE_JOB_REQUEST` | 与 stdin source 必须二选一;缺失或两个同时存在 rejected | all jobs bins | local `GovernanceJobsEntryRequestSource::File`;file body 是完整 `GovernanceJobRequest<T>` |
| job request stdin | `--job-request-stdin` | `GOVERNANCE_JOB_REQUEST_STDIN=true` | 与 file source 必须二选一;truthy 值仅允许 `true` | all jobs bins | local `GovernanceJobsEntryRequestSource::Stdin`;stdin body 是完整 `GovernanceJobRequest<T>` |
| artifact output root | `--artifact-root <path>` | `GOVERNANCE_ARTIFACT_ROOT` | 缺失默认 `artifacts/test/<run_id>`;`run_id` 从 job request metadata 读取 | all jobs bins | local output path;不进入 application request |
| report output root | `--report-root <path>` | `GOVERNANCE_REPORT_ROOT` | 缺失默认 `reports/runs/<run_id>`;`run_id` 从 job request metadata 读取 | all jobs bins | local output path;不进入 application request |
| dry-run diagnostics | `--dry-run-diagnostics` | `GOVERNANCE_JOB_DRY_RUN_DIAGNOSTICS=true` | 默认 `false`;truthy 值仅允许 `true` | all jobs bins | local selector;为 true 时只做 validation / diagnostic artifact,不得调用 application mutation |

Binary 到 request envelope 的绑定:
...
- 不定义 `--run-id`、`--idempotency-key`、`--actor-ref`、`--trace-id`、`--scope`、`--target` 这类 protocol metadata 同义 flag;这些值只来自 `GovernanceJobRequest<T>`。
- Env 不承载完整 job input body;env 只能承载 selector、path 或 boolean。
- `artifact_root` / `report_root` 是运行产物路径,不是 application job input,不得进入 `GovernanceJobReport` 或 stored job result。

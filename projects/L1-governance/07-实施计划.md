...
| PH-07 | commit-07-d | 建立 jobs crate entry, runner wiring,正式 entry-local request loading and operations artifact output | entry-worker-job;operations-replay-core |
...
| artifact / evidence | artifact root、report path、raw JSON schema、digest、redaction、EV/VETO 来源真实 |
| entry local args | binary flag/env、request source、artifact/report root、dry-run selector 和 DTO 承载边界已闭合 |
...
- `GovernanceTruthChangeSubjectMapper` 的 PH-04 subject helpers:policy fact、shared rule set、policy conflict、control applicability、control review、compliance conclusion、nonconformity。
- `ExternalGovernanceSourceResolverPort.resolve_scope_subject_relation(...)` 及 fake resolver fixture:policy/shared rules flow 必须用 `GovernanceScopeSubjectRelation` 的 `Allowed/Mismatch/Unknown` 判定 subject/scope mismatch,不得解析 ref 字符串或做恒真同一性检查。
...
- PH-04 outbound payload DTO:`PolicyEffectiveFactChangedPayload`、`SharedRuleSetChangedPayload`、`PolicyConflictChangedPayload`、`ControlApplicabilityChangedPayload`、`ControlReviewChangedPayload`、`ComplianceConclusionChangedPayload`、`NonconformityChangedPayload`。

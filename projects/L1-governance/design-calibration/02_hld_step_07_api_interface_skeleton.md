...
| `ControlApplicabilityChanged` | `ControlApplicability` change | control applicability ref + applicability state + trace context | artifact、workspace、archive、external GRC export | 不携带 ControlDefinition 或 evidence body |
| `ControlReviewChanged` | `ControlReview` change | review ref + applicability ref + review state + reviewer / evidence refs + trace context | artifact、workspace、archive、external GRC export | 不携带 ControlDefinition、evidence body 或 reviewer profile |

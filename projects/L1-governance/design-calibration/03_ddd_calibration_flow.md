...
现有 `03-详细设计.md` 是 2026-05-16 旧草稿,包含旧版 `GovernanceRequest / Gate / Decision / Exception / RiskAcceptance` 教学主线和旧采集式结构,只能作为问题诊断输入。正式详细设计必须由本轮 `03_ddd_step_*` 中间产物逐步装配,不得从旧文档直接继承未重新校准的对象、接口、流程或状态。
...
| 输入 | 权威级别 | 用途 |
|---|---|---|
| `projects/L1-governance/00-需求文档.md` | 正式上游 | 仓定位、需求边界、数据归属、业务规则、验收红线 |
| `projects/L1-governance/01-架构设计.md` | 正式上游 | 系统边界、依赖方向、数据所有权、一致性和通信方式 |
| `projects/L1-governance/02-概要设计.md` | 直接输入 | 代码主体框架、主要组成部分、对象轮廓、接口骨架、处理流、状态机 |
| `projects/L1-governance/design-calibration/02_hld_*` | 解释性输入 | 理解概要设计结论来源;若与正式 `02` 冲突,以正式 `02` 为准 |
| `projects/L1-governance/03-详细设计.md` | 旧草稿 / 问题诊断输入 | 识别旧主线残留;不得作为新版详细设计真相源 |

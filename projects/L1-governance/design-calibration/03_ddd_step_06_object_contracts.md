...
| `GovernanceTruthCursor` | committed truth / reference marker / trace 位置 | command accepted path 由 Step 7 `GovernanceUnitOfWork.assign_truth_change_cursor()` 在 truth save 已进入同一 UoW 后分配;consumer/reference-refresh path 由 Step 7 `GovernanceUnitOfWork.assign_reference_change_cursor()` 在 reference state / typed snapshot save 已进入同一 UoW 后分配;query/projection/job read path 可来自 committed truth scan、trace cursor、projection source cursor 或正式 job input cursor | 不作为 optimistic version;不得由 page cursor、version、timestamp、id generator、trace id、idempotency digest、dedup key 或 hard-coded string 推导 |
...
| reference-only cursor 不等于 truth change | consumer / refresh 只更新 reference/snapshot/stale marker 时,`assign_reference_change_cursor()` 只表达 reference marker 提交位置;不得伪造 `GovernanceTruthChange`、outbox payload 或 accepted core truth |
...
| external GRC export marker trace subject 有正式 helper | `PrepareExternalGrcExportFlow` 若 Step 9/10 要求 `GovernanceTraceRecord::from_marker(...)`,subject 必须来自 `GovernanceHandoffMarkerSubjectMapper.external_grc_export_marker_subject(truth_snapshot.scope_ref,target_ref)`,其 canonical key 为 `governance:external-grc-export-marker:<scope_ref.scope_ref>:<target_ref.0>`;两个 ref 值均按完整 opaque value 处理,不得被业务逻辑解析 |
...
/// Formal publisher result after attempting to publish a stored outbox payload snapshot.
pub enum OutboxPublisherOutcome {
    /// Publisher accepted the payload for outbound delivery.
    Published(OutboxPublicationRef),
    /// Publisher failed in a retryable way; outbox remains retryable failed.
    RetryableFailed(OutboxFailureReason),
    /// Publisher failed permanently; outbox must move to dead-letter.
    FatalFailed(OutboxDeadLetterReason),
}
...
| `OutboxPublisherOutcome` | publisher 发布结果分类 | `Published` 映射 `mark_published`;`RetryableFailed` 映射 `mark_failed`;`FatalFailed` 映射 `mark_dead_lettered`;实现不得自行从字符串 / 异常类型推断 |
...
pub struct GovernanceJobReport {
    ...
    /// Outbox records scanned by PublishGovernanceOutbox.
    pub scanned_outbox_refs: GovernanceOutboxRefSet,
    /// Outbox records successfully published by PublishGovernanceOutbox.
...
pub struct GovernanceJobReportAssembly {
    ...
    pub operation_name: GovernanceOperationName,
    /// Outbox records scanned by an outbox job.
    pub scanned_outbox_refs: GovernanceOutboxRefSet,
    /// Outbox records successfully published by an outbox job.
    pub published_outbox_refs: GovernanceOutboxRefSet,
    /// Outbox records that failed or were dead-lettered by an outbox job.
    pub failed_outbox_refs: GovernanceOutboxRefSet,
    /// Derived views rebuilt, marked stale, or inspected by maintenance jobs.
    pub view_refs: DerivedGovernanceViewRefSet,
...
| `scanned_outbox_refs` | `GovernanceOutboxRefSet` | outbox publish / retry 扫描对象 | 来源于 application outbox service pending page;只保存 ref |
| `published_outbox_refs` | `GovernanceOutboxRefSet` | outbox publish 成功对象 | 来源于 `OutboxPublisherOutcome::Published` branch;只保存 ref |
| `failed_outbox_refs` | `GovernanceOutboxRefSet` | outbox publish 失败 / dead-letter 对象 | 来源于 application mark failed / dead-letter branch;只保存 ref |
...
| 函数签名 | 作用 | 参数说明 | 返回 | 副作用 / 不变量 |
|---|---|---|---|---|
| `pub fn record_outbox_scan(&mut self, outbox_refs: GovernanceOutboxRefSet) -> Result<(), ApplicationError>` | 记录 outbox pending scan 结果 | scanned outbox refs | `Result<(), ApplicationError>` | 累加 scanned refs 和 scanned_count;不读取 payload body |
| `pub fn record_outbox_published(&mut self, outbox_ref: GovernanceOutboxRef) -> Result<(), ApplicationError>` | 记录 outbox publish success | published outbox ref | `Result<(), ApplicationError>` | 累加 published refs 和 changed_count;不调用 publisher |
...
|---|---|---|
| `PublishGovernanceOutbox` | `scanned_outbox_refs`、`published_outbox_refs`、`failed_outbox_refs`、`scanned_count`、`changed_count`、`failed_count` | 不按 current truth 重构 payload;不回滚 accepted truth;worker loop 不从 repository 反推 success/failed refs |
...
| outbox counters 不替代 outbox result refs | PublishGovernanceOutbox 必须保存 scanned/published/failed outbox refs;worker loop entry 只能从 report 复制这些集合 |
...
| `source_cursor` | truth snapshot/change、reference marker、view state、trace、outbox、dashboard、report、projection worker | `GovernanceTruthCursor` | command accepted path: Step 7 `GovernanceUnitOfWork.assign_truth_change_cursor()` after truth save in same UoW;consumer/reference-refresh path: Step 7 `GovernanceUnitOfWork.assign_reference_change_cursor()` after reference/snapshot save in same UoW;read/job path: committed truth scan、trace cursor、projection state、formal job input | Step 7 UoW helper/read ports;Step 11 cursor ordering | cursor 不得当 optimistic version、page cursor、source/snapshot version、timestamp、trace id、event dedup key、idempotency digest 或 lock token |
...
| projection / reference local state | `DerivedGovernanceViewState`、external snapshot / mirror、reference state helpers | view/ref/snapshot identity 来自 typed ref;freshness/degraded/visibility from projection/reference policy;command stale source_cursor from committed truth cursor;consumer/refresh stale source_cursor from committed reference marker cursor;reference state from consumer/refresh outcome | Step 7 projection repository、affected view lookup、reference snapshot repository、UoW cursor helpers;Step 9 query/consumer/refresh flow;Step 11 projection identity and stale persistence | affected views 无正式读取面;reference refresh 成功/失败缺 version 来源;consumer/refresh 用 source version、dedup key、timestamp 或 idempotency digest 伪造 stale cursor;query 试图直接修复 stale/unresolved state |
...
| GVN-S6-OPEN-012 | observability/audit 必须定义 trace subject、audit source、history kind、handoff target、redacted issue refs 的正式映射 | Step 15 | trace/audit/history/handoff object;consumer reference marker trace 已由 Step 7 `GovernanceReferenceMarkerSubjectMapper` 闭合;external GRC export marker trace 已由 Step 7 `GovernanceHandoffMarkerSubjectMapper` 闭合 | 其他 job report / handoff marker 若要求 trace 或 audit,仍需正式 mapper / audit source |

...
| `ConsumeArtifactEvidenceChangedFlow` | reserve dedup; derive `reference_ref = envelope.source_ref`; read current reference state version for `reference_ref`; call `save_evidence_summary_ref(reference_ref, payload.evidence_ref, expected_version, uow)`; save reference state from envelope with same expected_version; stale compliance/control views; `marker_subject_ref = GovernanceReferenceMarkerSubjectMapper.reference_marker_subject(reference_ref)`; append marker trace; receipt | evidence/artifact body present; missing evidence ref; attempting to use `payload.evidence_ref.external_ref` as bundle key |

...
| From | To | 触发函数 | Step 9 flow | 前置条件 | 状态副作用 | Flow 副作用 | 非法时错误 |
|---|---|---|---|---|---|---|---|
| factory | `Fresh` | `DerivedGovernanceViewState::for_view(...)` | projection state init / `RebuildGovernanceProjectionsFlow` missing-state first materialization | formal `DerivedGovernanceViewRef` exists;source cursor present;expected version `None`;typed view body built in same UoW | create state with `Fresh`;failure ref empty | save/replace view state with first view body in projection repo;do not call `start_rebuild(...)` or `mark_fresh(...)` in this branch | `DomainError::InvalidStateTransition` |

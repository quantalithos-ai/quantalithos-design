...
| `save_evidence_summary_ref(reference_ref, summary_ref, expected_version, uow)` | 保存 evidence summary marker under the explicit reference bundle | `reference_ref` must be the inbound envelope source ref / `ReferenceResolutionState.reference_ref`;`expected_version` is the same reference bundle version;same UoW as reference state;no artifact/evidence body;`summary_ref.external_ref` is not a bundle key | `EvidenceSummaryRef` | version conflict |

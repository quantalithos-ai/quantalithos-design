...
| `ApprovalResponsibilityChangedAudit` | `AssignApprovalResponsibilityFlow` / `RecordApprovalVoteFlow` / `DelegateApprovalResponsibilityFlow` accepted 后 | `trace_record_ref`, `responsibility_record_ref`, `actor_ref`, `responsibility_ref`, `chain_ref`, `from_state`, `to_state`, `vote_optional`, `outbox_ref` | decision views / downstream |

...
| `ConsumeArtifactEvidenceChanged_event` | `ArtifactEvidenceChangedPayload` | evidence summary ref saved under explicit envelope source `reference_ref` + reference state saved with same bundle version;digest mismatch/body rejected;unsupported version;using `evidence_ref.external_ref` as bundle key rejected by test;affected compliance/control views stale | consumer |
...
| `RebuildGovernanceProjections_job` | `RebuildGovernanceProjectionsFlow` | rebuild selected projections from `GovernanceTruthSnapshot`;missing state first materialization creates `Fresh` state with first view body and does not enter `Rebuilding`;existing `Stale` / `Failed` / `Unavailable` state uses `start_rebuild -> mark_fresh`;replace view/state/dependency index;partial failure report carries rebuilt / inspected `view_refs`;duplicate report replay returns same `view_refs`;no truth repair | job runner |
| `RefreshExternalContextSnapshots_job` | `RefreshExternalContextSnapshotsFlow` | `ExplicitRefs` / `UnhealthyReferences` / `GovernanceScope` expansion through tracked reference states;resolver dispatch uses `ReferenceResolutionState.refresh_target` not ref string;resolver success/failure;versioned save;affected views stale;report carries `refreshed_reference_refs` / `failed_reference_refs`;duplicate replay returns same item refs | job runner |

...
| `Gate` | `gate_ref` / `requirement_ref` / `required_responsibility_ref` | typed refs | id generator + command intent / created responsibility ref | `Gate::open(...)`;requirement path then `request_decision_by_ref(...)` | `OpenGovernanceGateRequest.*`;`GateCommandResult.required_responsibility_ref` | no requirement -> final `Open` and no responsibility ref;invalid requirement -> reject;RecordDecision must receive preexisting `PendingDecision` | gate command tests | 后续 `06` |

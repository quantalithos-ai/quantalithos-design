...
| `GovernanceInput` | `input_ref` / `context_ref` / `input_source_ref` | typed refs | id generator + command body + repository lookup | `GovernanceInput::submit(...)` | `SubmitGovernanceInputRequest.*` | missing context / source unresolved -> reject | input command tests | 后续 `06` |
| `Gate` | `gate_ref` / `requirement_ref` | typed refs | id generator + command intent / policy lookup | `Gate::open(...)` | `OpenGovernanceGateRequest.*` | missing requirement -> reject | gate command tests | 后续 `06` |

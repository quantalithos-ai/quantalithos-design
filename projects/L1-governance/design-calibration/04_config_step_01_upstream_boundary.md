...
| 当前状态 | 已完成;待用户审查 |
| 输入基线 | 新版 `00/01/02/03`;旧 `05/06` 仅作待复核方向输入 |
| 输出文件 | `projects/L1-governance/design-calibration/04_config_step_01_upstream_boundary.md` |
| 停审方式 | 完成本 Step 后暂停,由用户审查后再进入 Step 2 |

## 2. 本步目标

确认 `L1-governance` 配置设计依赖的需求、架构、概要、详细设计和下游输入是否足够,并明确哪些配置线索可以作为正式配置设计输入。
...
| 问题 | 回答 |
|---|---|
| 当前配置设计要承接哪些需求、非功能、安全和环境差异? | 承接 Governance truth 独立性、正文排除、唯一编译期依赖、外部依赖延迟可发现、外围增强失效不影响核心闭环、敏感 / forbidden body 不落日志审计和配置不得越界等要求。环境差异先只承接旧 `05/06` 中 test / staging 方向,正式环境矩阵留给 Step 6 重新收敛。 |
...
| 来源文档 | 配置输入 | 回填章节 |
|---|---|---|
| `00-需求文档.md` | `L0-core` 唯一编译期依赖;外部系统和相邻仓只通过运行期 / 事件 / ref / snapshot / handoff 协作;外部依赖延迟必须可发现;外围增强失效不影响核心闭环;正文排除和配置不得越界 | `04` §1 / §3 / §4 / §11 |
| `01-架构设计.md` | 配置不得弱化 Governance truth、Policy / shared rules、正文排除、派生不反写、依赖裁剪;具体 DB / bus / cache / search / rule engine / external GRC 产品未锁定 | `04` §1 / §4 / §13 / §14 |
...
- Governance truth 是否独立于 process、work、artifact、conversation、runtime、observability 或 external GRC。
- `L0-core` 是否是唯一编译期依赖。
- 是否允许用配置改变 truth ownership、正文排除、shared rules、状态机合法迁移、query no-write、projection 不反写、outbox snapshot 来源或 external GRC 不定义 truth。
- 是否选择具体 DB、message bus、cache、search、object storage、audit store、scheduler、rule engine 或 external GRC 产品。
- 是否重新定义 detailed design 中的 struct / enum / trait / function / DTO / flow / error / state。
- 是否重写测试方案、验收标准、实施计划或部署运维手册。

### 8.3 配置设计必须回答的问题

- L1-governance 有哪些配置控制面和配置域。
- 哪些行为允许配置化,哪些禁止配置化。
- 配置来源、优先级和冲突处理规则是什么。

...
| 输出文件 | `projects/L1-governance/design-calibration/04_config_step_04_categories_boundaries.md` |
| 停审方式 | 完成本 Step 后暂停,由用户审查后再进入 Step 5 |

## 2. 本步目标

本 Step 在 Step 3 的配置控制面基础上,定义 `L1-governance` 的配置类别、更新边界和禁止配置化边界。

本 Step 只回答:

- 当前系统有哪些配置类别。
- 哪些配置类别允许启动读取、job-run-start 冻结或 entry-local 生效。
- P0 是否允许热更新。
- 哪些安全、审计、事务、一致性和领域规则禁止配置化。
- 禁止配置化项如需改变应走什么设计流程。
- 每个配置域适用哪些配置类别,哪些类别明确不适用。
- 每个禁止配置化项是否回指架构红线或详细设计不变量。
...
| `03-详细设计.md` §3 / §5 / §9 / §10 / §11 / §12 / §13 | 已完成 | 提供源码依赖、模块边界、flow、状态、事务、错误、幂等和配置绑定不变量 |
| `03_ddd_step_14_config_external_binding.md` | 已完成 | 提供配置引用表、config section、禁止配置化边界、runtime builder 绑定顺序 |
| `02-概要设计.md` §11 / §13 | 已完成 | 提供配置影响轮廓和禁止配置化边界 |
| `01-架构设计.md` §13~§15 | 已完成 | 提供数据所有权、派生不反写、依赖裁剪、external GRC 不定义 truth 等架构红线 |
| `00-需求文档.md` §10~§14 | 已完成 | 提供正文排除、外部依赖、非功能、安全和验收红线 |

## 4. SOP 问题回答

| 问题 | 回答 |
|---|---|
| 当前系统有哪些启动配置、运行时配置、策略配置、敏感配置、调试配置? | 分类为 static design boundary、startup runtime config、job-run-start config、entry-local parameters、policy-like technical knobs、sensitive ref config、diagnostic / redaction config、test fixture / deterministic config、feature / peripheral enablement。static design boundary 不是可配置项,只作为禁止配置化边界。 |
| 哪些配置允许热更新? | P0 不允许核心 hot update。配置变化通过 process restart 或 new job run 生效。允许的 entry-local 参数只影响当前入口 / 当前 job,不改变全局 runtime。后续若需要 hot reload,必须新增 `03` runtime contract、validation、rollback 和 audit 规则。 |
| 哪些配置只能冷更新或启动读取? | store binding、adapter availability、source resolver / publisher / handoff / export binding、topic map、boundary limits、redaction deny list、idempotency retention、clock / id adapter、feature peripheral enablement 都必须 startup validate 后冻结。 |
| 哪些安全、审计、事务、一致性或领域规则禁止配置化? | Governance truth 归属、外部正文排除、formal decision 边界、shared rules 优先级、domain state matrix、responsibility / capability guard、command metadata / idempotency、accepted trace / audit / outbox / stored result、query no-write、projection / job / reconciliation / handoff 不反写真相、outbox payload snapshot 来源、optimistic version、duplicate replay、visibility guard、external GRC 不定义 truth、唯一 non-core 依赖裁剪都禁止配置化。 |
...
| 每个禁止配置化项是否回指架构红线或详细设计不变量? | 已在禁止配置化项表中回指 `00/01/02/03` 的数据所有权、正文排除、依赖裁剪、flow、状态、事务、幂等、query no-write、projection / job no-truth-repair 和 external GRC 边界。 |
| 每个配置域分类边界完成后是否通过停审? | 已通过。每个配置域都有适用类别、不适用类别、禁止项和更新边界。未发现需要新增 `03` 代码契约。 |
| 所有分类完成后,是否存在同一行为在不同配置域分类不一致或禁止项遗漏? | 已完成跨分类审计。`retry / timeout / batch` 归 technical knobs,不进入 domain policy;`feature enablement` 只控制外围 emission / export,不改变 accepted truth;`test fixture` 只用于 local / CI,不进入 production-like profile。 |
...
| `03` §13 禁止配置化边界 | 已列核心禁止项,但未说明如果要改变应走什么流程 | 本 Step 增加禁止项变更流程 |
...
| policy-like technical knobs 是否等同 Governance Policy | A. 等同;B. 明确只指 retry / timeout / retention / batch 等技术参数 | 采用 B。Governance Policy truth / shared rules 不是配置项 |
| feature flags 是否可改变核心语义 | A. 可打开 / 关闭核心 command 行为;B. 只控制外围 emission / export / derived event | 采用 B。feature flag 不得改变 accepted truth |
...
|---|---|---|---|---|
| static design boundary | 非配置项,用于声明不可被配置覆盖的设计不变量 | truth ownership、正文排除、状态矩阵、query no-write | 不适用 | 若被当成配置项,会绕过需求 / 架构红线 |
...
| entry-local parameters | 只影响当前入口或当前请求 / job 的本地参数 | config source selector、profile selector、job input ref、dry-run diagnostic mode | 不属于 hot update;只影响当前 entry | 如果覆盖全局配置或业务 scope,会绕过 typed DTO |
| policy-like technical knobs | 技术执行策略,不等同 Governance Policy truth | retry、retention、dedup window、page limit、timeout | P0 startup 或 job-run-start 冻结 | 误用于改变 shared rules / domain policy |
...
| diagnostic / redaction config | 控制安全诊断输出和 forbidden field 扫描 | safe diagnostic refs、redaction deny list、metric label allowlist | P0 startup 冻结 | 诊断放宽会泄露 forbidden body 或 high-cardinality label |
...
| feature / peripheral enablement | 控制外围能力是否启用 | trace available event emission、derived view event emission、external GRC export enabled | P0 startup 冻结 | 若控制核心 accepted path,会改变业务语义 |
...
|---|---|---|---|
| Governance truth 归属 | 防止 process / work / artifact / runtime / external GRC 反向定义治理事实 | `00` 数据归属;`01` 数据所有权;`03` §12 | 修改 `00/01/02/03` truth boundary,再同步测试 / 验收 / 实施 |
| 外部正文排除 | 防止保存 method、artifact、conversation、runtime、observability、archive、external GRC 正文 | `00` 禁止保存正文;`03` forbidden field | 修改需求和详细对象契约,不得由配置放宽 |
| formal Gate / Decision 边界 | 防止 UI card、process waiting、runtime cache、report row 替代正式裁决 | `02` Gate / Decision 正式性;`03` 状态矩阵 | 修改概要 / 详细状态和 flow |
| Policy / shared rules 优先级 | 防止低 scope config、runtime default 或 external status 覆盖 shared rules | `01` shared rules 红线;`02` Policy 边界 | 修改 Policy 对象契约和状态矩阵 |
| domain state matrix | 防止 profile 改变合法迁移 | `03` §10 状态矩阵 | 修改 Step 10 状态矩阵和 domain tests |
| actor / capability / visibility guard | 防止配置绕过授权、责任和可见性 | `03` command/query flow;Step 7 ports | 修改安全 / visibility 设计和测试 |
...
| accepted trace / audit / outbox / stored result | 防止 accepted truth 无法追溯、传播或 duplicate replay | `03` §9 / §11 / §15 | 修改 accepted flow、持久化和观测契约 |
...
| handoff / archive / external export no-truth-repair | 防止下游接收状态定义 Governance truth | `01` handoff 边界;`03` jobs | 修改 handoff / export object contract |
...
| adapter availability registry | startup runtime config;feature enablement | domain state config | formal decision、truth lifecycle、shared rules | availability 只影响 dependency surface |
...
| reference snapshot store binding | startup runtime config;job-run-start refresh knobs | external body config | external body exclusion、decision auto-accept | snapshot 只保存 ref / summary / state |
| outbox store binding | startup runtime config;publisher job knobs | payload reconstruction config | outbox snapshot source、publish failure rollback | publisher 只读 stored payload |
...
| reference refresh binding | job-run-start config;source resolver binding | external body / auto-decision config | body exclusion、formal decision boundary | refresh 不创建裁决 |
...
| trace handoff target binding | startup runtime config;sensitive ref config;job-run-start target snapshot | accepted truth config | observability ledger body、handoff no-truth-repair | handoff 是外围交接 |
| archive handoff binding | startup runtime config;sensitive ref config | archive body storage config | archive package body、truth dependency on archive success | archive 失败不回滚 truth |
| external export binding | startup runtime config;feature peripheral enablement;sensitive ref config | external status truth config | external GRC 不定义 truth | export 是下游消费 |
| redaction / safe output binding | startup runtime config;diagnostic config | hot relax config | forbidden body、raw secret、high-cardinality labels | 安全输出必须启动前校验 |
...
| 是否把 Governance Policy truth 当作配置 | 无 | policy-like knobs 仅指 retry / retention / timeout |
| 是否把 feature flag 用于核心语义 | 无 | feature 只控制外围 emission / export |
| 是否把 test override 带入 production-like | 无 | test fixture 仅 local / CI / deterministic |
| 是否遗漏 Step 3 配置域 | 无 | Step 3 配置域均进入 §8.4 |
| 是否遗漏 `03` 禁止配置化边界 | 无 | `03` §13 全部进入 §8.3 |
...
| policy-like technical knobs 不等同 Governance Policy truth | 否 | 分类澄清 | 不适用 | 无回写 |
| feature enablement 只控制外围 event / export / derived capability,不改变 accepted truth | 否 | 承接 Step 14 feature section | 不适用 | 无回写 |
...
- 不得把 Governance Policy truth、shared rules、state matrix 或 formal decision 作为配置项。
...
| feature flags 的最终默认值 | 影响 trace available / derived view / external export | Step 7 配置项清单定义 |

...
| 哪些 Command / Query / Event / Job 必须做协议和流程测试? | 23 个 Command、14 个 Query、9 个 Inbound Consumer、12 个 Outbound Event、7 个 Operations Job 全部进入 P0 最小协议和流程测试切口。后续 Step 6 再按切口展开具体用例。 |
...
| 12 个 Outbound Event 是否有 payload snapshot / no body / publish failure 入口 | 通过 | 已通过 outbound schema cuts 承接 |

...
| 输出文件 | `projects/L1-governance/design-calibration/05_test_plan_step_04_strategy_layers.md` |
...
- 哪些问题必须在 unit 层发现。
- 哪些问题必须在 service 层验证编排。
- 哪些问题必须依赖 repository / adapter / worker / job integration。
- 哪些问题需要 API / contract test。
...
| 输入 | 状态 | 本 Step 用途 |
|---|---|---|
| `05_test_plan_step_03_test_objects_cuts.md` | 已完成 | 提供 P0 测试对象、切口、停审和来源审计 |
| `03-详细设计.md` §5 | 正式输入 | 固定七个模块职责和入口模块边界 |
| `03-详细设计.md` §8 | 正式输入 | 固定 Command / Query / Consumer / Job 的函数级编排 |
| `03-详细设计.md` §9~§12 | 正式输入 | 固定状态、事务、错误、幂等和恢复测试发现层级 |
| `03-详细设计.md` §13~§15 | 正式输入 | 固定配置绑定、observability、redaction 测试发现层级 |
| `04-配置设计.md` §12 | 正式输入 | 固定配置测试主题、配置验收门禁和 evidence 方向 |

## 4. SOP 问题回答

| 问题 | 回答 |
|---|---|
| 哪些问题必须在 unit 层发现? | contracts DTO / metadata / digest schema、domain object factory、不变量、policy accept/reject、state enum 合法 / 非法转换、config parser / typed validator、redaction helper 和 low-cardinality label rule 必须在 unit / contract unit 层发现。它们不依赖 repository 或外部 adapter,越早失败越好。 |
| 哪些问题必须在 service 层验证编排? | Command accepted transaction 顺序、query no-write、consumer dedup / snapshot / stale / receipt、duplicate replay、stored result / receipt / report、commit unknown、error mapping、source unavailable mapping 和 UoW rollback 必须在 application service 层验证。它们的风险来自编排顺序,不是单个 domain 对象。 |
| 哪些问题必须依赖 DB / adapter / worker 集成测试? | repository version/page/unique/rollback、projection dependency index、reference scope index、outbox stored payload snapshot、publisher single-winner、runtime builder assembly、fake / controlled adapter failure injection、worker unsupported / delayed / dead-letter 和 job runner partial report 必须用 repository fake、adapter fake、worker / job integration 验证。P0 不要求真实 DB / bus 产品。 |
| 哪些问题需要 API / contract test? | public protocol envelope、metadata 必填、Command / Query / Event / Job DTO roundtrip、handler required-field validation、protocol error mapping、Query response missing / not visible / degraded surface 和 outbound event payload schema 需要 API / contract test。 |
...
| 位置 | 当前问题 | 本 Step 处理 |
|---|---|---|
| 旧 `05-测试方案.md` §2 / §8 | 旧分层把单元、集成、事件链、恢复、E2E 并列,但没有明确每类风险应该在哪一层失败 | 本 Step 建立切口到发现层级映射 |
...
| Service 层 | 未突出编排顺序 | 明确 UoW、outbox、stored result、duplicate replay、query no-write | 这些是 P0 主要可落码风险 |
| Integration 层 | 易被理解为真实外部产品 | 明确 P0 使用 repository / adapter fake 和 controlled seam | 产品未锁定不阻塞 P0 |
| Release gate | 容易承担过多语义 | 限定为跨入口 smoke、report、redaction、evidence 汇总和 veto scan | 防止用总门禁替代具体测试 |
...
| integration 是否要求真实产品 | A. P0 真实 DB / bus / external GRC;B. P0 fake / controlled adapter | 采用 B。真实产品属于 P1/P2 |
| query no-write 放在哪层 | A. E2E;B. service + repository write audit | 采用 B。必须能精确断言无写副作用 |
| dependency boundary 放在哪层 | A. 手工审查;B. architecture check + release gate 汇总 | 采用 B。VF-GOV-010 需要可留证 |
| redaction 放在哪层 | A. 只扫最终 report;B. helper unit + integration artifact scan | 采用 B。既防规则错误,也防实际输出泄露 |

## 8. 结构化中间产物

### 8.1 测试分层图: L1-governance 测试金字塔
...
- P0 integration 使用 fake / controlled adapters,不要求真实 DB、bus 或 external GRC 产品。
- Release gate 只汇总证据和跨入口 smoke,不能替代底层断言。
- Query no-write 和 job no truth repair 需要 repository write audit 或等价测试工具支持。
...
| Service 是否覆盖 UoW、duplicate、query no-write、consumer receipt | 通过 | 见 §8.3 / §8.4 |
...
- 状态、不变量、DTO、digest、config parser 和 redaction helper 必须在 unit / contract 层发现。
- UoW、outbox、stored result、duplicate replay、query no-write、consumer receipt 和 job no truth repair 必须在 service / runner 层发现。
- repository version、rollback、projection / reference index、publisher / handoff failure 和 runtime builder 必须在 fake / controlled integration 层发现。
...
| dependency boundary scan 采用脚本还是人工 gate | 影响 VF-GOV-010 evidence | Step 9 / Step 13 再定义 |

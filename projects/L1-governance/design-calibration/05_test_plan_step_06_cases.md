...
| TC-GOV-CMD-010 | ActivatePolicyEffectiveFact accepted | P0 | method policy snapshot resolved;`policy_snapshot.scope_ref` matches request scope;scope valid | call `ActivatePolicyEffectiveFactFlow` | policy fact `Active` or conflict marker created by policy;scope mismatch rejected before save | method body not stored;shared rules/priority guard applied;`matches_scope(...)` only compares stable scope identity | 是 | EV-CAND-GOV-CMD-010 |

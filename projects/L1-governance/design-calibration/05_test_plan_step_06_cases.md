...
| TC-GOV-CMD-010 | ActivatePolicyEffectiveFact accepted | P0 | method policy snapshot resolved;request `subject_ref` and `scope_ref` present;`policy_snapshot.scope_ref` matches request scope;scope valid for subject | call `ActivatePolicyEffectiveFactFlow` | policy fact `Active` or conflict marker created by policy;subject/scope mismatch or snapshot scope mismatch rejected before save | method body not stored;shared rules/priority guard applied;`matches_scope(...)` only compares stable scope identity;`PolicyScopePolicy` uses request subject/scope,not scope reverse lookup | 是 | EV-CAND-GOV-CMD-010 |
...
| TC-GOV-CMD-012 | UpdateSharedRuleSet accepted | P0 | rule refs valid;request `subject_ref` and `scope_ref` present;scope authorized for subject | call `UpdateSharedRuleSetFlow` draft/activate/add/deprecate/retire | shared rule set state/rule refs updated;subject/scope mismatch rejected before save | lower-scope override not allowed silently;conflict detection marker if needed;`PolicyScopePolicy` uses request subject/scope,not scope reverse lookup | 是 | EV-CAND-GOV-CMD-012 |

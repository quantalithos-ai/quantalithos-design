...
| TC-GOV-CMD-002 | SubmitGovernanceInput accepted / pending evidence | P0 | context `Ready`;input source ref valid | call `SubmitGovernanceInputFlow` | input `Accepted` or `PendingEvidence` by evidence state | no external body saved;input result stored;affected views stale | 是 | EV-CAND-GOV-CMD-002 |
| TC-GOV-CMD-003 | UpdateGovernanceInputState transition | P0 | input `Received` / `PendingEvidence`;expected version loaded | call `UpdateGovernanceInputStateFlow` to accept/reject/supersede | state changes to requested formal variant or rejects invalid transition | reason/ref required;history/trace/outbox/result saved only on accepted transition | 是 | EV-CAND-GOV-CMD-003 |
| TC-GOV-CMD-004 | OpenGovernanceGate accepted | P0 | context `Ready`;optional approver requirement fixture | call `OpenGovernanceGateFlow` | gate `Open` or `PendingDecision`;responsibility/chain optionally saved | gate, responsibility, trace/outbox/stale/result in one UoW | 是 | EV-CAND-GOV-CMD-004 |
| TC-GOV-CMD-005 | RecordGovernanceDecision finalizes gate | P0 | gate `PendingDecision`;chain satisfied or policy permits | call `RecordGovernanceDecisionFlow` approve/reject/waive | decision finalized;gate `Decided`;stored result returned | decision basis/reason present;no process/work state writes Decision truth | 是 | EV-CAND-GOV-CMD-005 |
...
| TC-GOV-CMD-010 | ActivatePolicyEffectiveFact accepted | P0 | method policy snapshot resolved;scope valid | call `ActivatePolicyEffectiveFactFlow` | policy fact `Active` or conflict marker created by policy | method body not stored;shared rules/priority guard applied | 是 | EV-CAND-GOV-CMD-010 |
...
| TC-GOV-CMD-012 | UpdateSharedRuleSet accepted | P0 | rule refs valid;scope authorized | call `UpdateSharedRuleSetFlow` draft/activate/add/deprecate/retire | shared rule set state/rule refs updated | lower-scope override not allowed silently;conflict detection marker if needed | 是 | EV-CAND-GOV-CMD-012 |
...
| TC-GOV-QUERY-006 | GetPolicyEffectiveView hit/degraded | P0 | policy view exists;method snapshot optional unavailable fixture | call `GetPolicyEffectiveViewFlow` | returns fresh or degraded policy view | degraded marker when method snapshot unavailable;query does not refresh | 是 | EV-CAND-GOV-QUERY-006 |
...
| TC-GOV-QUERY-008 | GetControlCoverage hit | P0 | control coverage projection exists | call `GetControlCoverageFlow` | returns coverage complete/gap/pending view | no nonconformity auto-created for gap | 是 | EV-CAND-GOV-QUERY-008 |
...
| TC-GOV-QUERY-010 | GetNonconformityStatus hit | P0 | nonconformity projection exists | call `GetNonconformityStatusFlow` | returns NC status view | action/verification state surface follows Step 10;no auto-close | 是 | EV-CAND-GOV-QUERY-010 |
...
| TC-GOV-QUERY-013 | GetGovernanceDashboard stale/degraded | P0 | dashboard projection stale or source degraded | call `GetGovernanceDashboardFlow` | returns stale/degraded marker | query does not rebuild projection or refresh reference | 是 | EV-CAND-GOV-QUERY-013 |
...
| TC-GOV-IDEMP-007 | projection dependency index only source | P0 | affected view repository returns known refs | run command/consumer/job stale marking | stale markers only for returned formal refs | no ad hoc `DerivedGovernanceViewRef` construction | 是 | EV-CAND-GOV-IDEMP-007 |

...
| TC-GOV-CMD-004 | OpenGovernanceGate accepted | P0 | context `Ready`;optional approver requirement fixture | call `OpenGovernanceGateFlow` | no requirement => gate `Open` and no responsibility ref;with requirement => responsibility/chain saved and gate `PendingDecision` | final gate state, responsibility, trace/outbox/stale/result in one UoW;requirement branch writes `required_responsibility_ref` from saved responsibility | 是 | EV-CAND-GOV-CMD-004 |

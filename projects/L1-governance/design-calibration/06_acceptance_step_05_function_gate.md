...
| AC-GOV-014 | 治理事实消费与追溯能力 | P0 | FR-GOV-009 的 14 个 Query、9 个 Consumer、13 个 Outbound Event、trace/audit refs-only 和 visibility/degraded surface 成立 | query 写入 truth/projection/reference;consumer 保存外部正文;outbound payload 非 stored snapshot | `TC-GOV-QUERY-001~016`;`TC-GOV-CONSUMER-001~012`;`TC-GOV-OUTBOX-001~016`;`EV-GOV-QUERY-001`;`EV-GOV-CONSUMER-001`;`EV-GOV-OUTBOX-001` |

...
| 输出文件 | `projects/L1-governance/06-验收标准.md`;`projects/L1-governance/design-calibration/06_acceptance_step_15_formal_document_assembly.md` |
| 停审方式 | 本轮按用户要求不停审连续推进;完成后统一报告 |
...
- 每章加入具体 `> 校准来源:`。
- 删除旧 GovernanceRequest / Gate / Decision / RiskAcceptance 主线。
- 不填写真实 `run_id`、implementation commit、缺陷状态、测试结果或最终 pass/fail。
- 将细节门禁、矩阵和停审记录保留在中间产物中,正式文档写成可裁决标准。
...
| 一票否决项是否真实生效? | 是。VETO-GOV-001~013 任一命中时不得通过或有条件通过。 |
| 每条 P0 门禁是否能回指设计契约、测试用例和 evidence ID? | 是。正式文档引用 EV/report,详细 TC/AC 映射在中间产物。 |
| 状态、字段、接口、事件名是否与详细设计和测试方案一致? | 是。使用 23 Command、14 Query、9 Consumer、12 Outbound Event、7 Job、正式状态族和 `EV-GOV-*`。 |
...
| 旧文档问题 | 处理 |
|---|---|
| 旧 `06` 围绕旧 GovernanceRequest / Gate / Decision / RiskAcceptance 主线 | 已删除并按新版 C-GOV / AC-GOV / VF-GOV / TC-GOV / EV-GOV 重建 |
| 旧文档使用 API / DB / audit entry 泛证据 | 已改为 `EV-GOV-*`、`reports/runs/<run_id>`、`artifacts/test/<run_id>` |
| 旧文档缺 VETO、evidence integrity、redaction、dependency boundary | 已补 VETO-GOV-001~013 和证据门禁 |
| 旧文档填写待评审结论占位但无三值规则 | 已改为三值结论和签署矩阵,不填写真实结论 |
...
| 主线 | 旧 request/decision | Governance truth center + protocol + evidence gates | 对齐新版 `00`~`05` |
| 证据 | 泛化证据 | run-scoped EV/report/artifact | 可复验 |
...
| 旧主线残留 | 通过 | 正文不再以旧 GovernanceRequest 主线组织 |
| P0 门禁闭环 | 通过 | 功能、红线、接口、状态、非功能、证据均有门禁 |
| VETO 覆盖 | 通过 | VETO-GOV-001~013 |
...
正式 `06-验收标准.md` 已按本 Step 装配。

## 10. 待确认事项

| 待确认事项 | 影响 | 当前处理 |
|---|---|---|
| 真实 source refs、implementation commit、core-contracts commit | 影响正式验收进入 | 正文保留 `<...>` 占位 |

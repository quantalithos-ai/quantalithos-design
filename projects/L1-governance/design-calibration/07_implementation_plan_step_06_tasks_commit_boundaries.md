...
| BATCH-04-05a | PH-04 prerequisite surface repair | `03` Step 6/7/8/13 | `IdGeneratorPort` PH-04 ids、`GovernanceTruthChangeSubjectMapper` PH-04 subject helpers including `control_review_subjects(review_ref)`、`ExternalGovernanceSourceResolverPort.resolve_scope_subject_relation`、PH-04 repository traits、history record types、outbox payload DTOs including `ControlReviewChangedPayload`、stored command result variants | 300~500 行;只补已在 `03` 闭合的 surface,不写 query/event/job | compile/contract smoke;`git diff --check` | commit-04-d |
...
| commit-04-d | PH-04 前置 surface、application services、handlers、fake repos 和 redaction targeted tests 通过后 | 先补 `03` 已闭合的 PH-04 prerequisite surface: `IdGeneratorPort` PH-04 ids、`GovernanceTruthChangeSubjectMapper` PH-04 subject helpers including `control_review_subjects(review_ref)`、`ExternalGovernanceSourceResolverPort.resolve_scope_subject_relation`、`PolicyEffectiveFactRepository` / `SharedRuleSetRepository` / `PolicyConflictRepository` / `ControlApplicabilityRepository` / `ControlReviewRepository` / `ComplianceConclusionRepository` / `NonconformityRepository` / `CorrectiveActionRepository`、`PolicyChangeRecord` / `ControlChangeRecord` / `ComplianceConclusionRecord` / `NonconformityChangeRecord`、PH-04 outbox payload DTO including `ControlReviewChangedPayload`、PH-04 stored result variants;再实现 policy/control/compliance/NC services、repos、handlers、stored result、trace/outbox、redaction tests | query views、event consumers、jobs;不得新增未在 `03` 闭合的 schema/port/status/phase | prerequisite surface compile;service-flow-fast PH-04 slice;redaction-boundary targeted;`git diff --check` |
...
| prerequisite surface 边界闭环 | commit-04-d 需要的 id generator、subject mapper、scope relation resolver、PH-04 repo traits、history records、outbox payload DTO、stored result variants 已在同 boundary 首批落码 | 通过:已纳入 BATCH-04-05a 和 commit-04-d 包含内容 | 若实现仓缺这些前置面,先写 BATCH-04-05a,不得绕过或新增未闭合 schema |
...
| BATCH-05-04 | query ports and decisions | `03` Step 7/9/11 | read repositories、projection index lookup reads、`ReferenceSnapshotRepository.get_actor_capability_snapshot(...)` typed read、visibility decision result、freshness/degraded surface | 300~500 行 | application compile | commit-05-b |
...
| commit-05-b | query ports/services、visibility/freshness/degraded/no-write tests 通过后 | query repositories, projection store, projection-backed query lookup reads, read visibility resolver port, resolver-first `GovernanceReadVisibilityResolution` helpers and loaded-truth consistency checks, `ReferenceSnapshotRepository.get_actor_capability_snapshot(...)` typed read, read visibility decision, domain truth 到 public view fields 的 query assembler, 14 query services | API handlers、event consumers、jobs | query no-write;projection/visibility tests;`cargo check`;`git diff --check` |
...
| commit-05-b | query service/read ports/visibility | Query response闭环;Ref-scope解析闭环;Sidecar truth读取面;Projection stale/read-model lookup闭环;validation truth | command idempotency/job 不适用 | `03` Step 7/9/11 | 开工前复核 | blocker 回写设计 | 设计者完成;实现者二次校验 |
...
#### PH-05 commit-05-a / commit-05-b crate 依赖闭口
...
| resolver-first visibility helper | commit-05-b 必须把 `GovernanceReadVisibilityResolution` helper 收敛为 `from_resolver(...)` / explicit resolved fields + `assert_matches_loaded_context(...)` 一致性校验;`GetGovernanceContext` 等 truth query 先走 `resolve_*_read(...)`,再加载 truth 组装 view | 实现者不得保留 `for_loaded_context(context, source_ref)` 这类会诱导从 loaded context 或 context id 生成 scope/read subject 的 helper |
...
| projection-backed query lookup read | commit-05-b 必须在 `GovernanceProjectionRepository` 落 `find_dashboard_view_ref_by_scope`、`find_policy_effective_view_ref_by_scope`、`find_decision_summary_view_ref_by_decision`、`find_decision_summary_view_ref_by_gate`、`find_control_coverage_view_ref_by_context`、`find_nonconformity_status_view_ref_by_nonconformity` 及 fake/durable 等价语义 | 实现者不得在 query service 中拼 `DerivedGovernanceViewRef` / `DecisionSummaryViewRef`、不得使用 `decision-summary:<decision_id>` 这类临时 id、不得扫描 projection store 或把 missing lookup 当成 create |
...
| IMPL-06-04 | 4 | 编写 13 个 outbound event DTO、payload snapshot、topic map、outbox record | `03` outbound event contracts | outbound events、outbox record/payload | contract-domain-fast event slice |

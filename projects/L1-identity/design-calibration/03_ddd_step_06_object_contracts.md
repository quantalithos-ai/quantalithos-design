...
| query material degraded marker 必须有正式 summary | loaded view missing、projection/reference/report/outbox/handoff integrity violation、member summary stale/degraded but missing freshness marker、career/memory list item missing/member mismatch、partial trace/audit/operations item missing、forbidden read material 等 query 内部检测分支必须先由 Step 7 `IdentityQueryMaterialDegradationMapper` 生成 `IdentityQueryMaterialDegradationSummary`;query service 只能复制 `degraded_marker_ref` / `degraded_kind`,不得从 view ref、record ref、memory reference ref、trace ref、report ref、outbox ref、handoff ref、scope、error string 或 fake 私有规则合成 |

`IdentityQueryMaterialDegradationSummary` 只用于 query 已经取得合法 `IdentityVisibilityAccessSummary` 后,在加载 projection / reference / report / outbox / handoff / career / memory / trace / audit material 时发现 material missing、owner/scope/subject/member/selector mismatch、unsafe/forbidden material、loaded member summary stale/degraded but missing `projection_freshness_ref` 或 partial item missing 的分支。它不替代 visibility resolver:resolver dependency unavailable / policy degraded 仍必须通过 `IdentityVisibilityAccessSummary { access_state: Degraded | Unavailable, degraded_marker_ref, degraded_kind }` 表达。它也不保存 raw projection body、reference body、report body、outbox payload、handoff payload、career body、memory body、trace body、audit entry body、adapter error、stack trace、credential 或 secret。
...
- `MemberSummaryProjectionRebuildViewInput`、`MemberSummaryProjectionRebuildPlan`、`IdentityMaintenanceLoadedTarget` 和 `IdentityMaintenanceInspectionContext` 这类 Step 7 port 输出 helper shell,仅用于 maintenance job prepared context,不新增 truth。
...
```rust
/// Complete body-free input for one MemberSummaryView rebuild.
pub struct MemberSummaryProjectionRebuildViewInput {
    /// Stable summary view ref selected by the formal projection builder/catalog.
    pub view_ref: MemberSummaryViewRef,
    /// Member represented by this summary.
    pub member_ref: GlobalMemberRef,
    ...
    /// Memory reference safe summary slices.
    pub memory_slice_refs: Vec<MemberSummarySliceRef>,
    /// Visibility result prepared for this view surface.
    pub visibility_result_ref: VisibilityResultRef,
    /// Public read surface category for the rebuilt view.
    pub read_surface_kind: IdentityReadSurfaceKind,
...
|---|---|---|---|
| `MemberSummaryProjectionRebuildViewInput` | Step 7 projection builder/catalog method for one member summary view rebuild | all `MemberSummaryView::from_projection(...)` fields are already prepared;`member_ref` must equal plan member;scope,visibility,read material and slice refs must be body-free typed refs | 不从 projection ref、visibility scope、existing view、config string、error text 或 fake map 推 view 字段 |
| `MemberSummaryProjectionRebuildPlan` | Step 7 projection repository/catalog method for member-summary projection rebuild | `projection_ref` must equal requested target;`view_inputs` must be non-empty when the member summary writer is selected;each input `member_ref` must equal plan member;missing/empty plan is a maintenance issue,not a default view or default scope | 不从 projection ref、member ref、view ref、config string 或 fake map 推 scope / slice / visibility / freshness / material marker |
...
| maintenance prepared helper shell | 并入 6.4 helper shell | rebuild view input / plan / inspection context 是 Step 7 port 输出,不是新 aggregate | Step 7 projection / maintenance repository;Step 9 maintenance job |
...
| member summary rebuild inputs | `get_member_summary_rebuild_plan(projection_ref)` 返回 non-empty `MemberSummaryProjectionRebuildViewInput[]`,每项携带保存 `MemberSummaryView` 所需的 body-free typed refs / markers | rebuild 从 projection ref、visibility scope、existing view、config string、error text 或 fake map 推 scope / slice / visibility / freshness / material marker |
...
| DDD-S6-OPEN-026 | `IdentityProjectionRef` / `ProjectionStateRef` 的正式 builder / lookup / catalog 来源 | projection rebuild、projection state read、projection fake | commit-07-b member summary rebuild 已闭合:Step 7 `get_member_summary_rebuild_plan(projection_ref)` 是 `MemberSummaryView::from_projection(...)` 所需 view ref、member ref、visibility scope、safe slices、visibility result、read surface、source cursor、freshness marker 和 material marker 的唯一来源;summary query stale marker 已由 `MemberSummaryView.projection_freshness_ref` 承载,缺失时通过 Step 7 `member_summary_view_missing_freshness(...)` 返回 degraded;future non-member projection writer/catalog 仍需单独闭口 |

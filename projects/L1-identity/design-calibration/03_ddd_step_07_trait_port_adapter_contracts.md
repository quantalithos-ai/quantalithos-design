...
所有 `resolve_*_read(...)` 的 `Some(IdentityVisibilityAccessSummary)` 必须包含 `read_subject_ref`、`scope_ref`、`visibility_result_ref` 和 `access_state`。`read_subject_ref` 是 query service 构造 `IdentityVisibilityDecision.read_subject_ref` 的唯一正式来源。repository / adapter 内部可调用 formal read subject mapper,或使用 request-scoped typed ref、loaded view/report typed ref 生成 canonical read subject;application service、handler 和 fake 不得从 route、member id、view id、report id、topic key、scope 或 result ref 反推。
...
|---|---|---|---|
| `resolve_member_summary_read` | member summary query precheck | returns access summary containing read subject/scope/result refs;`read_subject_ref` is copied into `IdentityVisibilityDecision` | 不从 route/member string 拼 subject 或 scope |
...
- Resolver output is only `IdentityVisibilityAccessSummary` with `read_subject_ref`;it must not return projection body,reference safe summary body,outbox payload,topic binding secret,handoff receipt body,target path or archive package.
...
| read subject/scope 是否闭合 | 通过 | visibility repository 返回带 `read_subject_ref` / `scope_ref` / `visibility_result_ref` 的 prepared access summary,service 不从 route/string 推断 |

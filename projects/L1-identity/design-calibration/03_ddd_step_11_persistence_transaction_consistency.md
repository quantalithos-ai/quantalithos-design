...
| `member_summary_views` | body-free member summary read model | PK `member_summary_view_ref`;unique current `(member_ref, visibility_scope_ref)` | `member_ref`,`visibility_scope_ref`,`read_surface_kind`,`visibility_result_ref`,`source_cursor`,`projection_freshness_ref` | replaced under projection state version;query copies loaded `projection_freshness_ref`,does not read projection state |
...
| `IdentityProjectionRepository.get_member_summary_rebuild_plan` | member summary projection builder/catalog index | read-only | returns non-empty `MemberSummaryProjectionRebuildViewInput[]` carrying all body-free `MemberSummaryView` fields;不从 projection/scope/view/config string、existing view 或 fake map 推 slice/visibility/freshness/material fields |

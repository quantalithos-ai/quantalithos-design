...
| `IdentityMaintenanceRepository` target expansion | rebuild / refresh / reconciliation job | formal maintenance scope expansion;returns projection/reference/report targets only | Step 7 `expand_maintenance_targets/list_projection_targets_for_rebuild/list_reference_targets_for_refresh/list_report_targets`;Step 11 maintenance target catalog | target expansion cannot return core truth write targets;job does not repair member/lifecycle/role/career/memory truth |

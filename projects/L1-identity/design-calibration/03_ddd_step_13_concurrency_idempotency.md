...
| `external_reference_owner_index` | refresh/reconciliation target selection | owner/kind/stale index owned by reference state save | Step 7 `list_reference_states_by_owner/list_reference_states_by_kind/list_stale_reference_states`;Step 11 owner index | maintenance must not parse external ref strings to infer owner/kind |
| `IdentityMaintenanceRepository` target expansion / inspection | rebuild / refresh / reconciliation job | formal maintenance scope expansion plus typed target inspection context;returns projection/reference/report targets only | Step 7 `expand_maintenance_targets/list_projection_targets_for_rebuild/list_reference_targets_for_refresh/list_report_targets/load_maintenance_target_inspection_context`;Step 11 maintenance target catalog | target expansion cannot return core truth write targets;job does not repair member/lifecycle/role/career/memory truth;service/fake cannot decode opaque target marker |

...
|---|---|---|---|
| `contracts_protocol_surface_roundtrip` | Step 5 `identity-contracts`;Step 8 shared protocol / Command DTO | typed refs、Command request/result、`IdentityCommandResponse<T>.effect`、protocol rejection、public error、view/event/job shell 可序列化、反序列化并保持 required fields;command-specific typed result DTO 不重复携带 effect | contract unit |
...
| `MaintainRoleCapabilitySummary_command_duplicate` | Step 13 replay | duplicate replays stored role typed result plus response effect,does not rerun source/evidence resolver or create second snapshot/outbox | application service |
...
| `AppendCareerRecord_command_duplicate` | Step 13 replay | duplicate command replays stored career typed result plus response effect;does not append another record or re-evaluate duplicate source marker | application service |
...
| `MaintainMemoryReference_command_duplicate` | Step 13 replay | duplicate replays stored memory relation typed result plus response effect;does not call resolver/callback mapper or create second relation/outbox | application service |
...
| `PrepareTraceHandoff_command_duplicate` | Step 13 replay | duplicate replays stored pending handoff typed result plus response effect;does not create a new handoff intent,does not append delivery receipt marker,does not call delivery adapter | application service |
...
| `ReadMemberSummary_query` | Step 8 `ReadMemberSummary`;Step 9 flow | stable `MemberSummaryViewRef` lookup via persisted `(member_ref, visibility_scope_ref)`;fresh/stale/degraded/missing summary;loaded view scope mismatch degrades;not visible before leaking slices;no view creation or rebuild;no scope inference from `visibility_result_ref` | query handler |
...
| `ListPendingIdentityOutbox_query` | Step 8 `ListPendingIdentityOutbox`;Step 9 flow | pending/retryable/by subject/by member/by trace selectors;ByTrace empty page copies `resolve_outbox_trace_page_read(...)`;visibility-filtered page;payload marker body-free;no publish/retry/topic raw string | query handler |

...
| 输出文件 | `projects/L1-identity/design-calibration/05_test_plan_step_14_regression_risks.md` |
| 正式文档状态 | 本 Step 不修改正式 `05-测试方案.md` |
| 停审方式 | 完成本 Step 后暂停,由用户审查后再进入 Step 15 |

## 2. 本步目标

定义 L1-identity 在需求、设计、实现、配置、测试和证据发生变化时应触发哪些最小回归、何时必须全量回归,以及哪些暂不覆盖的 P1/P2 / future / candidate 风险必须进入残余风险记录。
...
| `05_test_plan_step_06_cases.md` | 已审核通过 | 提供 `TC-ID-*` 用例族和自动化候选 |
| `05_test_plan_step_09_automation_gates.md` | 已审核通过 | 提供 PR / main / nightly / release suite 和 blocking 级别 |
| `05_test_plan_step_10_nonfunctional.md` | 已审核通过 | 提供 redaction、dependency、performance sample、P1/P2 边界 |
| `05_test_plan_step_11_defects_retest.md` | 已审核通过 | 提供缺陷复验矩阵和防回归新增规则 |
| `05_test_plan_step_12_entry_exit.md` | 已审核通过 | 提供进入 / 退出阻断准则和 residual 处理 |
| `05_test_plan_step_13_evidence.md` | 已审核通过 | 提供 `EV-ID-*` 证据族、artifact/report 归档和 review 报告 |
...
| 问题 | 回答 |
|---|---|
| 哪些变更触发最小回归? | contract/domain/application/query/consumer/outbox/job/config/redaction/dependency/report 任一局部变更,至少触发对应 suite、同 family 用例、相邻 suite 和相关 check。见 §8.1。 |
| 哪些变更触发全量回归? | 修改 truth object、public protocol、state matrix、UoW/idempotency/outbox/job/report 语义、redaction/dependency check、P0 profile、release evidence 结构或任一 S 级缺陷修复时,必须触发全量 P0 regression。 |
| 哪些风险暂不覆盖? | P1 real-like selected-run、真实 DB / bus / archive / metric / secret provider 产品行为、完整跨仓端到端、external HR / IdP 深度集成、production-like capacity、硬性能 P95/SLO、高级员工主页、复杂组织结构、full event-sourcing-first 和 UI / dashboard / report 体验暂不作为 P0 阻断。 |
| 谁接受残余风险? | P0 不可接受风险必须修复;P1/P2 residual 由验收负责人、架构负责人、产品负责人、合规负责人或测试负责人在新版 `06` / `reports/acceptance/risk-acceptance.md` 中确认。当前若未定具体人,列为待确认项。 |
| 哪些风险必须转入验收标准? | P1 selected-run 是否强制、性能阈值是否硬化、VETO 正式裁决矩阵、证据保留期、真实 adapter 验收边界、production-like/capacity 验收条件必须转入新版 `06` 或后续基线。 |
...
| P1 unavailable 是否算 P0 失败 | A. 算 P0 failed;B. 记录 residual | 采用 B。P1 不阻断 P0 |
...
|---|---|---|---|
| requirements / AC / VETO 变更 | 受影响 TC family + `release-main-smoke` + traceability review | 任一 C-ID / FR-ID / BR-ID / AC-ID / VETO-ID 语义变更 | 设计负责人 + 测试负责人 |
...
| 真实 DB / bus / archive / metric / secret provider 产品行为未覆盖 | 产品选型未锁定 | 不能证明具体产品性能和故障模式 | P0 证明 repository/port 语义;产品绑定后补 P1/P2 | 架构负责人待确认 |
| 完整跨仓端到端业务流程未覆盖 | 相邻仓真实状态机和权限语义不在 identity P0 | 不能证明真实全链路业务消费体验 | P0 证明 refs/events/adapters seam;后续 cross-repo integration 补测 | 验收负责人待确认 |
| external HR / IdP 深度集成未覆盖 | 外部身份系统非 P0 依赖 | 不能证明 vendor identity semantics | P0 禁止把外部身份当 identity truth;真实集成走 P2 | 产品负责人待确认 |
...
| 高级员工主页 / dashboard / analytics 未覆盖 | 当前 P0 只证明基础 query/read model | 不能证明复杂消费体验 | 基础 query/projection P0;高级产品体验 P2 | 产品负责人待确认 |
| 复杂组织结构、自动能力评级、绩效评分未覆盖 | 当前 P0 禁止外围增强反向定义 identity truth | 不能证明复杂人力模型 | 作为 future capability;不得阻断核心 command/query/job | 产品负责人待确认 |
| full event-sourcing-first 未覆盖 | P0 采用当前 detailed design 的 truth + trace + replay 口径 | 不能证明全量事件溯源架构 | 记录为架构演进风险;不影响当前 truth center | 架构负责人待确认 |
...
| 项 | 处理 |
|---|---|
| `VETO-ID-001~006` 命中 | 必须修复并全量 P0 regression |
...
| query/job 反写真相 | 必须修复;复跑 affected suite + `operations-replay-core` |
| accepted truth 缺 trace/audit/outbox/result | 必须修复;复跑 `service-flow-fast` and `release-main-smoke` |
...
| duplicate replay 重跑 mutation 或从 current truth 重算 result/report | 必须修复;复跑 idempotency family and affected suite |

### 8.6 必须转入新版 `06-验收标准.md` 的事项

| 事项 | 转入原因 | 建议 `06` 收口 |
|---|---|---|
| VETO 正式通过 / 不通过裁决矩阵 | `05` 只定义测试证据 | 建立 `EV-ID-*` / `AC-ID-*` / `VETO-ID-*` 裁决矩阵 |
...
| 是否有 residual 无接受人或待确认项 | 通过 | 当前均列角色待确认 |
...
| P0 redline 不可风险接受 | 否 | 承接 `00` VETO 和 Step 11 | 可回填 `05` |
| P1/P2 residual 不阻断 P0 | 否 | 范围边界 | Step 2 / 10 已一致 |
| 验收角色和 VETO 裁决矩阵未固定 | 是 | 验收标准闭口 | 新版 `06` 必须收口 |
...
- `VETO-ID-*`、redaction、dependency、query/job no truth repair、accepted trace/audit/outbox/result、evidence integrity、P0 profile unavailable 等 P0 红线不得风险接受。
- P1 real-like selected-run、真实产品 adapter、production-like capacity、旧 P95/SLA、高级员工主页 / dashboard、复杂组织结构、full event-sourcing-first 和长期归档策略属于 residual,不阻断 P0,但必须在新版 `06` 或 `reports/acceptance/risk-acceptance.md` 中明确接受人和触发条件。
...
| 新版 `06` 的 VETO 裁决矩阵 | 影响验收引用 | Step 14 只列转入事项 |
...
| 可进入 Step 15 | 待用户确认 | 用户审核通过后进入 Step 15: 整理正式测试方案文档 |

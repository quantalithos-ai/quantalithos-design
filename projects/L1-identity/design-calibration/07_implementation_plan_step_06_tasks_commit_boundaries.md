...
| commit-07-b | projection rebuild view input/source cursor、reference refresh selection key、reference bundle version、report-only reconciliation、job no-repair | Step 6/7/9/10/11/12 | 通过 |
...
| commit-07-b | projection/reference/report maintenance | projection rebuild;projection rebuild view body-field input;Reference refresh selection key;Reference typed sidecar version;maintenance job typed plan/outcome/inspection;job policy executable summary;job no-repair;machine artifact 不适用 | publish/deliver 后续 | Step 6/7/9/10/11/12;`MEM-ID-013`;`MEM-ID-014`;`MEM-ID-015` | 通过 | 允许开工 | 设计者完成;实现者二次校验 |

...
| forbidden_rule | Do not reverse lookup `ExternalReferenceRef` from `ReferenceResolutionStateRef`, state row strings, sibling stores, private fake maps, resolver output, business source refs or source versions; reference refresh selection ports must hand service the formal bundle key directly. | active |
...
| BLK-ID-07B-MEMBER-SUMMARY-VIEW-INPUT-001 | design_gate | resolved | RebuildIdentityProjectionFlow could select member and visibility scopes but still lacked formal source for `MemberSummaryView` body-free fields: stable view ref, anchor/lifecycle/role/career/memory slices, visibility result, read surface, source cursor, freshness marker and read material marker. Implementation could only infer from projection ref, scope, existing view or fake map. | Step 6/7/9/11/12/13 now define `MemberSummaryProjectionRebuildViewInput` and require `get_member_summary_rebuild_plan(...)` to return non-empty complete view inputs; `RebuildIdentityProjectionFlow` constructs `MemberSummaryView` only from these inputs and treats missing/empty input as safe issue. | read_docs |
| BLK-ID-07B-REFERENCE-REFRESH-SELECTION-KEY-001 | design_gate | resolved | RefreshExternalReferenceStateFlow ByOwner / ByKind / Stale target selection returned or implied `ReferenceResolutionStateRef` pages while the flow later needed `ExternalReferenceRef` to load, resolve and save the reference bundle. Implementation could only parse state refs, scan sibling stores or keep fake-only reverse maps. | Step 7 reference selection ports now return `ExternalReferenceRef` bundle keys;Step 8/9/11/12/13 require owner/kind/stale/scope selection to hand the bundle key directly and forbid state-ref reverse lookup. | read_docs |

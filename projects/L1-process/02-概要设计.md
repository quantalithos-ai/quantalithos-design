...
| `ProcessCheckpoint` | domain truth object | `checkpoint_id: ProcessCheckpointId`、`process_instance_id: ProcessInstanceId`、`activity_ref: Option<ActivityRef>`、`checkpoint_state: CheckpointState`、`evidence_ref: CheckpointEvidenceRef` | `mark_superseded(ProcessCheckpointRef next_ref)`、`invalidate(CheckpointInvalidReason reason)`、`expire(CheckpointExpireReason reason)`、`can_resume(ProcessInstance instance)` |
| `RecoveryAttempt` | domain truth object | `recovery_attempt_id: RecoveryAttemptId`、`process_instance_id: ProcessInstanceId`、`checkpoint_ref: ProcessCheckpointRef`、`recovery_state: RecoveryAttemptState`、`failure_reason: Option<RecoveryFailureReason>`、`abandon_reason: Option<RecoveryAbandonReason>` | `mark_applied(RecoveryHistoryId history_id, ActorRef actor)`、`mark_failed(RecoveryHistoryId history_id, RecoveryFailureReason reason)`、`abandon(RecoveryHistoryId history_id, RecoveryAbandonReason reason, ActorRef actor)` |

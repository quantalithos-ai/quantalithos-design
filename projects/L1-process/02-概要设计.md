...
| `WaitingGate` | domain truth object | `waiting_gate_id: WaitingGateId`、`process_instance_id: ProcessInstanceId`、`activity_ref: ActivityRef`、`gate_state: WaitingGateState`、`pause_context_ref: PauseContextRef` | `bind_decision(GovernanceDecisionRef decision_ref)`、`resume(ActorRef actor)`、`cancel(WaitingCancelReason reason, ActorRef actor)`、`expire(WaitingExpireReason reason)` |
| `PauseContext` | value object / domain state | `pause_context_id: PauseContextId`、`process_instance_id: ProcessInstanceId`、`activity_ref: ActivityRef`、`pause_reason: PauseReason`、`resume_requirement_ref: ResumeRequirementRef` | `is_resumable_with(GovernanceDecisionRef decision_ref)`、`from_activity(Activity activity, PauseReason pause_reason, ResumeRequirementRef requirement_ref)` |
...
| `RecoveryAttempt` | domain truth object | `recovery_attempt_id: RecoveryAttemptId`、`process_instance_id: ProcessInstanceId`、`checkpoint_ref: ProcessCheckpointRef`、`recovery_state: RecoveryAttemptState` | `mark_started(ActorRef actor)`、`mark_succeeded(RecoveryResultRef result_ref)`、`mark_failed(RecoveryFailureReason reason)` |
...
| `AdvanceProcessActivity` | process instance ref + activity ref + progression input | `ActivityProgressionService` | `ActivityFeedbackPolicy`、`GatewayRoutingPolicy`、`Activity::apply_progression(...)`、`Token::move_to(...)` | save activity + token/gateway + progression record + trace + outbox | 不创建 WorkItem、runtime step 或 tool call |

...
详细设计影响判定: 本章明确 P0 加载链为 defaults -> JSON -> env -> validate -> builder,P0 不支持 core hot reload,configured adapter 不得 fallback fake success,无需回写 `03-详细设计.md`。
...
详细设计影响判定: 本章定义 invalid startup config 不进入 runtime builder,configured adapter runtime failure 不回退 fake success,publish / handoff / resolver failure 不回滚 committed truth,无需回写 `03-详细设计.md`。

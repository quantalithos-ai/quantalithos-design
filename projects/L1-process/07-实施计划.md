...
经验沉淀示例: 若多次因 command duplicate replay、UnitOfWork 起点、shared DTO 二级类型、optimistic version 读取面、history / progression record id 来源、domain method 返回面、command intent membership 校验所需 body-free summary / port 读取面等同类闭环缺口导致暂停,属于可复用经验,应更新对应标准、SOP 或项目永久记忆种子并给出具体示例;若只是单个 Process flow 的字段名或事务行文修正且不能泛化,则说明本次无新增可复用经验。

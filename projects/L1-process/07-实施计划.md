...
| runtime entry args | `04-配置设计.md` §5 defines `--config`、`--profile`、`--run-id`、`--job-input`、`--dry-run`、`--diagnostics` and `PROCESS_*` env keys | CLI / env key or job binary argument surface missing / invented outside design 则暂停 |
...
PH-02~PH-05 中的 `outbox intent` 只表示 PH-08 前用于 command side-effect 验证的过渡保存面。进入 commit-08-b 后,所有旧 outbox intent port、fake store 和 accepted-flow call site 必须迁移为正式 `ProcessOutboxRecord` 保存面;旧 `event_kind + subject_ref + trace_ref` 形态不得继续作为并行 outbox path 或 publisher 输入。由于 waiting gate、checkpoint、recovery 和 rhythm accepted write path 在 PH-08 前已经存在,commit-08-b 同时承接这些 path 所需的 payload builder / mapping closure,不得把 call site 迁移和对应 payload schema 分拆到不同提交。
...
| commit-09-f | PH-09 | recovery maintenance and full job suite 通过后 | recovery maintenance policy summary store/port、checkpoint expiry fields、attempt failure counter/time fields、maintenance runner、pending expiry abandon、failed retry-exhausted abandon、rerun behavior、full job evidence report | final release reports;new recovery attempt creation;new process instance creation | `TC-PROC-JOB-001~007`;`EV-JOB-001`;no truth repair |

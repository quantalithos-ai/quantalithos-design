...
把 Step 1~Step 16 已确认并可回填的全部结论,按正式需求文档结构整理成 `projects/L1-process/00-需求文档.md`。本步只做重组、摘录、统一术语和交叉引用,不新增未经讨论的新需求。
...
| 旧 `projects/L1-process/00-需求文档.md` | 已删除 | 不做局部修补,只作为历史输入已在 Step 1~16 被裁剪 |
...
| 移除旧 13 节结构、旧技术栈和旧硬指标口径 | 已完成;旧技术栈和旧指标仅作为“不继承 / 候选”说明出现 |
...
| 是否仍使用旧 `US-001` / `F-001` / `BR-001` 编号 | 未使用为正式编号 |
| 是否把旧 P95 / 30s / PostgreSQL 写成需求硬基线 | 否 |
| 是否写入 API 路径、DTO schema、handler / service / repository、事务或表结构 | 否 |
| 是否保留 process / method-library / work / governance / artifact / runtime / identity / conversation / workspace 边界 | 通过 |

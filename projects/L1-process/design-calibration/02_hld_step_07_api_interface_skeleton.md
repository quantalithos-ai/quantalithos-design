...
| `AdvanceProcessActivity` | process instance ref + activity ref + progression input + expected position + context | `ActivityProgressionCommandResult` | `Activity`、`Token`、`Gateway`、`ActivityProgressionRecord`、`ProcessTraceRecord`、`ProcessOutboxRecord` | Process execution management | 只推进 Process 节点位置,不创建 WorkItem 或 runtime step |

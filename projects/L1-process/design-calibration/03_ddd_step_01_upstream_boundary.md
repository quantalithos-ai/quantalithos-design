...
- 上游文档:
  ...
  - 旧版 `projects/L1-process/03-详细设计.md` 的 git 历史版本,仅作为问题诊断样本
- 已确认结论:
  - `L1-process` 是过程运行、过程形态采用、等待门控、checkpoint / recovery、过程派生视图和过程追溯交接的 L1 仓。
  - Process 不拥有 method-library definition body、Work truth、Governance decision truth、Runtime execution truth、Conversation body、Archive package 或 Observability ledger。
  - 编译期依赖只允许 `L0-core`;其他 sibling 仓通过 ref、snapshot、event、consumer、port、handoff 或下游消费协作。
...
2. 概要设计中的代码主体框架是否已经足够稳定?
   ...
   回答:足够稳定。概要设计已经把 Process 主体分为 Inbound / Operations、Application Services、Domain Model、Domain Policy、Ports、Persistence、Projection、Outbox、Handoff 等实现分层,并明确 `Process truth core`、`Runtime shape management`、`Profile adoption management`、`Process execution management`、`Gate coordination`、`Checkpoint and recovery`、`Process timing and rhythm`、`Process consumption and traceability`、`Derived maintenance and reconciliation`、`External context mirror support` 这 10 个组成部分。详细设计可以继续将这些主体落到 crate、module、file、trait、service 和 adapter。

3. 概要设计中的关键对象、接口骨架、处理流和状态机是否足够继续展开?

   回答:足够进入详细设计。概要设计已经完成以下闭环:
   - 关键对象反查到 Step 6,包括 truth / state、policy、projection / read model、reference / snapshot、audit / history / outbox。
   - Command / Query / Consumer / Event / Job 已有正式名称、输入摘要、输出摘要和读写边界。
   - SyncRuntimeProcessShape、AdoptProcessProfile、StartProcessInstance、AdvanceProcessActivity、RecordActivityFeedback、ResumeWaitingGate、CreateProcessCheckpoint、RecoverProcessInstance 等主流程已经收敛。
   - 状态集合已经固定,包括 runtime shape、profile、instance、activity、token、gateway、waiting gate、checkpoint、recovery、derived view、reference resolution、trace handoff 等状态族。
...
4. 哪些内容仍停留在概要设计轮廓,进入详细设计前必须补清?
   ...
   - projection rebuild truth source、reference snapshot refresh、outbox publication、trace / archive handoff 细节。
   - 正式状态转换矩阵和测试切口。

5. 哪些需求或架构结论会影响详细设计,但不能在详细设计中重新定义?

   回答:以下结论只能承接,不得在详细设计中改写:
   - Process 只拥有 runtime process shape / profile / instance / activity / token / gateway / waiting gate / checkpoint / recovery / derived process view / process trace 等过程语境。
   - method-library definition body、Work truth、Governance decision truth、Runtime execution truth、Conversation body、Archive package 和 Observability ledger 不得进入 Process truth。
   - `L0-core` 是唯一编译期 sibling 依赖。
   - Query、projection、reconciliation、report 和 handoff 不反写真相。
   - checkpoint / recovery 不创建第二份 process truth,只能沿同一 ProcessInstance 连续恢复。
   - 配置不得重定义状态机红线、truth 边界或正文排除。

### 4. 旧版 03 问题诊断

| 位置 | 当前问题 | 影响 |
|---|---|---|
| 旧版 `03-详细设计.md` 文档元信息 | 关联旧 `02-概要设计.md v0.3.0`,状态为 Draft | 与新版 `02-概要设计.md` 不一致,不能作为当前详细设计基线 |
| 旧版 `03-详细设计.md` §3 | 保留 `[]` 占位表和目录树占位 | 不满足详细设计可落码性要求 |
| 旧版 `03-详细设计.md` §5 / §6 | 保留 `ProcessTemplate`、`Template`、`WaitingGateState`、`Checkpoint`、`TimelineProjection`、`ProgressSummaryDTO` 等旧对象 | 与新版 `RuntimeProcessShape`、`WaitingGate`、`ProcessCheckpoint`、`ProcessReadModel` 等对象口径冲突 |
| 旧版 `03-详细设计.md` §7 | 使用旧 RPC / command 签名和 `process.template.synced` 等旧事件 | 与新版 Command / Consumer / Outbound Event / Job 骨架冲突 |
| 旧版 `03-详细设计.md` §9 | 直接给出 `process_templates`、`process_instances` 等旧表结构草案 | 持久化契约需要从新版对象、状态、事务和 repository contract 推导,不能复用旧表结构 |
| 旧版 `03-详细设计.md` 全文结构 | 采用旧 15 章结构 | 不符合 `详细设计书写规范.md` 的 18 章结构和校准来源要求 |
...
- `L1-process` 是否拥有 method-library definition body、Work truth、Governance decision truth、Runtime execution truth、Conversation body、Archive package 或 Observability ledger。
- `RuntimeProcessShape` 是否可以保存 method definition 正文。
- `ProcessInstance` / `Activity` / `Token` / `Gateway` 是否可以成为 Runtime truth 或 Work truth。
- `WaitingGate` 是否可以成为 governance decision truth。
- checkpoint / recovery 是否可以创建第二份 Process truth。
- Query、projection、reconciliation、report 和 handoff 是否可以反写真相。
- 是否允许非 `L0-core` 仓进入编译期依赖。
- 状态机红线、truth 边界、正文排除和 recovery 不分叉是否允许被配置覆盖。
...
| 风险 | 是否阻塞 Step 2 | 处理口径 |
|---|---|---|
| 旧 `03` 与新版 `02` 大范围冲突 | 不阻塞 | 旧 `03` 已删除,只作为诊断输入,不作为真相源 |
| `04-配置设计.md` 当前未完成新版重建 | 不阻塞 Step 2,影响 Step 14 | Step 14 只定义详细设计需要读取的配置引用和绑定点;正式配置手册后续单独写 |
...
本文不再回答 Process truth 归属、相邻仓 truth 边界、runtime shape 与 method definition body 边界、waiting gate 与 governance decision 边界、checkpoint / recovery 不分叉、query / projection / reconciliation / handoff 不反写真相、编译期依赖裁剪等已经由需求、架构和概要设计收稳的问题。
...
- 无阻塞 Step 2 的待确认事项。
- 后续 Step 14 需要处理 `04-配置设计.md` 当前未完成新版重建的问题。

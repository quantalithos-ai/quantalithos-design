...
##### 7.2.5 PH-05 rhythm / timing shared schema

PH-05 rhythm / timing 的 public protocol、domain object 签名、trace / outbox payload 和 repository key 共同使用以下 shared types。它们全部归 `contracts::refs`;domain 只能复用这些类型,不得在 `domain::rhythm` 中再定义同名别名或靠裸字符串分派。

```rust
/// Stable identifier for a process stage state.
pub struct ProcessStageId {
    /// Process-local stage id value generated by L1-process.
    pub value: String,
}

/// Public reference to a process stage state.
pub struct ProcessStageRef {
    /// Stable reference value derived from ProcessStageId.
    pub value: String,
}

/// Body-free profile stage reference used to create ProcessStageState.
pub struct ProfileStageRef {
    /// Stable profile stage reference from the adopted profile summary.
    pub value: String,
    /// Stage kind declared by the adopted profile stage summary.
    pub stage_kind: ProcessStageKind,
}

/// Stage category visible to rhythm policy and public timing protocols.
pub enum ProcessStageKind {
    /// Intake or preparation stage before detailed planning.
    Intake,
    /// Planning stage for scope, sequencing, or commitment.
    Planning,
    /// Execution stage where process activity work is carried out.
    Execution,
    /// Review or approval stage.
    Review,
    /// Release or delivery stage.
    Release,
    /// Retrospective or learning stage.
    Retrospective,
    /// Custom profile stage category that is still body-free.
    Custom,
}

/// Subject whose rhythm or timebox state is changed by a timing command.
pub enum ProcessTimingSubjectRef {
    /// Timing state attached to one concrete process instance.
    ProcessInstance(ProcessInstanceRef),
    /// Timing state attached to a process profile template/default.
    ProcessProfile(ProcessProfileRef),
}

/// Reference to committed rhythm or timebox truth used by trace and outbox.
pub struct ProcessTimingRef {
    /// Subject affected by the timing change.
    pub process_subject_ref: ProcessTimingSubjectRef,
    /// Stage state changed by the operation when applicable.
    pub stage_ref: Option<ProcessStageRef>,
    /// Timebox binding changed by the operation when applicable.
    pub timebox_binding_ref: Option<ProcessTimeboxBindingRef>,
}

/// Stable identifier for a process timebox binding.
pub struct ProcessTimeboxBindingId {
    /// Process-local binding id value generated by L1-process.
    pub value: String,
}

/// Public reference to a process timebox binding.
pub struct ProcessTimeboxBindingRef {
    /// Stable reference value derived from ProcessTimeboxBindingId.
    pub value: String,
}

/// Process-owned timebox reference.
pub struct ProcessTimeboxRef {
    /// Stable process timebox reference value.
    pub value: String,
}

/// Kind of external timebox reference accepted by L1-process.
pub enum ExternalTimeboxKind {
    /// L1-work process timebox reference.
    WorkProcessTimebox,
    /// L1-work iteration reference used only as an external timing marker.
    WorkIteration,
}

/// External work timebox or iteration reference.
pub struct ExternalTimeboxRef {
    /// Stable reference value assigned by the owning boundary.
    pub value: String,
    /// Owning external timebox kind.
    pub external_kind: ExternalTimeboxKind,
}

/// General rhythm reason used by activation and binding policy.
pub struct RhythmReason {
    /// Non-empty reason value or reason reference.
    pub value: String,
}

/// Reason for pausing a process stage.
pub struct StagePauseReason {
    /// Non-empty reason value or reason reference.
    pub value: String,
}

/// Reason for completing a process stage.
pub struct StageCompletionReason {
    /// Non-empty reason value or reason reference.
    pub value: String,
}

/// Reason for skipping a process stage.
pub struct StageSkipReason {
    /// Non-empty reason value or reason reference.
    pub value: String,
}

/// Reason for releasing a process timebox binding.
pub struct TimeboxReleaseReason {
    /// Non-empty reason value or reason reference.
    pub value: String,
}

/// Reason for marking a process timebox binding invalid.
pub struct TimeboxInvalidReason {
    /// Non-empty reason value or reason reference.
    pub value: String,
}
...
/// Structured reason for a stage state update.
pub enum StageChangeReason {
    /// Reason for activating a stage.
    Activate(RhythmReason),
    /// Reason for pausing a stage.
    Pause(StagePauseReason),
    /// Reason for completing a stage.
    Complete(StageCompletionReason),
    /// Reason for skipping a stage.
    Skip(StageSkipReason),
}
```

| 类型 | 最小 schema / 变体 | validation / 映射 |
|---|---|---|
| `ProcessStageId` / `ProcessStageRef` | `value: String` | 非空;`ProcessStageRef.value` 由 `ProcessStageId.value` 派生;不得保存 profile body |
| `ProfileStageRef` | `value: String`;`stage_kind: ProcessStageKind` | `value` 非空;`stage_kind` 来自 adopted profile 的 body-free stage summary;这是 `ProcessStageState.stage_kind` 的唯一正式来源 |
| `ProcessStageKind` | `Intake` / `Planning` / `Execution` / `Review` / `Release` / `Retrospective` / `Custom` | public enum;不得从 stage ref 字符串猜 kind;`Custom` 仍不得携带会议正文或 profile body |
| `ProcessTimingSubjectRef` | `ProcessInstance(ProcessInstanceRef)` / `ProcessProfile(ProcessProfileRef)` | `ProcessInstance` 表示 concrete instance rhythm truth;`ProcessProfile` 表示 profile-level timing template/default;service 不得用裸字符串判断 subject kind |
| `ProcessTimingRef` | `process_subject_ref`;`stage_ref: Option<ProcessStageRef>`;`timebox_binding_ref: Option<ProcessTimeboxBindingRef>` | `stage_ref` 和 `timebox_binding_ref` 至少一个非空;所有 ref 必须属于同一 subject;用于 `ProcessTruthChange::TimingChanged`、trace 和 outbox |
| `ProcessTimeboxBindingId` / `ProcessTimeboxBindingRef` | `value: String` | 非空;`ProcessTimeboxBindingRef.value` 由 `ProcessTimeboxBindingId.value` 派生 |
| `ProcessTimeboxRef` | `value: String` | 非空;process-owned timing marker;不得等同或改写 Work Iteration truth |
| `ExternalTimeboxKind` / `ExternalTimeboxRef` | `external_kind`;`value: String` | `value` 非空;只保存外部稳定引用;`WorkIteration` 只表示外部 timing marker,不得让 Process 修改 Iteration truth |
| `RhythmReason` | `value: String` | 非空;用于 bind / activate 等通用 rhythm 解释;正文说明进入 trace / audit |
| `StagePauseReason` / `StageCompletionReason` / `StageSkipReason` | `value: String` | 非空;分别传入 `pause` / `complete` / `skip`;不得互相转换或复用裸字符串 |
| `StageChangeReason` | `Activate(RhythmReason)` / `Pause(StagePauseReason)` / `Complete(StageCompletionReason)` / `Skip(StageSkipReason)` | 必须与 `StageTarget` 同 variant;不匹配 -> `InvalidRequest` |
| `TimeboxReleaseReason` / `TimeboxInvalidReason` | `value: String` | 非空;分别传入 `release` / `mark_invalid` |

`ProcessStageState::from_profile_stage(stage_id, process_instance_id, stage_ref)` 必须直接复制 `stage_ref.stage_kind` 到 `ProcessStageState.stage_kind`。若 profile stage 不在 adopted profile 的 body-free stage summary 中,application 必须在调用 factory 前 reject;domain factory 不读取 profile repository、不解析 profile body,也不得根据 `ProfileStageRef.value` 推断 kind。

`UpdateProcessStageStateFlow` 为 stage 变化构造 `ProcessTimingRef` 时,`process_subject_ref` 必须来自 loaded `ProcessStageState.process_instance_id`,按同值构造成 `ProcessTimingSubjectRef::ProcessInstance(ProcessInstanceRef { value: stage.process_instance_id.value })`;不得从 request、stage ref 字符串或 repository 私有索引反推 subject。

`UpdateProcessStageStateRequest.stage_target` 和 `stage_change_reason` 的唯一映射如下:

| `stage_target` | 必须匹配的 `stage_change_reason` | Domain method |
|---|---|---|
| `Activate` | `StageChangeReason::Activate(reason)` | `ProcessStageState.activate(actor)`;`reason` 进入 policy / trace context |
...
pub struct ProcessTimeboxBinding {
    ...
    /// Subject owning this timing binding.
    pub process_subject_ref: ProcessTimingSubjectRef,
    /// Process-owned timebox reference.
...
| `process_subject_ref` | `ProcessTimingSubjectRef` | timing binding 所属 instance 或 profile | 必填;与 command subject 一致 |
...
| 函数签名 | 作用 | 参数说明 | 返回 | 使用场景 |
|---|---|---|---|---|
| `pub fn bind(binding_id: ProcessTimeboxBindingId, process_subject_ref: ProcessTimingSubjectRef, process_timebox_ref: ProcessTimeboxRef, external_timebox_ref: ExternalTimeboxRef, actor: ActorRef) -> Result<Self, DomainError>` | 建立绑定 | id、subject、process ref、external ref、actor | `Result<ProcessTimeboxBinding, DomainError>` | `BindProcessTimebox` |

...
| `trace_handoff_refs` | trace / archive handoff marker | `handoff_ref`;unique(`handoff_kind`,`trace_record_ref`,`target_ref`) while not terminal;unique(`handoff_kind`,`external_ref`) when external exists | `trace_record_ref`,`target_ref`,`handoff_state`,`handoff_kind`,`external_ref`,`receipt_ref`,`archive_package_ref`,`failure_ref`,`cancel_reason`,`delivered_at` | `storage_version`;archive handoff stores `ArchiveHandoffTargetRef.stored_target_ref` in `target_ref` |
...
| `TraceRepository::append_trace(...)` | 追加 trace record | 与 committed Process truth change 同事务;external snapshot / reference-only consumer 不调用 | `()` | `RepositoryError` |
...
| `ReferenceSnapshotRepository::list_reference_states_for_refresh(...)` | 按 `ExternalContextRefreshScope` 分页列出已注册 reference resolution state | read-only;refresh job only;filters map as: `context_kinds` -> external context kind index,empty means all kinds;`process_instance_ref` -> reference-state dependency index for that process instance;`reference_state` -> lifecycle state index;stable order `(ExternalContextKind, ExternalContextRef, ReferenceResolutionStateId)`;returned versions are used by refresh success / failure state saves | `Page<Versioned<ReferenceResolutionState>>` | `RepositoryError` |
...
| `ConsumeMethodDefinitionChangedFlow` | envelope valid 后 `UnitOfWork.begin()` | snapshot/reference/stale marker/receipt/idempotency saved 后 | invalid envelope before tx、dedup conflict、digest mismatch quarantine、repository failure | reserve event、upsert method snapshot/reference state、mark affected shape/profile/projection stale when allowed、save `ConsumerReceipt::Accepted` with `trace_record_ref = None` |

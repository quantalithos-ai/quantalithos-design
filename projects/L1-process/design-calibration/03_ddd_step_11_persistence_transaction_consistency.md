...
| `Gateway` | `domain/token_gateway.rs` | instance / activity command | activity command、reconciliation | route / join state 按 Step 10;不实现完整 BPMN 引擎 |
...
| `gateways` | gateway flow state | `gateway_id` | `shape_node_ref`,`gateway_state` | `storage_version` |
...
| `recovery_attempts` | recovery attempt truth | `recovery_attempt_id` | `process_instance_id`,`checkpoint_ref`,`recovery_state` | `storage_version` |
...
|---|---|---|---|---|
| `SyncRuntimeProcessShapeFlow` | resolver 返回 snapshot 后,service 调 `UnitOfWork.begin()` | shape、method snapshot、trace、outbox、operation result、idempotency complete 保存后 | resolver reject、shape policy reject、repository conflict、operation result save failure | reserve command、save / update shape、upsert method snapshot、append trace、append outbox `RuntimeShapeChanged`、save command operation result |
| `AdoptProcessProfileFlow` | request 校验和 dependency load 后 | profile、change record、audit / trace、outbox、operation result 保存后 | inactive shape、work context mismatch、version conflict、result save failure | reserve command、save profile、append change record、append audit / trace、append outbox `ProfileChanged`、save command operation result |
| `UpdateProcessProfileTailoringFlow` | load profile with version 后 | profile and change side effects 保存后 | high-risk evidence missing、shape invalid、stale version、result save failure | reserve command、save profile using loaded version、append change / audit / trace、append outbox、save command operation result |
| `StartProcessInstanceFlow` | active profile / work context loaded 后 | instance、initial activity/token/gateway、trace/outbox/result 保存后 | profile not active、shape invalid、missing start node、repository conflict | reserve command、save instance/activity/token/gateway、append progression record、append trace、append outbox `InstanceChanged`、save command operation result |
| `AdvanceProcessActivityFlow` | instance/activity/token/gateway loaded 后 | all changed flow objects and outbox saved 后 | expected position conflict、invalid transition、gateway route reject、repository conflict | reserve command、save activity/token/gateway/instance when changed、append progression、append trace、append outbox `ActivityProgressed`、save command operation result |
| `RecordActivityFeedbackFlow` | activity loaded and feedback resolved 后 | activity、progression、trace/result 保存后 | feedback mismatch、runtime body rejected、resolver unavailable when required、repository conflict | reserve command、save activity、append progression / trace、append outbox when publishable、save command operation result |
| `OpenWaitingGateFlow` | instance/activity loaded 后 | pause context、gate、instance、token、outbox/result saved 后 | invalid instance/activity/gate policy、token conflict、repository conflict | reserve command、save pause context、save gate、save instance/token、append waiting change、append trace/outbox、save command operation result |
| `ResumeWaitingGateFlow` | gate/context/instance/token loaded 后 | gate、instance、token、outbox/result saved 后 | decision mismatch、already terminal、token missing、repository conflict | reserve command、save gate、save instance/token、append waiting change、append trace/outbox、save command operation result |
| `CreateProcessCheckpointFlow` | instance / optional activity loaded 后 | checkpoint/history/trace/result saved 后 | instance missing、activity mismatch、evidence invalid、repository conflict | reserve command、save checkpoint、supersede previous when policy requires、append history / trace / audit、save command operation result |
| `StartRecoveryAttemptFlow` | checkpoint and instance loaded 后 | attempt、instance、history/outbox/result saved 后 | checkpoint invalid、fork violation、instance terminal、repository conflict | reserve command、save recovery attempt、save instance recovering、append history/trace、append outbox `RecoveryAttemptChanged`、save command operation result |
| `CompleteRecoveryAttemptFlow` | attempt and instance loaded 后 | attempt、instance when changed、history/outbox/result saved 后 | outcome mismatch、terminal attempt、missing failure reason、repository conflict | reserve command、save attempt、save instance for applied/failed outcome,append history/trace/outbox、save command operation result |
| `BindProcessTimeboxFlow` | work snapshot resolved 后 | binding、trace/outbox/result saved 后 | external timebox unavailable、invalid binding、repository conflict | reserve command、save binding、append trace、append outbox `TimingChanged`、save command operation result |
| `UpdateProcessStageStateFlow` | stage loaded 后 | stage、trace/outbox/result saved 后 | illegal stage target、version conflict、result save failure | reserve command、save stage、append trace、append outbox `TimingChanged`、save command operation result |
...
| `ConsumeRuntimeActivityFeedbackFlow` | envelope valid 后 | feedback marker/activity stale marker/receipt saved 后 | feedback mismatch quarantine、source unavailable delayed、repository failure | reserve event、upsert runtime feedback marker、mark activity pending/stale marker、save consumer receipt |

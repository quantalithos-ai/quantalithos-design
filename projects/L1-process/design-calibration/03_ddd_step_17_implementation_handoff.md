...
| `WaitingGate` / `PauseContext` / `WaitingGateChangeRecord` | `waiting_gate_id` / `pause_context_id` / `change_id` / pause / requirement / decision refs | generated ids + typed refs | `IdGeneratorPort::new_waiting_gate_id()` / `new_pause_context_id()` / `new_waiting_gate_change_id()` + open gate command + governance event marker | `PauseContext::from_activity`;`WaitingGate::open_for_activity`;`WaitingGateChangeRecord::from_gate_transition`;`attach_decision`;`resume` | gate command / governance event | generator failure / missing requirement / decision mismatch | waiting gate tests | 后续 `06` 同步 |

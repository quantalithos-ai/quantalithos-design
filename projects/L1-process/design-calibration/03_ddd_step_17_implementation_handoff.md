...
| `ProcessInstance` | `process_instance_id` / `profile_ref` / `project_ref` / `token_set_ref` | ids / refs | id generator + active profile lookup | `ProcessInstance::create`;`start` | `StartProcessInstanceRequest.*` | inactive profile / missing start node -> reject | `StartProcessInstance_contract` | 后续 `06` 同步 |
...
| `StartProcessInstanceRequest` | `ProcessInstance`;`Activity`;`Token`;`Gateway` | 是 | profile / runtime shape lookup、id generator | process instance != Project truth | reject | `StartProcessInstanceFlow` |
| `AdvanceProcessActivityRequest` / `RecordActivityFeedbackRequest` | `Activity`;`Token`;`Gateway`;progression record | 是 | repository current position、runtime feedback marker | feedback ref != feedback body | conflict / reject | activity flows |
...
| `ProcessInstanceState` | Step 6 / 10 | `create`;`start`;`pause_for_gate`;`resume_from_gate`;`mark_recovering` | instance / gate / recovery flows | terminal advance;recovery fork | `process_instance_state_transitions` | 后续 `06` 同步 |

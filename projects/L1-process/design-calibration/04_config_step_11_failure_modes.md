...
| controlled adapter runtime failure 不回退 fake success | 否 | integration-like 安全语义 | 无 | 无回写 |
| publish / handoff / resolver failure 不回滚 committed truth | 否 | 承接 03 一致性规则 | 无 | 无回写 |

## 6. 回填草稿

`04-配置设计.md` §11 应输出失效模式表,覆盖配置文件不可读、解析失败、重复 key、非法 env、missing ref、controlled ref 缺失、raw secret、unsupported source、retention 冲突、topic map 缺失、configured / controlled adapter 不可达、redaction check fail 等情况。高风险失败必须 fail-fast / fail-closed,不得 silent fallback。

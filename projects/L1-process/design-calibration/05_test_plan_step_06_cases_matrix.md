...
| TC-PROC-CMD-001~013 | 13 Command success / reject / duplicate / conflict | P0 | fake repositories、fake idempotency、fake resolvers、deterministic id generator;waiting gate fixture 可注入 missing pause context | 调用每个 command flow 的 success、domain reject、duplicate、same key different digest;`OpenWaitingGate` id generation;`ResumeWaitingGate` missing pause context | success 写 truth / trace / outbox / result;duplicate 返回 stored result;conflict 不进 domain;missing sidecar truth reject 且不写 success side effects | UoW 顺序、operation result save-before-complete、receipt.duplicate、waiting gate / pause context / gate change ids from `IdGeneratorPort`、sidecar truth 读取面 | 是 |
...
| TC-PROC-JOB-001~007 | Operations job completed / duplicate / invalid / partial | P0 | job fixture、fake store / adapter、recovery policy summary fixture | 执行 7 operations job,含 invalid input、duplicate key、per-item failure;`TC-PROC-JOB-007` 覆盖 recovery maintenance expired pending、fresh pending no-op、failed retry exhausted、failed below threshold no-op | JobRunReceipt 正确;partial failure 保留成功项;duplicate 不重跑 item loop;expired / exhausted attempt 被 abandoned;fresh / below-threshold attempt 不变 | JobDisposition、report refs、no auto repair truth、policy summary loader used、no history-count / adapter retry-counter inference | 是 |

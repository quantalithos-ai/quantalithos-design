...
| commit-08-b | PH-08 | formal outbox record 保存面迁移和 accepted write path payload builder tests 通过后 | 将旧 outbox intent port / fake / accepted-flow call sites 全部迁移为 `ProcessOutboxRepository::append(ProcessOutboxRecord, ...)`;保存 `event_kind`、`truth_ref`、`trace_context`、`visibility_marker`、`payload_snapshot`;补齐当前 accepted write path 需要的 `ProcessInstanceChangedEvent`、`ActivityProgressedEvent`、`WaitingGateChangedEvent`、`ProcessCheckpointCreatedEvent`、`RecoveryAttemptChangedEvent`、`ProcessTimingChangedEvent` payload builders 和 outbox mapping | trace/view payloads、publisher loop;旧 `event_kind + subject_ref + trace_ref` 旁路保存 outbox;未迁移的 PH-02~PH-05 accepted-flow call site | targeted payload mapping tests;outbox record persistence tests;old call site grep |
| commit-08-c | PH-08 | trace / derived view payload builder tests 通过后 | `ProcessTraceAvailableEvent`、`DerivedProcessViewChangedEvent` DTO 和 payload builders | publisher loop;retry / failed state | targeted payload mapping tests |
| commit-08-d | PH-08 | complete outbound mapping / redaction matrix tests 通过后 | complete 10-event `ProcessTruthChange -> ProcessOutboxEventKind -> ProcessOutboundEventPayload` mapping、payload redaction matrix、topic map preflight | publisher retry / failure loop | full outbound contract + redaction tests |
| commit-08-e | PH-08 | publisher retry/failure 和 publish job contract tests 通过后 | publisher port/fake、`contracts/src/jobs.rs` 最小 publish job shared schema (`JobMetadata`、`JobRunReceipt`、`JobDisposition`、`JobError`、`JobReportRef`、`ProcessOutboxScope`、`PublicationReceipt`、`PublishProcessOutboxJob`)、publish job idempotency / stored receipt replay、`ProcessOutboxService.publish_pending(...)`、worker/job loop、retry/failed state、topic map verification | non-publish operations jobs | `TC-PROC-PUB-001`;publish job contract tests;`EV-WORKER-002` |
| commit-09-a | PH-09 | non-publish shared job schema / result replay foundation tests 通过后 | remaining job scope / report refs、non-publish job idempotency result surface、runner foundation fixtures;不得重新定义或后置 `PublishProcessOutboxJob` | projection / refresh / reconciliation job DTO、job runners | non-publish job shared contract tests |
...
| PH-08 | IMPL-08-02 | 2 | 先迁移正式 `ProcessOutboxRecord` 保存面,再补齐 accepted write path 所需 instance / activity / waiting / checkpoint / recovery / timing payload builder | contract + repository tests | payload snapshot 随 accepted transaction 保存;truth change mapping stable;旧 outbox call site 清零 |
| PH-08 | IMPL-08-03 | 3 | 定义 trace / derived view payload builder | contract tests | payload snapshot 不重算 current truth |
| PH-08 | IMPL-08-04 | 4 | 定义完整 outbound 映射矩阵、payload redaction matrix 和 topic map preflight | contract tests | 10 event mapping / redaction complete |
| PH-08 | IMPL-08-05 | 5 | 实现 publisher fake、最小 publish job shared schema、publish job idempotency replay、`ProcessOutboxService.publish_pending(...)`、topic map、retry / failed | worker + publish job contract tests | publish / retry / failed;receipt / error stable |
| PH-09 | IMPL-09-01 | 1 | 定义 non-publish job shared schema 和 result replay foundation | job contract tests | remaining metadata / receipt / report refs stable;不重新定义 publish job |

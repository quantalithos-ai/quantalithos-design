...
| 对象 | 所属部分 | 主要责任 | 关键字段骨架 | 函数骨架 |
|---|---|---|---|---|
| `WorkTraceRecord` | `Work consumption / trace` | 记录工作事实变化、来源、消费和交接的追溯线索 | `trace_id: WorkTraceId`、`subject_ref: WorkTraceSubjectRef`、`trace_context: TraceContext` | `from_truth_change(WorkTruthChange change, TraceContext trace_context)`、`prepare_handoff(ArchiveHandoffRef archive_ref)` |

...
| `MemberWorkView` | `Derived consumption support` | 提供成员维度的正式工作只读视图 | `member_ref: ProjectMemberRef`、`assigned_work_refs: FormalWorkRefSet`、`view_state: DerivedWorkViewState` | `from_truth(ProjectMemberTruthSummary member, ProjectWorkTruthSnapshot snapshot)`、`contains(FormalWorkRef work_ref)` |
| `IterationSummaryView` | `Iteration commitment` | 提供 Iteration 承诺范围和完成摘要 | `iteration_id: IterationId`、`committed_refs: FormalWorkRefSet`、`summary_state: DerivedWorkViewState` | `from_truth(IterationTruthSummary iteration, ProjectWorkTruthSnapshot snapshot)`、`summarize_progress()` |

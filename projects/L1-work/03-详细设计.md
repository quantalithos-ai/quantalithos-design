...
`WorkItem` 与 `ChildWorkItem` 共用 `WorkItemState` 完整生命周期。二者进入 `Completed` 时都必须写入 `completion_ref: Option<ExternalEvidenceRef>` 的 verified evidence 引用;缺失或未验证 evidence 不得形成 truth / outbox。

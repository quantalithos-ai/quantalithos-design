...
> 对应文档: `projects/L1-work/02-概要设计.md`
> 对应 SOP: `standards/document/概要设计讨论流程_SOP.md`
> 创建日期: 2026-06-03
> 当前目标: 在已完成新版 `00-需求文档.md` 和 `01-架构设计.md` 的前提下,按最新概要设计 SOP 校准 `L1-work`。

---

## 1. 本轮校准原则

- 概要设计必须承接新版需求和新版架构,不能回到旧版“先用人话理解本仓 / 项目管理功能说明 / ImplementationPlan 教学叙事”作为主线。
- `L1-work` 的概要设计要下沉到代码主体骨架层,但不能提前写完整字段、函数实现、DDL、协议 schema、配置 JSON、测试用例或实施 commit boundary。
- 本轮概要设计必须围绕“项目工作事实真相仓”的可实现结构展开:代码主体框架、主要组成部分、关键对象、API / 接口骨架、关键处理流、状态机、配置影响和详细设计承接清单。
...
| `projects/L1-conversation/00~07` | 已完成深度校准 | 承接 conversation context、formalize / promote 来源、trace / handoff 和授权查询来源 |
| `projects/L3-method-library/00~07` | 已完成深度校准 | 承接 role、task、work product、process template、view profile 等定义来源 |
...
| 编号 | 问题 | 当前结论 |
|---|---|---|
| D-HLD-WORK-001 | 是否在旧 `02-概要设计.md` 上局部修补 | 否。旧文档作为历史输入,正式文档在 Step 14 删除旧文件后重建。 |
| D-HLD-WORK-002 | 概要设计是否继续以 Project / Backlog / WorkItem / Iteration 的新人解释作为第一层结构 | 否。它们是重要对象候选,但第一层结构必须从“项目工作事实真相仓”的代码主体骨架展开。 |
| D-HLD-WORK-003 | ImplementationPlan 是否作为 Work truth 主对象展开 | 否。Work 只承接 explicit formalize / promote 后的正式结果和来源引用,不拥有 ImplementationPlan 正文或执行推进。 |
| D-HLD-WORK-004 | 概要设计是否只写模块间结构 | 否。概要设计必须下沉到代码主体骨架层,足以支撑后续 `03-详细设计.md`。 |

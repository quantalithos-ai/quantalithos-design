...
- 上一步中间产物:
  - `projects/L1-work/design-calibration/03_ddd_step_01_upstream_boundary.md`
- 上游正式文档:
  - `projects/L1-work/00-需求文档.md`
  - `projects/L1-work/01-架构设计.md`
  - `projects/L1-work/02-概要设计.md`
- 概要设计承接清单:
  - `projects/L1-work/02-概要设计.md` §12
  - `projects/L1-work/design-calibration/02_hld_step_12_detailed_design_handoff.md`
- 概要设计风险:
  - `projects/L1-work/02-概要设计.md` §13
  - `projects/L1-work/design-calibration/02_hld_step_13_risks_open_questions.md`

### 3. SOP 问题回答

1. 本轮详细设计必须覆盖哪些模块?

   回答:本轮必须覆盖 L1-work 核心闭环所需的全部实现模块,包括 inbound / operations、application services、domain model and policies、ports and external seams、persistence / projection、outbox and handoff、config binding、observability / audit、test cuts。业务主轴必须覆盖概要设计固定的 10 个主要组成部分:`Work truth core`、`Project subject management`、`Project member responsibility`、`Formal work universe`、`Work decomposition / promote boundary`、`Dependency / blocker coordination`、`Iteration commitment`、`Work consumption / trace`、`Derived consumption support`、`Local reference / snapshot / projection support`。

2. 本轮必须定义哪些对象、接口、事件、job 和状态机?

   回答:本轮必须把概要设计第 6~9 章固定的主语全部展开为可落码契约:
   - 核心 truth / state 对象:`Project`、`ProjectMember`、`Backlog`、`WorkItem`、`ChildWorkItem`、`WorkDependency`、`WorkBlocker`、`Iteration`、`IterationCommitment`、`PromoteResult`、`DerivedWorkViewState`、`ReferenceResolutionState`。
   - policy / guard 对象:`WorkTruthPolicy`、`ProjectLifecyclePolicy`、`MemberResponsibilityPolicy`、`FormalWorkPolicy`、`BacklogAvailabilityPolicy`、`PromotePolicy`、`DependencyGraphPolicy`、`IterationCommitmentPolicy`、`CompletionEvidencePolicy`、`DerivedWorkViewPolicy`。
   - projection / read model 对象:`ProjectBoardView`、`MemberWorkView`、`IterationSummaryView`、`WorkSearchProjection`、`ReconciliationReport`。
   - reference / snapshot / audit / outbox 对象:`SourceWorkRef`、`ExternalEvidenceRef`、`MemberCapabilitySnapshot`、`MethodDefinitionSnapshot`、`WorkTraceRecord`、`WorkAuditTrail`、`WorkOutboxRecord`、`PromoteDecisionRecord`、`DependencyChangeRecord`、`IterationChangeRecord`。
   - Command / Query / Consumer / Outbound Event / Operations Job 入口必须保持概要设计第 7 章名称和读写性质。
   - 状态机必须覆盖 Project、ProjectMember、Backlog、Work lifecycle、Promote decision、Dependency、Blocker、Iteration、IterationCommitment、Derived freshness、Reference resolution、Outbox publication。

3. 哪些能力属于 P1 / 后续阶段,不应在本轮展开?

   回答:本轮详细设计只覆盖核心可落码闭环和边界接缝,不展开外围增强。以下内容只保留接口接缝、状态或风险,不写成当前核心实现契约:
   - advanced board、复杂容量规划、跨项目 dependency 的完整产品能力。
   - auto unblock、智能拆分、自动排期、自动优先级调整等自动化优化能力。
   - 完整 ES / CQRS / Graph-first 架构升级。
   - observability / archive 的完整长期存储协议、全局指标模型和归档包正文。
   - process planning、governance decision、artifact evidence、runtime progress、conversation suggestion 的正文模型。
...
| `04-配置设计.md` 缺失 | 本 Step 撰写时配置完整手册尚未存在;当前已生成正式 `04` | 历史风险已关闭;详细设计 Step 14 只定义代码需要读取的配置引用和绑定点 |
| `05-测试方案.md` / `06-验收标准.md` 旧口径 | 本 Step 撰写时下游测试验收仍需后续同步;当前已生成正式 `05/06` | 历史风险已关闭;详细设计 Step 16 只给新版测试切口,完整测试 / 验收以正式 `05/06` 为准 |
...
| 方案 | 优点 | 缺点 | 结论 |
|---|---|---|---|
| A. 本轮只覆盖核心 Command 写路径 | 文档量较小 | Query、consumer、projection、outbox、job 和 trace 缺设计,实现仓仍会阻塞 | 不采用 |
...
| 目标 | 说明 | 交付给实现者的结果 |
|---|---|---|
| 收稳仓内实现边界 | 将 Work truth、projection、snapshot、outbox、handoff 和 external seam 分层落到代码模块 | 实现者可以创建 workspace / crate / module / file layout |
...
| 函数级处理流 | Command、Query、Consumer、Job、outbox publish、projection rebuild、handoff 的调用链和副作用 | Step 9 |
| 状态矩阵 | Project、member、backlog、work、promote、dependency、blocker、iteration、derived、reference、outbox 状态 | Step 10 |
...
| Identity、Conversation、Method、Process、Governance、Artifact、Runtime、Workspace、Observability、Archive 正文模型 | 对应相邻仓设计文档 |
| 具体数据库、搜索、缓存、队列、调度产品最终选型 | 详细设计后续 Step 只约束代码接缝;最终落到配置 / 实施 |
| advanced board、容量规划、auto unblock、跨项目依赖完整产品能力 | 后续版本 / 产品增强 |

...
| `ProjectMemberRepository.list_by_member(member_ref, page)` | 按 identity member 枚举 Work-owned 项目承担 | read-only;page stable order;不解析 identity 正文 | `Page<ProjectMember>` | `RepositoryError` |
...
| `ProjectionRepository.list_views_affected_by_references(reference_refs, page)` | 按成功 refresh 的 typed external refs 枚举既有 affected public view refs | read-only;input refs 和 output views 均按 stable identity 去重并稳定排序后分页;只返回 Step 8 §9.2 public `DerivedWorkViewRef`;empty page 表示不标脏;不得从 external ref 临时拼接 view ref | `Page<DerivedWorkViewRef>` | `RepositoryError` |
...
| `ReferenceSnapshotRepository.get_reference_state_with_version(reference_ref)` | 读取 reference state 及 optimistic version | read-only;更新已有 state / failed marker 前必须调用 | `Option<(ReferenceResolutionState, Version)>` | `RepositoryError` |
| `ReferenceSnapshotRepository.get_member_snapshot_with_version(member_ref)` | 读取 member snapshot 及 optimistic version | read-only;更新已有 member snapshot 前必须调用 | `Option<(MemberCapabilitySnapshot, Version)>` | `RepositoryError` |
| `ReferenceSnapshotRepository.get_method_snapshot_with_version(definition_ref)` | 读取 method snapshot 及 optimistic version | read-only;更新已有 method snapshot 前必须调用 | `Option<(MethodDefinitionSnapshot, Version)>` | `RepositoryError` |
| `ReferenceSnapshotRepository.save_reference_state(state, expected_version, &uow)` | 保存 reference state | consumer / job UoW;已有 state 的 expected_version 来自 `get_reference_state_with_version`;missing create/upsert 才能为 `None` | `Version` | version conflict |
| `ReferenceSnapshotRepository.save_member_snapshot(snapshot, expected_version, &uow)` | 保存 member snapshot | same UoW as reference resolved state;已有 snapshot 的 expected_version 来自 `get_member_snapshot_with_version` | `Version` | version conflict |
| `ReferenceSnapshotRepository.save_method_snapshot(snapshot, expected_version, &uow)` | 保存 method snapshot | same UoW as reference resolved state;已有 snapshot 的 expected_version 来自 `get_method_snapshot_with_version` | `Version` | version conflict |
| `ReferenceSnapshotRepository.mark_reference_failed(reference_ref, reason, occurred_at, expected_version, &uow)` | 保存 failed marker | UoW;preserve last snapshot;已有 state 的 expected_version 来自 `get_reference_state_with_version`;missing failed marker create/upsert 才能为 `None` | `Version` | version conflict |
...
| projection view identity closure | `mark_stale` 的 affected views 必须来自 Step 8 §9.2 已定义的 public `DerivedWorkViewRef`;没有 query / projection identity 的 truth 或 marker 不得临时派生 view ref;reference refresh 只能通过 `ProjectionRepository.list_views_affected_by_references(...)` 枚举 existing public views | ProjectionRepository / function flows |
...
| Inbound event consumer | consumer service after dedup key build | reference / snapshot / pending intake + stale when public view exists + idempotency complete 后 | missing envelope -> no UoW accepted;unsupported version dead-letter;affected view enumeration repo failure rollback;repo failure rollback | idempotency reserve;reference / snapshot / intake writes;affected public views must come from formal repository read surface;empty affected view page means no stale marker;idempotency complete |

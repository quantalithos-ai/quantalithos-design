...
| Step 8 / 9 duplicate 口径 | 已说明 duplicate 返回 stored result,但未区分 in-flight / completed / conflict | 本 Step 增加重复处理矩阵 |
...
| duplicate 成功 | completed same digest 返回 stored `ApplicationResultRef` 对应 result surface,不重放 domain transition。 |
...
|---|---|---|---|
| 所有 Command: Project / Backlog / Member / Work / Promote / Dependency / Blocker / Iteration | `CommandMetadata.request.idempotency_key`,作用域为 operation | 至少覆盖客户端 retry 和服务端超时恢复窗口;具体保留期 Step 14 配置化 | completed same digest 返回既有 result;in-flight retry later;different digest `IdempotencyConflict` |
...
| `Completed` with `result_ref` | same digest | load duplicate result by `ApplicationResultRef` | 返回既有 result surface,receipt 标记 duplicate |
...
| 场景 | 重入来源 | 保护方式 | 恢复方式 |
|---|---|---|---|
| Command handler 超时后客户端重试 | HTTP/RPC timeout | same idempotency key + digest;completed 返回 stored result | 客户端使用同 key retry;不同输入必须换 key |
| Command UoW commit unknown 后重试 | adapter commit 返回 unknown | retry 前调用 `IdempotencyRepository.get(key, operation)` | completed 返回 stored result;reserved unknown 返回 `TemporarilyUnavailable` 并交 reconciliation |
...
3. if record.status == Completed and digest matches and result_ref exists:
     return duplicate stored result
...
|---|---|---|---|
| `TC-WORK-IDEM-001` | duplicate same digest | second request 返回同 result_ref,无新 truth / trace / outbox | application service |
...
| `03_ddd_step_11_persistence_transaction_consistency.md` | Repository 函数表补 `IdempotencyRepository.get`;更新 DDD11-OPEN-005 | 事务一致性与 Step 13 幂等审计对齐 |
...
`CommitStatusUnknown` 不允许盲重试。重试前必须调用 `IdempotencyRepository.get(key, operation)`。若记录已 completed 且 digest 匹配,返回既有 result;若仍是 reserved unknown,不得再次执行 domain 写入,返回 temporarily unavailable 并交 reconciliation / operator review。

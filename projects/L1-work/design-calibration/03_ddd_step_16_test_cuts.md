...
| `RefreshExternalReferenceSnapshots_contract` | `RefreshExternalReferenceSnapshotsFlow` | `None` / `StaleOnly` uses stale refs;`Project` uses `ReferenceSnapshotRepository.list_project_references(project_ref, page)` with typed-ref dedup / stable order / paging;`ExplicitRefs` dedups request refs;success refs drive `ProjectionRepository.list_views_affected_by_references(...)`;empty affected view page does not call `mark_stale`;resolver unavailable writes failed marker with `get_reference_state_with_version(...)` version;existing reference / member / method snapshot update uses versioned read;missing create/upsert uses `None`;version conflict preserves last good snapshot | job runner |

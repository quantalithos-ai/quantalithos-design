...
> 本文件是 `projects/L1-work/04-配置设计.md` 的 Step 2 中间产物。
...
- 回填章节: `projects/L1-work/04-配置设计.md` §2 本次配置设计目标与范围
...
| `00-需求文档.md` | Work truth 仓定位、核心能力闭环、数据归属、非目标、外部依赖和非功能红线 | 确认配置服务 Work truth runtime 和外部接缝,不扩大为 Workspace / Runtime / Artifact / Conversation 功能配置 |
| `01-架构设计.md` | 架构边界、依赖方向、配置不得绕过边界、运行期协作和演进取舍 | 确认配置不能改变系统边界、数据所有权、派生不反写、外部正文排除和唯一编译期依赖规则 |
| `02-概要设计.md` §11 | 配置影响轮廓、受配置影响的主要部分、禁止配置化边界 | 确定本轮配置设计覆盖 runtime builder、adapter config、store config、job config、projection config 和配置错误边界 |
| `03-详细设计.md` §13 / §16 / §17 | 配置引用表、外部依赖绑定、跨仓 Rust 依赖绑定、实施承接和风险收口 | 固定 P0 必须覆盖的配置入口、默认值口径、外部依赖绑定和下游回写门禁 |

已确认结论:

```text
本轮配置设计必须覆盖 L1-work P0 默认可验证路径的配置控制面:
runtime bootstrap -> local/in-memory store -> fake or configured resolver -> fake or configured outbox publisher -> fake or configured handoff -> jobs / projections / reports / redaction checks。

本轮配置设计必须为 store、boundary、idempotency、projection、jobs、external、outbox、handoff、features、clock / id generator 和 report output 提供配置说明。
...
P0 必须定义能让 Work truth center 默认可验证路径跑通的配置控制面。这里的“运行主链”不是生产部署全集,而是实现者和测试方案可以用同一套配置语义启动 api / worker / jobs,构造 repository、resolver、publisher、handoff 和 job runner,验证 truth 写入、query、projection、outbox、reference refresh、handoff 和 report 行为。

| P0 配置控制面 | 必须回答的问题 | 支撑的主链 |
|---|---|---|
| runtime bootstrap | 配置从哪里读取,以什么 profile 构造 Work runtime | api / worker / jobs 启动 |
| store | truth、projection、idempotency、outbox 使用什么 store profile | command、query、projection、outbox、idempotency |
| boundary | command body、page limit、query read timeout 等入口限制如何配置 | synchronous command / query |
| idempotency | command retention、event dedup retention、reserved record max age 如何配置 | command duplicate / conflict、consumer dedup |
| projection | projection store、stale threshold、replace scope、failed marker 如何配置 | authorized query、rebuild projection、derived views |
| jobs | batch、parallelism、retry、timeout、job run id 和 report output 如何配置 | projection rebuild、reference refresh、reconciliation、outbox publish、handoff |
| external resolver | identity、method-library、source work、evidence、process timebox 如何 fake / configured 解析 | member、formalize、promote、completion、iteration |
| outbox publisher | event collaboration 如何发布、重试、失败和 fake 标记 | outbound event / publish job |
| handoff | trace / archive handoff adapter 如何装配、重试和失败 | trace handoff / archive handoff |
| features | derived views、advanced search 等外围能力如何启停且不影响核心 truth | query / projection 外围能力 |
| clock / id generator | deterministic fake 与系统 clock / id generator 如何配置 | 测试可重复、truth / outbox / report id 生成 |
...
| real identity / conversation / method / process / governance / artifact / runtime adapter 参数全集 | P1 | P0 定义 resolver / event source 接缝;真实 adapter 字段后续接入 |
...
| production search backend / index tuning | P1 | P0 保留 advanced search feature 口径;真实搜索产品和索引参数后续补齐 |
...
| auto repair consistency job | P2 或重新设计 | 当前 reconciliation 只允许 diagnostic / report,不允许配置开启自动修复 truth |
...
| DB / MQ / object store / search 产品选型参数全集 | 定义 store / publisher / event source / report root / search backend 接缝 |
...
| real source adapter 参数全集 | 后续真实 identity / conversation / method / process / governance / artifact / runtime adapter 可能需要字段扩展 | P0 以 fake / fixture 和 unresolved-capable adapter 推进 |
...
| production search backend | 若 advanced search 进入生产主线,需要搜索产品、索引和容量参数 | P0 仅保留 feature / adapter 接缝,默认不要求真实搜索产品 |
...
| 位置 | 当前问题 | 影响 |
|---|---|---|
| `projects/L1-work/04-配置设计.md` | 本 Step 撰写时文件不存在;当前已生成正式范围声明 | 历史风险已关闭;实现时以正式 `04` 为准 |
...
| 当前 `05/06` | 已按新版配置控制面重校准 | 历史风险已关闭;测试验收以正式 `05/06` 为准 |

## 5. 改动前后对比

| 项 | 改动前 | 改动后 | 原因 |
|---|---|---|---|
| 配置设计目标 | 只有详细设计中的配置绑定点 | 明确 `04` 要形成 store、boundary、idempotency、projection、jobs、external、outbox、handoff、features 和 security redaction 的配置控制面 | 配置不是代码字段列表,而是跨下游文档的控制面 |
...
| 下游承接 | `05/06` 已按新版配置控制面重校准 | 正式 `05/06` 承接 `04` 的配置矩阵 | 防止旧对象口径回流 |
...
- `L1-work` 当前最需要的是可实现、可测试的 truth center default path,不是生产拓扑全集。
...
| 确认本步不新增 `WorkRuntimeConfig` 字段、不改变 adapter trait、不新增 ConfigError 枚举值 | 否 | 无代码契约变化 | 无 | 无回写 |
| 确认 P1/P2 只保留扩展接缝和风险,不写成当前必须实现项 | 否 | 与 `03` §13 / §17 的配置后续工作和风险口径一致 | 无 | 无回写 |
...
- P0-min 默认是否固定为 in-memory store、fake publisher、fake resolver、fake handoff、deterministic clock / id。
...
| 部署 / 测试 / 验收 / 实施非范围已有去向 | 通过 | §3.3 / §3.4 / §7.2 |
| 本步无 `03` 待回写项 | 通过 | §8 |

可以进入 Step 3 “建立配置控制面总览”。

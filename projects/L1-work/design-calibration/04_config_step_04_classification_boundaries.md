...
> 本文件是 `projects/L1-work/04-配置设计.md` 的 Step 4 中间产物。
> 本步定义 Work 配置类别、热更新 / 冷更新口径和禁止配置化边界。
...
- 回填章节: `projects/L1-work/04-配置设计.md` §4 配置分类与边界
...
| `02-概要设计.md` §11 | 禁止配置化边界 | 作为本步禁止配置化项主来源 |
| `03-详细设计.md` §13 / §14 / §17 | 配置引用表、外部依赖绑定、观测字段边界和风险收口 | 确认模块读取边界、secret / body / evidence 禁止项 |
...
```text
配置可以控制 runtime 装配、store / resolver / publisher / handoff adapter、entry profile、job policy、retention、projection feature、reports output、clock / id adapter 和 redaction profile。

配置不得控制或绕过 Work truth ownership、external body exclusion、formalize / promote boundary、ProjectMember / Actor visibility、domain state machine、idempotency semantics、audit / outbox chain、projection read-only boundary、outbox / handoff failure rule 和 reconciliation diagnostic-only rule。
```
...
|---|---|---|---|---|
| 启动配置 | 决定 Work runtime graph 如何装配 | `runtime.profile`、config path、entry enablement | 否 | 运行中替换会造成 repository、adapter 和 policy set 不一致 |
| 运行装配配置 | 绑定 store、resolver、publisher、handoff、clock / id 接缝 | store profile、resolver profile、publisher profile、handoff profile、clock profile | 否 | 外部接缝漂移会污染 truth、outbox 或 handoff evidence |
...
| 运行参数配置 | 控制 batch、parallelism、retry、timeout、run id 和 report 输出 | batch limit、retry policy、timeout、report root | 否;job run 开始读取 | 运行中改变会破坏幂等、partial failure 和 evidence |
| 策略配置 | 选择已定义安全和横切策略 profile | redaction policy、retention policy、projection feature、owner uniqueness policy | 否 | 策略被误用为绕过边界的开关 |
...
| 诊断 / 调试配置 | 支撑 local / CI 诊断,不改变 Work truth | verbose report、redaction check mode、safe diagnostic verbosity | local / test 有限允许 | 泄露 forbidden body、secret 或 private profile |
...
P0 默认不支持核心配置热更新。凡是会改变 runtime graph、store、resolver、publisher、handoff、entry、job、retention、projection、redaction、clock / id 或 report evidence 的配置,均采用启动读取、冷更新或 job run 开始读取。

当前只允许 local / test 诊断行为有限变化,且不得关闭 redaction、validator、forbidden body check、fake marker、idempotency、visibility guard、audit / outbox 或 evidence output。
...
|---|---|---|
| Work truth 归属 | 配置不能把 Project / WorkItem / Iteration truth 转给 process、workspace、runtime、artifact、conversation 或 governance | 回到需求、架构和概要设计重校准 |
| 外部正文排除 | 配置不能允许保存 conversation、artifact、ImplementationPlan、runtime progress、method definition body 等正文 | 回到数据归属和详细对象契约重校准 |
| formalize / promote 显式边界 | 配置不能让 event、query、projection rebuild、process timing 或 runtime signal 直接创建正式 Work truth | 回到概要接口 / 处理流和详细协议重校准 |
| ProjectMember / GlobalMember 分层 | 配置不能让 Work 接管 identity lifecycle 或把 GlobalMember 写成 Work truth | 回到架构依赖和数据归属重校准 |
| ProjectMember / Actor visibility | 授权消费是核心边界,不能通过 profile 或 feature flag 绕过 | 回到需求、架构、概要和授权消费设计重校准 |
| domain state machine 红线 | closed 重开、cancelled 复活、completed 伪造、iteration 承诺绕过等会破坏事实生命周期 | 回到详细设计状态矩阵重校准 |
| 幂等 duplicate / conflict 语义 | 冲突不能配置成成功,重复不能生成新事实 | 回到详细设计幂等和并发设计重校准 |
| audit / trace / outbox chain | accepted truth change 必须可追溯且可传播 | 回到追溯、审计、outbox 和验收门禁重校准 |
...
| reconciliation 自动修复 truth | 当前只允许 diagnostic / report,不允许自动覆盖 truth | 回到概要接口、详细处理流和风险项重校准 |
...
| 非 core sibling repo 变成 Cargo dependency | 破坏全局依赖裁剪和编译期边界 | 回到架构依赖裁剪和实施计划门禁重校准 |
...
| `02-概要设计.md` §11 | 已列禁止配置化边界,但不是正式配置分类章节 | 读者可能把禁止项当作普通配置开关 |
| `03-详细设计.md` §13 | 已列配置引用,但没有禁止项改变流程 | 实现或运维可能绕过设计链路添加开关 |
| `03-详细设计.md` §14 | 已列观测字段边界,但未进入配置分类 | 需要在敏感和诊断配置中承接 |
...
- L1-work P0 当前目标是让 truth center 默认可验证路径稳定成立,不是建设在线动态配置系统。
- 冷更新能避免同一个 Work truth、projection、outbox、handoff 或 report run 跨配置版本。
- 诊断配置可以存在,但必须限制在 local / test,且不能关闭 redaction、validator、fake marker、visibility guard、audit / outbox 或 evidence output。
...
```text
本步没有新增 `WorkRuntimeConfig` 字段、`ConfigError` 枚举值、adapter constructor 参数或 trait。

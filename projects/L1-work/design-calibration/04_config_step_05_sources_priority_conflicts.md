...
> 本文件是 `projects/L1-work/04-配置设计.md` 的 Step 5 中间产物。
...
- 回填章节: `projects/L1-work/04-配置设计.md` §5 配置来源、优先级与冲突处理
...
| `03-详细设计.md` §14 / §17 | 已确认日志、指标、审计、diagnostic、trace 和风险边界 | 确认 raw secret、forbidden body、private profile 不得经配置进入证据 |
...
|---|---|---|---|---|
| code defaults | 1,最低 | local / CI default path、in-memory store、fake publisher、fake resolver、fake handoff、deterministic clock / id、safe retry、strict redaction | 被更高普通来源覆盖 | 默认值必须可构造;不可构造是实现错误 |
| JSON config file | 2 | 启动配置、运行装配、入口、job、idempotency、projection、reports、ref-only sensitive config | 覆盖 defaults;同文件重复 key 或等价别名 fail-fast | 未指定文件可用 defaults;指定文件不可读 / 解析失败 fail-fast |
...
| `WorkRuntimeConfig` 必需结构缺失且无默认值 | 阻断启动 |
...
- provider 不可用: adapter 启动 fail-closed,或在明确支持 degraded / unresolved 的 resolver 场景写 unresolved marker。
- 不允许回退到 raw secret。
...
| environment variables | 只能提供 ref 或 profile | 不能提供 raw secret;不能关闭 redaction、visibility guard 或 forbidden body check |
...
```text
本步没有新增 `WorkRuntimeConfig` 字段、`ConfigError` 枚举值或 adapter constructor 参数。

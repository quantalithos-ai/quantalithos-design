...
| `TC-WORK-OPS-003` | RefreshExternalReferenceSnapshots | P0 | stale refs、Project reference scope 或 explicit reference scope 存在;Project scope fixture 含 project source、work source/method、member / evidence / timebox typed refs 且有重复样本;resolver 有 success / failure | 运行 `RefreshExternalReferenceSnapshots` | `None` / `StaleOnly` 读取 stale refs;`Project` 调用 `list_project_references(project_ref, page)` 并按 typed ref 去重排序分页;`ExplicitRefs` 按 request refs 去重分页;success 更新 snapshot / `ReferenceResolutionStatus`;failure 写 failed / stale marker | failed 保留 last good snapshot;affected views stale;Project scope 从 persisted truth / reference index 枚举,不从 projection / string id 临时派生 refs | 是 |

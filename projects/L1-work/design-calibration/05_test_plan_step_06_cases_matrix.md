...
| `TC-WORK-FORMAL-001` | 创建正式 WorkItem | P0 | Project `Active`;Backlog `Open`;assignee policy 通过;source ref 可解析 | 调用 `CreateWorkItem` | `WorkItem.work_state = WorkItemState::Formalized`;`title` / `source_ref` / `method_definition_ref` 持久化;加入 Backlog membership;enqueue `WorkItemChanged` | Work truth、source ref、method ref、trace、audit、outbox、board/search/member views stale | 是 |
...
| `TC-WORK-FORMAL-005` | ChildWorkItem 创建与非法 parent | P0 | root WorkItem `Formalized`;另有 child 或 terminal work 作为非法 parent | 调用 `CreateChildWorkItem` | root parent 成功生成 `ChildWorkItem` `Formalized`;`title` / `source_ref` / `method_definition_ref` 持久化;child / terminal parent 返回 reject | `WorkItemChanged`;parent ref 正确;非法 parent 无 truth / outbox | 是 |
...
| `TC-WORK-OPS-002` | RebuildWorkProjections | P0 | committed truth snapshot 存在且含 project source、formal work title/source/method fields;projection set 合法 | 运行 `RebuildWorkProjections` | `DerivedFreshnessState::Rebuilding` -> `Fresh`;optional `DerivedWorkViewChanged` | projection replace 来自 committed truth;search / board rows 使用 persisted title/source/method fields;不从 projection 反推 truth | 是 |
| `TC-WORK-OPS-003` | RefreshExternalReferenceSnapshots | P0 | stale refs、Project reference scope 或 explicit reference scope 存在;Project scope fixture 含 project source、work source/method、member / evidence / timebox typed refs 且有重复样本;projection affected-view index 有命中和空页样本;resolver 有 success / failure;已有 reference state / member snapshot / method snapshot 带 current version | 运行 `RefreshExternalReferenceSnapshots` | `None` / `StaleOnly` 读取 stale refs;`Project` 调用 `list_project_references(project_ref, page)` 并按 typed ref 去重排序分页;`ExplicitRefs` 按 request refs 去重分页;success 更新 snapshot / `ReferenceResolutionStatus`;用 success refs 调 `ProjectionRepository.list_views_affected_by_references(...)`;failure 写 failed / stale marker | failed 保留 last good snapshot;已有 reference / snapshot 更新必须先通过 versioned read 取得 expected_version,missing create/upsert 才能传 `None`;version conflict 不覆盖 last good snapshot 且 rollback 本条成功副作用;只有 repository 返回的 existing public affected views stale;affected view 空页不调用 `mark_stale`;Project scope 从 persisted truth / reference index 枚举,不从 projection / string id 临时派生 refs;service 不拼 `DerivedWorkViewRef` | 是 |
...
| `TC-WORK-OPS-005` | PrepareWorkTraceHandoff | P0 | trace records 存在;handoff port success / failure | 运行 `PrepareWorkTraceHandoff` | success 写 handoff marker;failure 写 `WorkJobFailureRef::TraceHandoff` failed refs;optional `WorkTraceAvailable` | failed ref 含 trace_id、subject_ref、target_ref;无 observability raw body;duplicate job 不重复 handoff body | 是 |
| `TC-WORK-OPS-006` | PrepareArchiveHandoff | P0 | `Subjects` scope 或 `ProjectCursor` scope 可由 `ArchiveSummaryRepository` 展开;archive port success / failure | 运行 `PrepareArchiveHandoff` | success 写 archive marker;failure 写 `WorkJobFailureRef::ArchiveHandoff` report | `Subjects` 只展开显式 subject refs;`ProjectCursor` 必须带 project_ref + source_cursor;只传 summary / refs;不写 archive long-term body;idempotent rerun | 是 |

...
| `consumer-outbox` | 7 Consumer、9 Outbound Event、outbox publication failed / retry / duplicate | main CI、release gate | merge / release candidate | 阻断 CI / release | `scripts/gates/run_main_gate.sh --suite consumer-outbox --config-profile ci-test` | `artifacts/test/<run_id>/consumer-outbox` | `reports/runs/<run_id>/consumer-outbox.md` |

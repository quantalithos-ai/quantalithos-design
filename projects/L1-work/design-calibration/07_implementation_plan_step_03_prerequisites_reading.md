...
| `MEM-WORK-009` | project | 设计修复后序任务 / 经验沉淀 | 修复设计文档后必须显式执行经验沉淀检查:先判断上一笔提交与本次改动是否属于同一个项目;不同项目则新增提交,但仍需在提交前判断本次是否产生可复用经验;同一项目则判断是否需要总结新经验,需要时把项目改动、经验更新和至少一个具体示例合并到同一笔上一提交,不需要时在最终回复中明确说明无新增经验;最后输出一段可交给遇阻 agent 继续实现的交接说明。 | 不适用 | `07-实施计划.md` | §3 Agent 启动与永久记忆种子表 | 每次修复设计文档后 / 提交前 | until superseded | 先完成经验沉淀检查,再提交或 amend,最后输出交接说明 | 是 |

经验沉淀示例: 若多次因 command duplicate replay、UnitOfWork 起点、shared DTO 二级类型、optimistic version 读取面等同类闭环缺口导致暂停,属于可复用经验,应更新对应标准、SOP 或项目永久记忆种子并给出具体示例;若只是单个 Work flow 的字段名或行文修正且不能泛化,则说明本次无新增可复用经验。
...
| 设计修复后经验检查 | 种子表包含修复设计文档后判断项目归属、提交合并方式、是否需要总结可复用经验、需要时添加示例的规则 | 不生成永久记忆,暂停并补种子表 |

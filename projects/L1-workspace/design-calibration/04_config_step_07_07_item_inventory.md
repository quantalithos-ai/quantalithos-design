...
配置域/项包括resource限额、timeout、store/owner/bus/cursor binding与observability redaction；数值必填、正数并满足关系约束，不提供默认值。

...
敏感项仅保存secret reference；cursor key、owner/bus credentials、DSN、TLS private material由secret provider读取，禁止Debug/Serialize/Clone和明文导出；轮换需保留兼容窗口。
...
| 之前 | 本步后 |
|---|---|
| 配置结论分散或未定义 | 形成可追溯的定义敏感配置与密钥管理结论 |

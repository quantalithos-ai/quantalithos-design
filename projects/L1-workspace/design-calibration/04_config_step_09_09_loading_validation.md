...
加载：解析来源→profile合并→未知键拒绝→类型/关系校验→secret ref解析→能力绑定→构造读写能力。启动失败fail-fast；安全/driver/limits变更需重启。
...
| 之前 | 本步后 |
|---|---|
| 配置结论分散或未定义 | 形成可追溯的定义配置加载、校验与生效机制结论 |

...
缺失/类型冲突/未知键启动fail-fast；owner不可达返回Unavailable，store写失败保留unknown并先resolve；cursor key不可用不明文降级；超限不截断Complete；sink丢失不影响业务。
...
| 之前 | 本步后 |
|---|---|
| 配置结论分散或未定义 | 形成可追溯的定义失效模式与降级/fail-fast策略结论 |

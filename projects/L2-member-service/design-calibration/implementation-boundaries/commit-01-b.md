...
| gate_status | pending |
| current_design_scope | Foundation / config |

## Boundary Header

本 boundary 的 planned 功能增量：Foundation / config。该文件是设计仓预创建台账，不代表代码、测试或提交存在。

## Required Reads

| document | required_section | status | notes |
|---|---|---|---|
| 03-详细设计.md | 03 §13、04 §3~§9、07 Step 8 | pending | 正式文档优先，冲突时暂停回写 |
...
| PH-01 calibration | Step-specific source | pending | Foundation / config 设计背景 |

## Allowed Scope

- config、builder、ports、deterministic fake。
...
- 业务 transition、真实 provider、event route。
...
|---|---|---|---|
| Design Gate | formal 03/04/05/06/07 与 03 §13、04 §3~§9、07 Step 8 逐项闭环 | pending | source refs only |
...
| Test Gate | GATE-MS-02 对应 targeted suite | pending | not_run |
...
|---|---|---|
| predecessor commit-01-a | pending / blocked | 由 owning source 闭合；未闭合保持 planned / wait_until_current |

...
| gate_status | pending |
| current_design_scope | Control service |

## Boundary Header

本 boundary 的 planned 功能增量：Control service。该文件是设计仓预创建台账，不代表代码、测试或提交存在。

## Required Reads

| document | required_section | status | notes |
|---|---|---|---|
| 03-详细设计.md | 03 §10~§12、05 idempotency、06 §8 | pending | 正式文档优先，冲突时暂停回写 |
...
| PH-02 calibration | Step-specific source | pending | Control service 设计背景 |

## Allowed Scope

- UoW、CAS、reservation、stored result。
...
|---|---|---|---|
| Design Gate | formal 03/04/05/06/07 与 03 §10~§12、05 idempotency、06 §8 逐项闭环 | pending | source refs only |
...
| Test Gate | GATE-MS-06 对应 targeted suite | pending | not_run |
...
|---|---|---|
| cursor/storage exact pending | pending / blocked | 由 owning source 闭合；未闭合保持 planned / wait_until_current |

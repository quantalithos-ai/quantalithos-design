...
| gate_status | pending |
| current_design_scope | Host generation |

## Boundary Header

本 boundary 的 planned 功能增量：Host generation。该文件是设计仓预创建台账，不代表代码、测试或提交存在。

## Required Reads

| document | required_section | status | notes |
|---|---|---|---|
| 03-详细设计.md | 03 §6、§8~§12、06 AC-MS-002/014 | pending | 正式文档优先，冲突时暂停回写 |
...
| PH-03 calibration | Step-specific source | pending | Host generation 设计背景 |

## Allowed Scope

- host、generation、action attempt。
...
- 真实 orchestrator、credential truth。
...
|---|---|---|---|
| Design Gate | formal 03/04/05/06/07 与 03 §6、§8~§12、06 AC-MS-002/014 逐项闭环 | pending | source refs only |
...
| Test Gate | GATE-MS-07 对应 targeted suite | pending | not_run |
...
|---|---|---|
| MSVC-UP-006 | pending / blocked | 由 owning source 闭合；未闭合保持 planned / wait_until_current |

...
| gate_status | pending |
| current_design_scope | Registration / session |

## Boundary Header

本 boundary 的 planned 功能增量：Registration / session。该文件是设计仓预创建台账，不代表代码、测试或提交存在。

## Required Reads

| document | required_section | status | notes |
|---|---|---|---|
| 03-详细设计.md | 03 §7~§12、06 AC-MS-003/011/012 | pending | 正式文档优先，冲突时暂停回写 |
...
| PH-03 calibration | Step-specific source | pending | Registration / session 设计背景 |

## Allowed Scope

- registration、endpoint、HostSession。
...
- Runtime run/turn/checkpoint、Member body。
...
|---|---|---|---|
| Design Gate | formal 03/04/05/06/07 与 03 §7~§12、06 AC-MS-003/011/012 逐项闭环 | pending | source refs only |
...
| Test Gate | GATE-MS-08 对应 targeted suite | pending | not_run |
...
|---|---|---|
| MSVC-UP-001/002 | pending / blocked | 由 owning source 闭合；未闭合保持 planned / wait_until_current |

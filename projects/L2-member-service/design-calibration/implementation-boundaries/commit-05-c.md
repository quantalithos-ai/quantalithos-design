...
| gate_status | pending |
| current_design_scope | Maintenance |

## Boundary Header

本 boundary 的 planned 功能增量：Maintenance。该文件是设计仓预创建台账，不代表代码、测试或提交存在。

## Required Reads

| document | required_section | status | notes |
|---|---|---|---|
| 03-详细设计.md | 03 §8/§10~§12、05 jobs | pending | 正式文档优先，冲突时暂停回写 |
...
| PH-05 calibration | Step-specific source | pending | Maintenance 设计背景 |

## Allowed Scope

- selector、single-item UoW、job result。
...
- new key、query repair、authorization。
...
|---|---|---|---|
| Design Gate | formal 03/04/05/06/07 与 03 §8/§10~§12、05 jobs 逐项闭环 | pending | source refs only |
...
| Test Gate | GATE-MS-15 对应 targeted suite | pending | not_run |
...
|---|---|---|
| predecessor commit-05-b | pending / blocked | 由 owning source 闭合；未闭合保持 planned / wait_until_current |

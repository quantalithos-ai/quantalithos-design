# Implementation Boundary Skeleton · commit-07-b
...
| activation posture | committed source cursor、receipt trace与 projection marker须重核 |

## Required Reads

- 项目 ledger、本文件、正式 `07-实施计划.md` §3、§6.3、§6.4、§6.11、§7、§10～§12。
- 正式 `03-详细设计.md` committed-fact Consumer、receipt / trace / projection source；`05/06` continuation / replay / projection cuts。
- 03 Step 7、9、11、13、16，07 Step 6、7；`commit-07-a` handoff后再次读取 current owner sources。

## Allowed Scope

- 仅 four committed-fact Consumer source refs、typed receipt / trace / projection relation、local continuation entry与 replay fence。
- 仅 `BATCH-07-02`。

## Forbidden Scope

- arbitrary Store scan、source repair、receipt reconstruction、semantic candidate consumption、Event materialization、generic listener。
- raw body、broker ACK、external success和 direct Store mutation。

## Planned Batches

| batch | future purpose |
|---|---|
| `BATCH-07-02` | committed-fact Consumer receipts and source fence |

## Required Checks

Future only：`GATE-04/06`、api-worker-entry、replay-recovery、projection-readmodel、receipt relation and no-source-scan check。
...
- `L2M-DDD-003`：receipt save/get / replay source未闭合。
- `L2M-DDD-007`：projection source/version未闭合。
- `L2M-UP-005`：24 candidate仍 non-materialized。

## Activation Rule

project ledger仅在 `commit-07-a` handoff、new baseline、committed source / receipt / projection mapping均闭合后推进。缺任一 relation时 `wait_design`，不能以 scan或 reconstruction补口。
...
| field | value |
|---|---|
| planned title | `feat(consumer): add committed-fact receipt continuation` |

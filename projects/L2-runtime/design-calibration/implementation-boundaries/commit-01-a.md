# commit-01-a — planned boundary skeleton

> phase: PH-01
> status: blocked / wait_design
...
| phase | PH-01 |
| stable IMPL | IMPL-01-01~03 |
| stable BATCH | BATCH-01-01~03 |
| stable Gate | GATE-01 |
| baseline | not_bound |
...
This is a planned handoff contract, not implementation authorization. Design-period conclusion is pass-designed at most; actual Gate status is blocked. The implementation repository is absent.

## Boundary Intent

**workspace skeleton** is one reviewable functional increment. It must not invent fields, states, Ports, configuration defaults, evidence or owner truth.
...
- Batch A: workspace manifests, seven member shells, only-Core candidate;
...
- owning tests and checks for GATE-01.

## Forbidden Scope

- business DTOs, shadow Core, framework/provider choice;
...
|---|---|---|
| A | IMPL-01-01 / BATCH-01-01 | typed carriers, factories and domain invariant |
| B | IMPL-01-02 / BATCH-01-02 | application service, Port, UoW, local adapter/fake parity |
| C | IMPL-01-03 / BATCH-01-03 | entry/consumer/job plus negative/replay/Unknown/zero-write tests |
...
| phase/owner/dependency/Rustdoc | pass-designed; blocker: L2R-IMPL-001; L2R-LANG-001/002; Core and binary identity pending |

## Gate Matrix

| sub-gate | future condition | design status | next action |
|---|---|---|---|
| Activation | sole current, predecessor handoff, baseline | blocked | wait_design |
| Design | Required Reads and formal closure | blocked | wait_design |
| Scope | allowed paths, one boundary, user isolation | blocked | wait_design |
| Worktree | real branch/HEAD/status/ownership | blocked | wait_design |
| Build | applicable nonempty tool output | blocked | wait_design |
| Test | nonempty selectors and negative/replay coverage | blocked | wait_design |
| Evidence | same-run pairing or exact N/A | blocked | wait_design |
| Commit | message/staged diff/identity/record | blocked | wait_design |
| Handoff | post-status/blocker/next boundary/ledger | blocked | wait_design |
...
|---|---|---|---|
| L2R-IMPL-001; L2R-LANG-001/002; Core and binary identity pending | open/pending | affected activation or positive scope remains blocked | fake, path existence, ACK, ping, planned text |
| target repo / immutable baseline | open | implementation cannot activate | alternate repo, dirty HEAD as baseline |

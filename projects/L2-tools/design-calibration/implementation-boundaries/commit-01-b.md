...
| item | contract |
|---|---|
| objective | Establish strict configuration shells, script CLIs and explicit run-scoped artifact/report path contracts. |
| formal authority | formal 04 §§3~12; formal 05 §§8~9/13; formal 06 §10; formal 07 §§6~8 |
| primary selector | CFG-001/007 plus path, schema and no-static-evidence fixtures |
| planned title | `chore(tooling): add strict config and run-scoped roots` |
| required body groups | `Strict configuration shell:`; `Run-scoped tooling roots:` |
...
|---|---|---|---|
| project ledger | `current_boundary = commit-01-b` | Project ledger has not advanced to this future boundary. | Remain planned; do not edit implementation files. |
| predecessor | real Commit and Handoff Gate for `commit-01-a`; `none` only for first boundary | `pending` | Inactive until predecessor is closed. |
...
| `design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md` | exact `commit-01-b` row and batches | `pending` | exact scope and batch authority |
| `design-calibration/07_implementation_plan_step_07_tests_acceptance_gates.md` | exact `commit-01-b` selector/gate row | `pending` | test and evidence direction |
...
| formal/calibration authority | formal 04 §§3~12; formal 05 §§8~9/13; formal 06 §10; formal 07 §§6~8; 04_config_step_07_config_items.md; 04_config_step_09_loading_validation_activation.md; 05_test_plan_step_09_automation_gates.md; 05_test_plan_step_13_evidence.md | `pending` | exact schema, flow, state and oracle source |
...
|---|---|---|
| allowed_rule | configuration schema/candidate shells without semantic defaults | `planned` |
| allowed_rule | `scripts/{gates,reports,checks,dev}` argument and path parsers | `planned` |
| allowed_rule | run-root, artifact/report schema and no-static-evidence fixtures | `planned` |
| allowed_rule | targeted tests and fixtures owned by this boundary may change with the owning behavior | `planned` |
| forbidden_rule | real run results, acceptance decisions or business logic | `active` |
| forbidden_rule | `latest`, project-subdirectory raw roots or cross-run aliases | `active` |
| forbidden_rule | secret values, provider readiness or static pass files | `active` |
...
|---|---|---|---|
| `BATCH-01-B1` | Create strict candidate/config schema shells. | CFG-001/007 shape and unknown-key fixtures | `planned` |
| `BATCH-01-B2` | Create script CLI and explicit root argument shells. | path and missing-argument dry runs | `planned` |
| `BATCH-01-B3` | Create artifact/report schema and no-static-evidence checks. | empty-safe builders without pass claims | `planned` |
...
| targeted behavior | strict shape, explicit run roots, missing/invalid CLI inputs, forbidden aliases and zero fabricated result | `pending` |
| static boundary | artifact/report root topology; no `latest`; no project subdirectory under raw root | `pending` |
...
|---|---|---|
| selector | CFG-001/007 plus path, schema and no-static-evidence fixtures expands to exact formal TC identities at execution time | `pending` |
| positive/negative | strict shape, explicit run roots, missing/invalid CLI inputs, forbidden aliases and zero fabricated result | `pending` |
...
| commit_message | planned subject is `chore(tooling): add strict config and run-scoped roots` with required groups `Strict configuration shell:` and `Run-scoped tooling roots:` | `pending` |
...
| field | value |
|---|---|
| planned_commit_message | `chore(tooling): add strict config and run-scoped roots`; planned only |
...
| next boundary | must become `commit-02-a` only after project-ledger transition | `pending` |
...
|---|---|---|---|---|
| `BLK-L2T-COMMIT_01_B-ACTIVATION-001` | Activation | `planned` | Project ledger has not advanced through predecessor handoff. | `wait_until_current` |
...
|---|---|---|
| boundary-specific review | Configuration binding and artifact materialization rules are applicable; no script output can become evidence before a real run. | Recheck the exact formal/calibration sources before implementation. |

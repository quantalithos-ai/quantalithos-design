...
| item | contract |
|---|---|
| objective | Implement the public typed-reference, metadata, shared carrier and error foundation with exact codec behavior. |
| formal authority | formal 03 §§5~7/11; formal 05 §§3/6; formal 06 §§6~7; formal 07 §§6~7 |
| primary selector | FOUNDATION-001~018 public-contract subset |
| planned title | `feat(contracts): add the public tool contract foundation` |
| required body groups | `Typed references and metadata:`; `Shared carriers and errors:` |
...
|---|---|---|---|
| project ledger | `current_boundary = commit-02-a` | Project ledger has not advanced to this future boundary. | Remain planned; do not edit implementation files. |
| predecessor | real Commit and Handoff Gate for `commit-01-b`; `none` only for first boundary | `pending` | Inactive until predecessor is closed. |
...
| `design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md` | exact `commit-02-a` row and batches | `pending` | exact scope and batch authority |
| `design-calibration/07_implementation_plan_step_07_tests_acceptance_gates.md` | exact `commit-02-a` selector/gate row | `pending` | test and evidence direction |
...
| formal/calibration authority | formal 03 §§5~7/11; formal 05 §§3/6; formal 06 §§6~7; formal 07 §§6~7; 03_ddd_step_06_object_contracts.md and shared/module annexes; 03_ddd_step_08_shared_protocol_carriers_annex.md | `pending` | exact schema, flow, state and oracle source |
...
|---|---|---|
| allowed_rule | `crates/contracts` public declarations, constructors and codec tests | `planned` |
| allowed_rule | typed refs, metadata/version carriers, `ProtocolError` and body-free shared carriers | `planned` |
| allowed_rule | complete English Rustdoc for every public declaration, field, variant and payload | `planned` |
| allowed_rule | targeted tests and fixtures owned by this boundary may change with the owning behavior | `planned` |
| forbidden_rule | domain transitions, Store/UoW implementation or external adapters | `active` |
| forbidden_rule | local aliases for missing Core or formal Tools types | `active` |
| forbidden_rule | provider body, secret value or implementation-only fields in public DTOs | `active` |
...
|---|---|---|---|
| `BATCH-02-A1` | Implement typed refs, metadata and version carriers. | construction and kind/ordering/dedup checks | `planned` |
| `BATCH-02-A2` | Implement shared carriers and `ProtocolError` surfaces. | body-free and source/error invariants | `planned` |
| `BATCH-02-A3` | Add codec, invalid-input and Rustdoc fixtures. | FOUNDATION contract subset closure | `planned` |
...
| targeted behavior | constructor validation, serde roundtrip, unknown/invalid values, body-free output and Rustdoc coverage | `pending` |
| static boundary | public type ownership; no duplicated Core schema; no undocumented public field/variant | `pending` |
...
|---|---|---|
| selector | FOUNDATION-001~018 public-contract subset expands to exact formal TC identities at execution time | `pending` |
| positive/negative | constructor validation, serde roundtrip, unknown/invalid values, body-free output and Rustdoc coverage | `pending` |
...
| commit_message | planned subject is `feat(contracts): add the public tool contract foundation` with required groups `Typed references and metadata:` and `Shared carriers and errors:` | `pending` |
...
| field | value |
|---|---|
| planned_commit_message | `feat(contracts): add the public tool contract foundation`; planned only |
...
| next boundary | must become `commit-02-b` only after project-ledger transition | `pending` |
...
|---|---|---|---|---|
| `BLK-L2T-COMMIT_02_A-ACTIVATION-001` | Activation | `planned` | Project ledger has not advanced through predecessor handoff. | `wait_until_current` |
| `L2T-UP-008` | conditional positive scope | `open_upstream` | Core tools-specific schema/package authority remains candidate-only. | `wait_design before any copied or guessed shared surface` |
...
|---|---|---|
| boundary-specific review | Typed-ref owner, DTO construction and nested Rustdoc checks are mandatory; concrete services and provider adapters remain outside this boundary. | Recheck the exact formal/calibration sources before implementation. |

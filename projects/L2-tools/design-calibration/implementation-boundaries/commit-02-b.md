...
| item | contract |
|---|---|
| objective | Implement six state families, the 41-object domain foundation and pure invariant/error guards. |
| formal authority | formal 03 §§5~6/9/11; formal 05 §§3/6.6; formal 06 §§5/8; formal 07 §§6~7 |
| primary selector | STATE-001~012 plus pure ERR branches |
| planned title | `feat(domain): add tool state and invariant foundations` |
| required body groups | `Domain object foundations:`; `State guards and invariant tests:` |
| next boundary | `commit-02-c` |

## Activation Guard

| rule | required observation | current observation | consequence |
|---|---|---|---|
| project ledger | `current_boundary = commit-02-b` | Project ledger has not advanced to this future boundary. | Remain planned; do not edit implementation files. |
| predecessor | real Commit and Handoff Gate for `commit-02-a`; `none` only for first boundary | `pending` | Inactive until predecessor is closed. |
...
| `design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md` | exact `commit-02-b` row and batches | `pending` | exact scope and batch authority |
| `design-calibration/07_implementation_plan_step_07_tests_acceptance_gates.md` | exact `commit-02-b` selector/gate row | `pending` | test and evidence direction |
...
| formal/calibration authority | formal 03 §§5~6/9/11; formal 05 §§3/6.6; formal 06 §§5/8; formal 07 §§6~7; 03_ddd_step_06_object_contracts.md and recalibration annexes; 03_ddd_step_10_*_state_annex.md | `pending` | exact schema, flow, state and oracle source |
...
|---|---|---|
| allowed_rule | `crates/domain` objects, factories, state enums and pure policies | `planned` |
| allowed_rule | legal/illegal/terminal transition predicates and pure error mapping | `planned` |
| allowed_rule | domain unit, invariant and negative tests | `planned` |
| allowed_rule | targeted tests and fixtures owned by this boundary may change with the owning behavior | `planned` |
| forbidden_rule | Store/UoW calls, external Ports or runtime composition | `active` |
| forbidden_rule | Query mutation, Job repair or evidence/report generation | `active` |
| forbidden_rule | states, variants or defaults absent from formal 03 | `active` |
...
|---|---|---|---|
| `BATCH-02-B1` | Implement the six state enums and transition vocabulary. | legal/reserved/illegal/terminal matrix | `planned` |
| `BATCH-02-B2` | Implement object factories and invariant predicates for all owning object groups. | 41-object construction source checks | `planned` |
| `BATCH-02-B3` | Add pure negative and invariant tests. | STATE-001~012 and pure ERR closure | `planned` |
...
| targeted behavior | factory construction, legal/illegal transitions, terminal immutability, source symmetry and invalid invariant branches | `pending` |
| static boundary | all object/state names match formal 03; no persistence or external dependency in domain | `pending` |
...
|---|---|---|
| selector | STATE-001~012 plus pure ERR branches expands to exact formal TC identities at execution time | `pending` |
| positive/negative | factory construction, legal/illegal transitions, terminal immutability, source symmetry and invalid invariant branches | `pending` |
...
| commit_message | planned subject is `feat(domain): add tool state and invariant foundations` with required groups `Domain object foundations:` and `State guards and invariant tests:` | `pending` |
...
| field | value |
|---|---|
| planned_commit_message | `feat(domain): add tool state and invariant foundations`; planned only |
...
| next boundary | must become `commit-02-c` only after project-ledger transition | `pending` |
...
|---|---|---|---|---|
| `BLK-L2T-COMMIT_02_B-ACTIVATION-001` | Activation | `planned` | Project ledger has not advanced through predecessor handoff. | `wait_until_current` |
...
|---|---|---|
| boundary-specific review | State/factory/source closure is applicable; UoW, concrete Query, event, job and evidence semantics are intentionally deferred. | Recheck the exact formal/calibration sources before implementation. |

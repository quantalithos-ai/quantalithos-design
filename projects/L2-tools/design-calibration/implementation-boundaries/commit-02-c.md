...
| item | contract |
|---|---|
| objective | Implement Store/Port traits, UnitOfWork, CAS, idempotency, stored replay and deterministic fake parity foundations. |
| formal authority | formal 03 §§5.4~5.5/6.3/10~13; formal 05 §§4/6.6; formal 06 §8; formal 07 §§6~7 |
| primary selector | TX-001~010 and CONC-001~023 foundation branches |
...
|---|---|---|---|
| project ledger | `current_boundary = commit-02-c` | Project ledger has not advanced to this future boundary. | Remain planned; do not edit implementation files. |
| predecessor | real Commit and Handoff Gate for `commit-02-b`; `none` only for first boundary | `pending` | Inactive until predecessor is closed. |
...
| `design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md` | exact `commit-02-c` row and batches | `pending` | exact scope and batch authority |
| `design-calibration/07_implementation_plan_step_07_tests_acceptance_gates.md` | exact `commit-02-c` selector/gate row | `pending` | test and evidence direction |
...
| formal/calibration authority | formal 03 §§5.4~5.5/6.3/10~13; formal 05 §§4/6.6; formal 06 §8; formal 07 §§6~7; 03_ddd_step_07_trait_port_adapter_contracts.md; 03_ddd_step_11_persistence_transaction_consistency.md; Step 12~13 error/concurrency | `pending` | exact schema, flow, state and oracle source |
...
|---|---|---|
| allowed_rule | `crates/application` Store/Port traits, UoW, idempotency and replay primitives | `planned` |
| allowed_rule | `crates/infra` deterministic fake Stores/Ports and parity fixtures | `planned` |
| allowed_rule | CAS, semantic uniqueness, commit tri-state and stored-result tests | `planned` |
| allowed_rule | targeted tests and fixtures owned by this boundary may change with the owning behavior | `planned` |
| forbidden_rule | concrete capability services, API entries or runtime graph composition | `active` |
| forbidden_rule | hidden retries, generic cache fallbacks or product-specific public semantics | `active` |
| forbidden_rule | positive provider readiness or durable product selection by convenience | `active` |
...
|---|---|---|---|
| `BATCH-02-C1` | Declare exact Store and external Port traits. | method/key/result/version construction closure | `planned` |
| `BATCH-02-C2` | Implement UoW, CAS and commit tri-state primitives. | atomicity/conflict/unknown tests | `planned` |
| `BATCH-02-C3` | Implement idempotency reservation and stored replay. | winner/duplicate/digest mismatch behavior | `planned` |
| `BATCH-02-C4` | Implement deterministic fakes and trait parity fixtures. | TX/CONC foundation closure | `planned` |
...
| targeted behavior | UoW ordering, CAS conflict, semantic uniqueness, duplicate winner/replay, commit unknown and fake parity | `pending` |
| static boundary | trait methods match formal 03; no generic cache fallback; no concrete capability service | `pending` |
...
|---|---|---|
| selector | TX-001~010 and CONC-001~023 foundation branches expands to exact formal TC identities at execution time | `pending` |
| positive/negative | UoW ordering, CAS conflict, semantic uniqueness, duplicate winner/replay, commit unknown and fake parity | `pending` |
...
| commit_message | planned subject is `feat(application): add ports transactions and replay foundations` with required groups `Application ports and transactions:` and `Replay and fake parity:` | `pending` |
...
| field | value |
|---|---|
| planned_commit_message | `feat(application): add ports transactions and replay foundations`; planned only |
...
| next boundary | must become `commit-03-a` only after project-ledger transition | `pending` |
...
|---|---|---|---|---|
| `BLK-L2T-COMMIT_02_C-ACTIVATION-001` | Activation | `planned` | Project ledger has not advanced through predecessor handoff. | `wait_until_current` |
...
|---|---|---|
| boundary-specific review | Port/UoW/idempotency/fake-parity experience is fully applicable; selected durable adapters require a later capability Spike. | Recheck the exact formal/calibration sources before implementation. |

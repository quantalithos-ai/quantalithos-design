...
| phase | `PH-03` |
| predecessor | `commit-02-c` |
| next_boundary | `commit-03-b` |
...
| item | contract |
|---|---|
| objective | Implement the accepted Tool identity and formal definition creation/read vertical slice. |
| formal authority | formal 03 §§5~12 for CF-01/QF-01; formal 05 CONTRACT/QUERY/STATE cases; formal 06 AC-006; formal 07 §§6~7 |
| primary selector | CONTRACT-001~002, QUERY-001 and related STATE-001 |
| planned title | `feat(contract): add tool identity and definition flows` |
| required body groups | `Identity and definition contracts:`; `Accepted service and read surface:` |
| next boundary | `commit-03-b` |

## Activation Guard

| rule | required observation | current observation | consequence |
|---|---|---|---|
| project ledger | `current_boundary = commit-03-a` | Project ledger has not advanced to this future boundary. | Remain planned; do not edit implementation files. |
| predecessor | real Commit and Handoff Gate for `commit-02-c`; `none` only for first boundary | `pending` | Inactive until predecessor is closed. |
...
| `design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md` | exact `commit-03-a` row and batches | `pending` | exact scope and batch authority |
| `design-calibration/07_implementation_plan_step_07_tests_acceptance_gates.md` | exact `commit-03-a` selector/gate row | `pending` | test and evidence direction |
...
| formal/calibration authority | formal 03 §§5~12 for CF-01/QF-01; formal 05 CONTRACT/QUERY/STATE cases; formal 06 AC-006; formal 07 §§6~7; 03 contract/evolution module, object, protocol and flow annexes; 03_ddd_step_10_contract_evolution_state_annex.md | `pending` | exact schema, flow, state and oracle source |
...
|---|---|---|
| allowed_rule | identity/current-definition contract and domain owner files | `planned` |
| allowed_rule | CF-01 application service, Store/UoW integration and deterministic fake | `planned` |
| allowed_rule | QF-01 read surface, API mapper and targeted tests | `planned` |
| allowed_rule | targeted tests and fixtures owned by this boundary may change with the owning behavior | `planned` |
| forbidden_rule | revision adoption/retirement, Binding or Invocation behavior | `active` |
| forbidden_rule | capability registry, marketplace/provider inventory or Runtime planning | `active` |
| forbidden_rule | Query refresh, repair or external resolution | `active` |
...
|---|---|---|---|
| `BATCH-03-A1` | Implement identity/definition request and result carriers. | exact construction and identity digest | `planned` |
| `BATCH-03-A2` | Implement `ToolContract` and `FormalToolDefinition` factories. | accepted/current state invariants | `planned` |
| `BATCH-03-A3` | Implement CF-01 service, Store/UoW and replay. | accepted creation vertical slice | `planned` |
| `BATCH-03-A4` | Implement QF-01 read and no-write tests. | CONTRACT-001~002/QUERY-001 closure | `planned` |
...
| targeted behavior | identity exactness, definition validation, accepted create, duplicate stored replay, current read and zero-write | `pending` |
| static boundary | identity/current-definition owner paths only; no later capability dependency | `pending` |
...
|---|---|---|
| selector | CONTRACT-001~002, QUERY-001 and related STATE-001 expands to exact formal TC identities at execution time | `pending` |
| positive/negative | identity exactness, definition validation, accepted create, duplicate stored replay, current read and zero-write | `pending` |
...
| commit_message | planned subject is `feat(contract): add tool identity and definition flows` with required groups `Identity and definition contracts:` and `Accepted service and read surface:` | `pending` |
...
| field | value |
|---|---|
| planned_commit_message | `feat(contract): add tool identity and definition flows`; planned only |
...
| next boundary | must become `commit-03-b` only after project-ledger transition | `pending` |
...
|---|---|---|---|---|
| `BLK-L2T-COMMIT_03_A-ACTIVATION-001` | Activation | `planned` | Project ledger has not advanced through predecessor handoff. | `wait_until_current` |
...
|---|---|---|
| boundary-specific review | Current/stored-result/CAS/Query no-write rules apply; adoption, external provider and delivery behavior are not in scope. | Recheck the exact formal/calibration sources before implementation. |

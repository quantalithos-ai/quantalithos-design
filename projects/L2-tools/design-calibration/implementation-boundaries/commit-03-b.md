...
| item | contract |
|---|---|
| objective | Implement revision impact, adoption, retirement, immutable history and replay for Tool contracts. |
| formal authority | formal 03 §§5~12 for CF-02~04/QF-02; formal 05 CONTRACT/STATE/TX; formal 06 AC-007~008; formal 07 §§6~7 |
| primary selector | CONTRACT-003~008, QUERY-002, STATE-002/009~012 and TX-001~002 |
| planned title | `feat(evolution): add tool revision lifecycle flows` |
| required body groups | `Revision impact and adoption:`; `Retirement history and replay:` |
...
|---|---|---|---|
| project ledger | `current_boundary = commit-03-b` | Project ledger has not advanced to this future boundary. | Remain planned; do not edit implementation files. |
| predecessor | real Commit and Handoff Gate for `commit-03-a`; `none` only for first boundary | `pending` | Inactive until predecessor is closed. |
...
| `design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md` | exact `commit-03-b` row and batches | `pending` | exact scope and batch authority |
| `design-calibration/07_implementation_plan_step_07_tests_acceptance_gates.md` | exact `commit-03-b` selector/gate row | `pending` | test and evidence direction |
...
| formal/calibration authority | formal 03 §§5~12 for CF-02~04/QF-02; formal 05 CONTRACT/STATE/TX; formal 06 AC-007~008; formal 07 §§6~7; 03 contract-evolution object/protocol/flow/state annexes; 03 Step 11~13 persistence/replay | `pending` | exact schema, flow, state and oracle source |
...
|---|---|---|
| allowed_rule | revision candidate, compatibility impact, adoption and retirement owner files | `planned` |
| allowed_rule | versioned current/history Store surfaces, projection-stale marker and QF-02 | `planned` |
| allowed_rule | CAS, duplicate/replay, illegal transition and history tests | `planned` |
| allowed_rule | targeted tests and fixtures owned by this boundary may change with the owning behavior | `planned` |
| forbidden_rule | Binding, Invocation, Query-triggered refresh or later projection rebuild | `active` |
| forbidden_rule | registry, marketplace, external delivery or provider selection | `active` |
| forbidden_rule | history rewrite, mutable terminal revisions or code-only compatibility defaults | `active` |
...
|---|---|---|---|
| `BATCH-03-B1` | Implement revision candidate and compatibility impact. | impact source and construction tests | `planned` |
| `BATCH-03-B2` | Implement adoption CAS and immutable history append. | current/history atomicity | `planned` |
| `BATCH-03-B3` | Implement retirement closure and terminal guards. | illegal/terminal tests | `planned` |
| `BATCH-03-B4` | Implement QF-02 history reads and stored replay. | CONTRACT-003~008 closure | `planned` |
...
| targeted behavior | impact/adopt/retire, stale expected version, immutable history, terminal protection, replay and no-write history reads | `pending` |
| static boundary | no later Binding/Invocation surface; all revision status/reason fields source from formal 03 | `pending` |
...
|---|---|---|
| selector | CONTRACT-003~008, QUERY-002, STATE-002/009~012 and TX-001~002 expands to exact formal TC identities at execution time | `pending` |
| positive/negative | impact/adopt/retire, stale expected version, immutable history, terminal protection, replay and no-write history reads | `pending` |
...
| commit_message | planned subject is `feat(evolution): add tool revision lifecycle flows` with required groups `Revision impact and adoption:` and `Retirement history and replay:` | `pending` |
...
| field | value |
|---|---|
| planned_commit_message | `feat(evolution): add tool revision lifecycle flows`; planned only |
...
| next boundary | must become `commit-04-a` only after project-ledger transition | `pending` |
...
|---|---|---|---|---|
| `BLK-L2T-COMMIT_03_B-ACTIVATION-001` | Activation | `planned` | Project ledger has not advanced through predecessor handoff. | `wait_until_current` |
...
|---|---|---|
| boundary-specific review | Revision current/history, CAS, stored replay and Query purity are applicable; external readiness remains irrelevant. | Recheck the exact formal/calibration sources before implementation. |

...
| item | contract |
|---|---|
| objective | Implement Capability Binding relation, anchor, snapshot and assessment contracts with pure state guards. |
| formal authority | formal 03 binding objects/protocols/state; formal 05 BIND-001~004; formal 06 AC-009; formal 07 §§6~7 |
| primary selector | BIND-001~004 and related STATE-003 |
| planned title | `feat(binding): add capability binding contracts` |
| required body groups | `Binding relation contracts:`; `Snapshot and assessment guards:` |
| next boundary | `commit-04-b` |

## Activation Guard

| rule | required observation | current observation | consequence |
|---|---|---|---|
| project ledger | `current_boundary = commit-04-a` | Project ledger has not advanced to this future boundary. | Remain planned; do not edit implementation files. |
| predecessor | real Commit and Handoff Gate for `commit-03-b`; `none` only for first boundary | `pending` | Inactive until predecessor is closed. |
...
| `design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md` | exact `commit-04-a` row and batches | `pending` | exact scope and batch authority |
| `design-calibration/07_implementation_plan_step_07_tests_acceptance_gates.md` | exact `commit-04-a` selector/gate row | `pending` | test and evidence direction |
...
| formal/calibration authority | formal 03 binding objects/protocols/state; formal 05 BIND-001~004; formal 06 AC-009; formal 07 §§6~7; 03_ddd_step_06_module_binding_source_annex.md; binding protocol/flow/state annexes | `pending` | exact schema, flow, state and oracle source |
...
|---|---|---|
| allowed_rule | binding contracts/domain owner files and typed Hub references | `planned` |
| allowed_rule | `CapabilityBinding` factory/state/guard and body-free snapshot/assessment carriers | `planned` |
| allowed_rule | pure relation/ref/kind/state/invalid-input tests | `planned` |
| allowed_rule | targeted tests and fixtures owned by this boundary may change with the owning behavior | `planned` |
| forbidden_rule | Hub registry/exposure truth, provider body or authorization decision | `active` |
| forbidden_rule | binding service/UoW, Hub mutation or positive readiness | `active` |
| forbidden_rule | name/default inference or local provider inventory | `active` |
...
|---|---|---|---|
| `BATCH-04-A1` | Implement relation, anchor and typed reference carriers. | body-free/kind/source checks | `planned` |
| `BATCH-04-A2` | Implement `CapabilityBinding` state and factories. | bound/unbound/invalidated invariants | `planned` |
| `BATCH-04-A3` | Add pure BIND guard and negative tests. | BIND-001~004 closure | `planned` |
...
| targeted behavior | relation construction, anchor/ref validation, bound/unbound exactness, body-free snapshots and illegal transitions | `pending` |
| static boundary | no Hub registry or provider body; typed reference owner and redaction checks | `pending` |
...
|---|---|---|
| selector | BIND-001~004 and related STATE-003 expands to exact formal TC identities at execution time | `pending` |
| positive/negative | relation construction, anchor/ref validation, bound/unbound exactness, body-free snapshots and illegal transitions | `pending` |
...
| commit_message | planned subject is `feat(binding): add capability binding contracts` with required groups `Binding relation contracts:` and `Snapshot and assessment guards:` | `pending` |
...
| field | value |
|---|---|
| planned_commit_message | `feat(binding): add capability binding contracts`; planned only |
...
| next boundary | must become `commit-04-b` only after project-ledger transition | `pending` |
...
|---|---|---|---|---|
| `BLK-L2T-COMMIT_04_A-ACTIVATION-001` | Activation | `planned` | Project ledger has not advanced through predecessor handoff. | `wait_until_current` |
...
|---|---|---|
| boundary-specific review | Relation source, typed-ref ownership, state guards and redaction apply; registry/provider truth is explicitly forbidden. | Recheck the exact formal/calibration sources before implementation. |

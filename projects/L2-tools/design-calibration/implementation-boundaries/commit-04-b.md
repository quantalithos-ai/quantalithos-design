...
| item | contract |
|---|---|
| objective | Implement binding persistence/services, controlled Capability Hub consumption and the read-only binding query. |
| formal authority | formal 03 binding Port/service/flow/TX; formal 05 BIND-005~008/QUERY-003; formal 06 AC-010~011; formal 07 §§6~8 |
| primary selector | BIND-005~008, QUERY-003 and TX-002 |
| planned title | `feat(binding): complete controlled binding consumption` |
| required body groups | `Binding services and persistence:`; `Controlled Hub seam and query:` |
...
|---|---|---|---|
| project ledger | `current_boundary = commit-04-b` | Project ledger has not advanced to this future boundary. | Remain planned; do not edit implementation files. |
| predecessor | real Commit and Handoff Gate for `commit-04-a`; `none` only for first boundary | `pending` | Inactive until predecessor is closed. |
...
| `design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md` | exact `commit-04-b` row and batches | `pending` | exact scope and batch authority |
| `design-calibration/07_implementation_plan_step_07_tests_acceptance_gates.md` | exact `commit-04-b` selector/gate row | `pending` | test and evidence direction |
...
| formal/calibration authority | formal 03 binding Port/service/flow/TX; formal 05 BIND-005~008/QUERY-003; formal 06 AC-010~011; formal 07 §§6~8; 03 binding application/external Port/flow/state annexes; 07 Step 8 dependency readiness row | `pending` | exact schema, flow, state and oracle source |
...
|---|---|---|
| allowed_rule | binding application service, versioned Store/UoW and API seam | `planned` |
| allowed_rule | controlled/blocked Hub consumption Port adapter and deterministic fake parity | `planned` |
| allowed_rule | QF-03 read, CAS/replay/no-refresh and source-gap tests | `planned` |
| allowed_rule | targeted tests and fixtures owned by this boundary may change with the owning behavior | `planned` |
| forbidden_rule | local capability registry, Hub mutation or provider readiness claim | `active` |
| forbidden_rule | name/default/visibility inference or raw provider body persistence | `active` |
| forbidden_rule | Query refresh, authorization evaluation or cross-owner writes | `active` |
...
|---|---|---|---|
| `BATCH-04-B1` | Implement controlled Hub Port and typed failure mapping. | missing/conflict/unverifiable symmetry | `planned` |
| `BATCH-04-B2` | Implement CF-05~07 binding service, Store and CAS. | declare/replace/invalidate lifecycle | `planned` |
| `BATCH-04-B3` | Implement QF-03 binding read surface. | visibility/no-write behavior | `planned` |
| `BATCH-04-B4` | Add fake parity, replay and no-refresh tests. | BIND-005~008/QUERY-003 closure | `planned` |
...
| targeted behavior | declare/replace/invalidate CAS, controlled source failure mapping, duplicate replay, QF-03 zero-write/no-refresh | `pending` |
| static boundary | non-Core dependency zero; no local registry/default inference; no positive readiness output | `pending` |
...
|---|---|---|
| selector | BIND-005~008, QUERY-003 and TX-002 expands to exact formal TC identities at execution time | `pending` |
| positive/negative | declare/replace/invalidate CAS, controlled source failure mapping, duplicate replay, QF-03 zero-write/no-refresh | `pending` |
...
| commit_message | planned subject is `feat(binding): complete controlled binding consumption` with required groups `Binding services and persistence:` and `Controlled Hub seam and query:` | `pending` |
...
| field | value |
|---|---|
| planned_commit_message | `feat(binding): complete controlled binding consumption`; planned only |
...
| next boundary | must become `commit-05-a` only after project-ledger transition | `pending` |
...
|---|---|---|---|---|
| `BLK-L2T-COMMIT_04_B-ACTIVATION-001` | Activation | `planned` | Project ledger has not advanced through predecessor handoff. | `wait_until_current` |
...
|---|---|---|
| boundary-specific review | CAS, controlled adapter, relation persistence, redaction and Query no-write apply; Hub registry ownership remains external. | Recheck the exact formal/calibration sources before implementation. |

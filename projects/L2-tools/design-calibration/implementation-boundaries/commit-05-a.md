...
| item | contract |
|---|---|
| objective | Implement canonical Tool invocation identity, anchor, context and admission without performing execution. |
| formal authority | formal 03 invocation objects/CF-08/QF-04/state; formal 05 INV-001~008; formal 06 AC-012~014; formal 07 §§6~7 |
| primary selector | INV-001~008, QUERY-004 and related STATE-004 |
| planned title | `feat(invocation): add canonical invocation admission` |
| required body groups | `Canonical invocation contracts:`; `Admission and no-execution state:` |
...
|---|---|---|---|
| project ledger | `current_boundary = commit-05-a` | Project ledger has not advanced to this future boundary. | Remain planned; do not edit implementation files. |
| predecessor | real Commit and Handoff Gate for `commit-04-b`; `none` only for first boundary | `pending` | Inactive until predecessor is closed. |
...
| `design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md` | exact `commit-05-a` row and batches | `pending` | exact scope and batch authority |
| `design-calibration/07_implementation_plan_step_07_tests_acceptance_gates.md` | exact `commit-05-a` selector/gate row | `pending` | test and evidence direction |
...
| formal/calibration authority | formal 03 invocation objects/CF-08/QF-04/state; formal 05 INV-001~008; formal 06 AC-012~014; formal 07 §§6~7; 03_ddd_step_06_module_invocation_admission_annex.md; invocation protocol/flow/state annexes | `pending` | exact schema, flow, state and oracle source |
...
|---|---|---|
| allowed_rule | invocation contracts/domain owner files and canonicalization policy | `planned` |
| allowed_rule | CF-08 application admission service and QF-04 input/read surface | `planned` |
| allowed_rule | caller/carrier parity, no-execution, idempotency and invalid-context tests | `planned` |
| allowed_rule | targeted tests and fixtures owned by this boundary may change with the owning behavior | `planned` |
| forbidden_rule | authorization/policy decision, Sandbox call or Runtime loop/planning | `active` |
| forbidden_rule | raw prompt/tool body persistence or caller-specific semantic forks | `active` |
| forbidden_rule | execution result, receipt, outcome or external status | `active` |
...
|---|---|---|---|
| `BATCH-05-A1` | Implement invocation metadata and context carriers. | caller-independent construction | `planned` |
| `BATCH-05-A2` | Implement anchor and canonicalizer. | semantic identity/digest parity | `planned` |
| `BATCH-05-A3` | Implement admission state/service and no-execution tests. | INV-001~008 closure | `planned` |
...
| targeted behavior | caller parity, canonical digest, malformed context, duplicate admission replay, rejected/no-execution and QF-04 purity | `pending` |
| static boundary | no Sandbox/Auth/Runtime call; no prompt/body field; no caller-specific DTO | `pending` |
...
|---|---|---|
| selector | INV-001~008, QUERY-004 and related STATE-004 expands to exact formal TC identities at execution time | `pending` |
| positive/negative | caller parity, canonical digest, malformed context, duplicate admission replay, rejected/no-execution and QF-04 purity | `pending` |
...
| commit_message | planned subject is `feat(invocation): add canonical invocation admission` with required groups `Canonical invocation contracts:` and `Admission and no-execution state:` | `pending` |
...
| field | value |
|---|---|
| planned_commit_message | `feat(invocation): add canonical invocation admission`; planned only |
...
| next boundary | must become `commit-05-b` only after project-ledger transition | `pending` |
...
|---|---|---|---|---|
| `BLK-L2T-COMMIT_05_A-ACTIVATION-001` | Activation | `planned` | Project ledger has not advanced through predecessor handoff. | `wait_until_current` |
| `L2T-UP-009` | conditional positive scope | `open_upstream` | SDK caller seam is future-only and cannot define canonical server semantics. | `keep SDK integration outside current P0 scope` |
...
|---|---|---|
| boundary-specific review | Selected input source, canonical identity, stored replay and no-execution state apply; authorization and runtime ownership are deferred. | Recheck the exact formal/calibration sources before implementation. |

...
| item | contract |
|---|---|
| objective | Implement execution requirements and fail-closed Authorization result consumption without owning authorization truth. |
| formal authority | formal 03 precondition objects/CF-09/Port/error; formal 05 PRE-001~004/ERR-007~008; formal 06 AC-015~016/VF; formal 07 §§6~9 |
| primary selector | PRE-001~004 and ERR-007~008 |
| planned title | `feat(precondition): add fail-closed execution preconditions` |
| required body groups | `Execution requirement mapping:`; `Authorization consumption failures:` |
| next boundary | `commit-05-c` |

## Activation Guard

| rule | required observation | current observation | consequence |
|---|---|---|---|
| project ledger | `current_boundary = commit-05-b` | Project ledger has not advanced to this future boundary. | Remain planned; do not edit implementation files. |
| predecessor | real Commit and Handoff Gate for `commit-05-a`; `none` only for first boundary | `pending` | Inactive until predecessor is closed. |
...
| `design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md` | exact `commit-05-b` row and batches | `pending` | exact scope and batch authority |
| `design-calibration/07_implementation_plan_step_07_tests_acceptance_gates.md` | exact `commit-05-b` selector/gate row | `pending` | test and evidence direction |
...
| formal/calibration authority | formal 03 precondition objects/CF-09/Port/error; formal 05 PRE-001~004/ERR-007~008; formal 06 AC-015~016/VF; formal 07 §§6~9; 03_ddd_step_06_module_precondition_handoff_annex.md; precondition protocol/flow/state/error annexes | `pending` | exact schema, flow, state and oracle source |
...
|---|---|---|
| allowed_rule | `ExecutionRequirement` and typed Authorization consumption assessment | `planned` |
| allowed_rule | Authorization consumption Port, blocked adapter/fake and CF-09 service | `planned` |
| allowed_rule | allow/constrained/deny/missing/stale/conflict/unverifiable mapping tests | `planned` |
| allowed_rule | targeted tests and fixtures owned by this boundary may change with the owning behavior | `planned` |
| forbidden_rule | L2-owned authorization, approval, policy evaluation or risk taxonomy | `active` |
| forbidden_rule | default allow, visibility-as-authorization or raw decision body | `active` |
| forbidden_rule | Sandbox call, host fallback or positive provider claim | `active` |
...
|---|---|---|---|
| `BATCH-05-B1` | Implement execution requirement carriers and mapping. | risk/requirement source checks | `planned` |
| `BATCH-05-B2` | Implement Authorization consumption Port and assessments. | typed result/failure symmetry | `planned` |
| `BATCH-05-B3` | Implement CF-09 service/error mapping. | fail-closed application behavior | `planned` |
| `BATCH-05-B4` | Add negative/blocked/fake-parity tests. | PRE-001~004/ERR closure | `planned` |
...
| targeted behavior | allow/constrained/deny, missing/stale/conflict/unverifiable, no default allow, replay and redaction | `pending` |
| static boundary | no policy evaluator or authorization owner in L2; no raw provider body; blocked adapter parity | `pending` |
...
|---|---|---|
| selector | PRE-001~004 and ERR-007~008 expands to exact formal TC identities at execution time | `pending` |
| positive/negative | allow/constrained/deny, missing/stale/conflict/unverifiable, no default allow, replay and redaction | `pending` |
...
| commit_message | planned subject is `feat(precondition): add fail-closed execution preconditions` with required groups `Execution requirement mapping:` and `Authorization consumption failures:` | `pending` |
...
| field | value |
|---|---|
| planned_commit_message | `feat(precondition): add fail-closed execution preconditions`; planned only |
...
| next boundary | must become `commit-05-c` only after project-ledger transition | `pending` |
...
|---|---|---|---|---|
| `BLK-L2T-COMMIT_05_B-ACTIVATION-001` | Activation | `planned` | Project ledger has not advanced through predecessor handoff. | `wait_until_current` |
| `L2T-UP-001~002` | conditional positive scope | `open_upstream` | Authorization owner/source/taxonomy/schema/freshness remain open for positive qualification. | `wait_design for positive adapter; local negative remains planned` |
...
|---|---|---|
| boundary-specific review | Input-source map, fail-closed state, typed failure and redaction rules apply; self-authorization and default allow are vetoed. | Recheck the exact formal/calibration sources before implementation. |

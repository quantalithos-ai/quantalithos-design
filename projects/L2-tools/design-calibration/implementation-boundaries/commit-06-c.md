...
| item | contract |
|---|---|
| objective | Implement local external-submission attempt state and independent Bus/Observability status-reference views. |
| formal authority | formal 03 handoff/continuation objects/CF-13/QF-06; formal 05 HANDOFF-005~008/CONT; formal 06 AC-022/VF; formal 07 §§6~10 |
| primary selector | HANDOFF-005~008, CONT-001~004 and CONC-010~012 subset |
| planned title | `feat(outbound): add local submission attempt state` |
| required body groups | `Local submission attempts:`; `Independent external status views:` |
| next boundary | `commit-07-a` |

## Activation Guard

| rule | required observation | current observation | consequence |
|---|---|---|---|
| project ledger | `current_boundary = commit-06-c` | Project ledger has not advanced to this future boundary. | Remain planned; do not edit implementation files. |
| predecessor | real Commit and Handoff Gate for `commit-06-b` | `pending` | Inactive until predecessor is closed. |
...
| `design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md` | exact `commit-06-c` row and batches | `pending` | exact scope and batch authority |
| `design-calibration/07_implementation_plan_step_07_tests_acceptance_gates.md` | exact `commit-06-c` selector/gate row | `pending` | test and evidence direction |
...
| formal/calibration authority | formal 03 handoff/continuation objects/CF-13/QF-06; formal 05 HANDOFF-005~008/CONT; formal 06 AC-022/VF; formal 07 §§6~10; 03 outcome/audit/handoff module and continuation protocol/flow/state annexes; Step 11~13 | `pending` | exact schema, flow, state and oracle source |
...
|---|---|---|
| allowed_rule | `ExternalSubmissionAttempt` domain/application/Store and versioned local claim | `planned` |
| allowed_rule | safe submission Port seam, local submitted/rejected/unknown disposition and independent status refs | `planned` |
| allowed_rule | duplicate/replay/unknown/no-delivery tests and QF-06 view mapping | `planned` |
| allowed_rule | targeted tests and fixtures owned by this boundary may change with the owning behavior | `planned` |
| forbidden_rule | Delivered/Observed inference, queue/DLQ/retry lifecycle or external status ownership | `active` |
| forbidden_rule | second external call after unknown or hidden fallback | `active` |
| forbidden_rule | event body persistence, route invention or acceptance readiness | `active` |
...
|---|---|---|---|
| `BATCH-06-C1` | Implement local submission attempt states and carriers. | state/source/terminal matrix | `planned` |
| `BATCH-06-C2` | Implement Store, claim and one-call local disposition. | semantic uniqueness and unknown fence | `planned` |
| `BATCH-06-C3` | Implement independent Bus/Observability status/gap views. | no reverse-write mapping | `planned` |
| `BATCH-06-C4` | Add replay/unknown/no-delivery tests. | HANDOFF-005~008/CONT closure | `planned` |
...
| targeted behavior | claim uniqueness, local submitted/rejected/unknown, replay, no second call, independent status refs and no Delivered/Observed inference | `pending` |
| static boundary | no queue/DLQ/retry owner; event remains body-free; status refs never rewrite local outcome | `pending` |
...
|---|---|---|
| selector | HANDOFF-005~008, CONT-001~004 and CONC-010~012 subset expands to exact formal TC identities at execution time | `pending` |
| positive/negative | claim uniqueness, local submitted/rejected/unknown, replay, no second call, independent status refs and no Delivered/Observed inference | `pending` |
...
| commit_message | planned subject is `feat(outbound): add local submission attempt state` with required groups `Local submission attempts:` and `Independent external status views:` | `pending` |
...
| field | value |
|---|---|
| planned_commit_message | `feat(outbound): add local submission attempt state`; planned only |
...
| next boundary | must become `commit-07-a` only after project-ledger transition | `pending` |
...
|---|---|---|---|---|
| `BLK-L2T-COMMIT_06_C-ACTIVATION-001` | Activation | `planned` | Project ledger has not advanced through predecessor handoff. | `wait_until_current` |
| `L2T-UP-004~006` | conditional positive scope | `open_upstream` | Sandbox feedback and Bus/Observability route/source/status positive contracts remain open. | `positive continuation remains blocked/conditional` |
...
|---|---|---|
| boundary-specific review | Local side-effect marker, one-call, unknown, body-free and independent status experience applies; external delivery truth is forbidden. | Recheck exact formal/calibration sources before implementation. |

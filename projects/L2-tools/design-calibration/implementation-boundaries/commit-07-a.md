...
| phase | `PH-07` |
| predecessor | `commit-06-c` |
| next_boundary | `commit-07-b` |
...
| item | contract |
|---|---|
| objective | Implement Query metadata, page/cursor/visibility contracts, named read Ports and zero-write test infrastructure. |
| formal authority | formal 03 Query contracts/Ports/flow/state; formal 05 QUERY foundation; formal 06 AC-023~025/VF; formal 07 §§6~7 |
| primary selector | QUERY foundation, QUERY-001~002 source fixtures and FOUNDATION-006 |
| planned title | `feat(query): add read-only query foundations` |
| required body groups | `Visibility and page contracts:`; `Read ports and no-write guards:` |
...
|---|---|---|---|
| project ledger | `current_boundary = commit-07-a` | Project ledger has not advanced to this future boundary. | Remain planned; do not edit implementation files. |
| predecessor | real Commit and Handoff Gate for `commit-06-c` | `pending` | Inactive until predecessor is closed. |
...
| `design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md` | exact `commit-07-a` row and batches | `pending` | exact scope and batch authority |
| `design-calibration/07_implementation_plan_step_07_tests_acceptance_gates.md` | exact `commit-07-a` selector/gate row | `pending` | test and evidence direction |
...
| formal/calibration authority | formal 03 Query contracts/Ports/flow/state; formal 05 QUERY foundation; formal 06 AC-023~025/VF; formal 07 §§6~7; 03 query protocol/flow annexes; 03_ddd_step_07_module_application_stores_annex.md; integrity/derived module annex | `pending` | exact schema, flow, state and oracle source |
...
|---|---|---|
| allowed_rule | Query metadata, page/cursor/visibility and response carriers | `planned` |
| allowed_rule | named read Port traits, read-shell services and zero-write/zero-external-call spies | `planned` |
| allowed_rule | pagination/visibility/invalid-cursor/no-write foundation tests | `planned` |
| allowed_rule | targeted tests and fixtures owned by this boundary may change with the owning behavior | `planned` |
| forbidden_rule | mutation, UoW/reservation, projection refresh/rebuild or repair | `active` |
| forbidden_rule | new marker kind, local resolver or external Port call from Query | `active` |
| forbidden_rule | body scan, fallback cache or visibility-as-authorization | `active` |
...
|---|---|---|---|
| `BATCH-07-A1` | Implement page, cursor, visibility and Query response contracts. | codec/validation/missing semantics | `planned` |
| `BATCH-07-A2` | Declare named read Ports and read-only service shells. | callable/source closure | `planned` |
| `BATCH-07-A3` | Implement zero-write/zero-external-call spies and fixtures. | Query-purity foundation | `planned` |
...
| targeted behavior | page/cursor validation, visibility outcomes, NotFound/NotVisible separation, zero UoW/write/external calls | `pending` |
| static boundary | Query dependency graph contains no mutation/UoW/external Port; no new marker kind | `pending` |
...
|---|---|---|
| selector | QUERY foundation, QUERY-001~002 source fixtures and FOUNDATION-006 expands to exact formal TC identities at execution time | `pending` |
| positive/negative | page/cursor validation, visibility outcomes, NotFound/NotVisible separation, zero UoW/write/external calls | `pending` |
...
| commit_message | planned subject is `feat(query): add read-only query foundations` with required groups `Visibility and page contracts:` and `Read ports and no-write guards:` | `pending` |
...
| field | value |
|---|---|
| planned_commit_message | `feat(query): add read-only query foundations`; planned only |
...
| next boundary | must become `commit-07-b` only after project-ledger transition | `pending` |
...
|---|---|---|---|---|
| `BLK-L2T-COMMIT_07_A-ACTIVATION-001` | Activation | `planned` | Project ledger has not advanced through predecessor handoff. | `wait_until_current` |
...
|---|---|---|
| boundary-specific review | Query response/source/visibility and no-write instrumentation apply; mutation, refresh and external resolution are explicitly out of scope. | Recheck exact formal/calibration sources before implementation. |

...
| phase | `PH-07` |
| predecessor | `commit-07-a` |
| next_boundary | `commit-07-c` |
...
| item | contract |
|---|---|
| objective | Implement QF-01 through QF-06 core Tool contract, binding, invocation, precondition and outcome read surfaces. |
| formal authority | formal 03 QF-01~06 protocols/flows/Stores; formal 05 QUERY-001~006; formal 06 AC-023~025/031; formal 07 §§6~7 |
| primary selector | QUERY-001~006 |
| planned title | `feat(query): add core tool read surfaces` |
| required body groups | `Core read bundles:`; `Query handlers and degraded mapping:` |
| next boundary | `commit-07-c` |

## Activation Guard

| rule | required observation | current observation | consequence |
|---|---|---|---|
| project ledger | `current_boundary = commit-07-b` | Project ledger has not advanced to this future boundary. | Remain planned; do not edit implementation files. |
| predecessor | real Commit and Handoff Gate for `commit-07-a` | `pending` | Inactive until predecessor is closed. |
...
| `design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md` | exact `commit-07-b` row and batches | `pending` | exact scope and batch authority |
| `design-calibration/07_implementation_plan_step_07_tests_acceptance_gates.md` | exact `commit-07-b` selector/gate row | `pending` | test and evidence direction |
...
| formal/calibration authority | formal 03 QF-01~06 protocols/flows/Stores; formal 05 QUERY-001~006; formal 06 AC-023~025/031; formal 07 §§6~7; 03 query protocol and flow annexes for QF-01~06; module application stores/entry annexes | `pending` | exact schema, flow, state and oracle source |
...
|---|---|---|
| allowed_rule | QF-01~06 application query services, read views and API mappers | `planned` |
| allowed_rule | visibility/page/degraded mapping over existing Stores/read Ports | `planned` |
| allowed_rule | no-write, fake parity, missing/stale/unavailable and pagination tests | `planned` |
| allowed_rule | targeted tests and fixtures owned by this boundary may change with the owning behavior | `planned` |
| forbidden_rule | UoW, reserve, capture, repair, rebuild or fallback from Query | `active` |
| forbidden_rule | provider call, visibility refresh or source-truth mutation | `active` |
| forbidden_rule | derived QF-07~11 projection implementation | `active` |
...
|---|---|---|---|
| `BATCH-07-B1` | Implement QF-01~03 contract and binding reads. | current/history/relation visibility | `planned` |
| `BATCH-07-B2` | Implement QF-04~06 invocation/precondition/outcome reads. | source/status separation | `planned` |
| `BATCH-07-B3` | Implement API response mappers. | typed degraded/error surface | `planned` |
| `BATCH-07-B4` | Add no-write, visibility and fake-parity tests. | QUERY-001~006 closure | `planned` |
...
| targeted behavior | Found/NotFound/NotVisible/Stale/Unavailable, pagination, fake parity and zero UoW/write/refresh/external calls | `pending` |
| static boundary | QF-01~06 only; no mutation dependency or later projection owner; API mapping preserves typed outcomes | `pending` |
...
|---|---|---|
| selector | QUERY-001~006 expands to exact formal TC identities at execution time | `pending` |
| positive/negative | Found/NotFound/NotVisible/Stale/Unavailable, pagination, fake parity and zero UoW/write/refresh/external calls | `pending` |
...
| commit_message | planned subject is `feat(query): add core tool read surfaces` with required groups `Core read bundles:` and `Query handlers and degraded mapping:` | `pending` |
...
| field | value |
|---|---|
| planned_commit_message | `feat(query): add core tool read surfaces`; planned only |
...
| next boundary | must become `commit-07-c` only after project-ledger transition | `pending` |
...
|---|---|---|---|---|
| `BLK-L2T-COMMIT_07_B-ACTIVATION-001` | Activation | `planned` | Project ledger has not advanced through predecessor handoff. | `wait_until_current` |
...
|---|---|---|
| boundary-specific review | Read-source, typed response, visibility, page and no-write rules fully apply; refresh/repair remains forbidden. | Recheck exact formal/calibration sources before implementation. |

...
| item | contract |
|---|---|
| objective | Implement derived integrity, report, search, diff, diagnostic and guidance projections for QF-07 through QF-11. |
| formal authority | formal 03 integrity/derived objects/QF-07~11/state/persistence; formal 05 QUERY-007~011/CONC; formal 06 AC-023~025/031; formal 07 §§6~7 |
| primary selector | QUERY-007~011, CONC-016~017 and JOB-003 read inputs |
| planned title | `feat(projection): add derived tool read material` |
| required body groups | `Derived projection storage:`; `Reports diagnostics and guidance:` |
...
|---|---|---|---|
| project ledger | `current_boundary = commit-07-c` | Project ledger has not advanced to this future boundary. | Remain planned; do not edit implementation files. |
| predecessor | real Commit and Handoff Gate for `commit-07-b` | `pending` | Inactive until predecessor is closed. |
...
| `design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md` | exact `commit-07-c` row and batches | `pending` | exact scope and batch authority |
| `design-calibration/07_implementation_plan_step_07_tests_acceptance_gates.md` | exact `commit-07-c` selector/gate row | `pending` | test and evidence direction |
...
| formal/calibration authority | formal 03 integrity/derived objects/QF-07~11/state/persistence; formal 05 QUERY-007~011/CONC; formal 06 AC-023~025/031; formal 07 §§6~7; 03_ddd_step_06_module_integrity_derived_annex.md; query flow/state/persistence/job-read annexes | `pending` | exact schema, flow, state and oracle source |
...
|---|---|---|
| allowed_rule | derived projection rows/Store, watermark and source-ref indices | `planned` |
| allowed_rule | QF-07~11 report/search/diff/diagnostic/guidance services and API mappers | `planned` |
| allowed_rule | fresh/stale/unavailable/degraded/no-write and projection-source tests | `planned` |
| allowed_rule | targeted tests and fixtures owned by this boundary may change with the owning behavior | `planned` |
| forbidden_rule | Query-triggered rebuild/refresh/repair or core truth mutation | `active` |
| forbidden_rule | event publication, external resolver or hidden cache fallback | `active` |
| forbidden_rule | invented SDK client, marketplace listing or provider readiness guidance | `active` |
...
|---|---|---|---|
| `BATCH-07-C1` | Implement integrity report, search and diff views. | source/ref/ordering checks | `planned` |
| `BATCH-07-C2` | Implement diagnostics and guidance views. | typed degraded/non-authoritative semantics | `planned` |
| `BATCH-07-C3` | Implement projection Store, watermark and indices. | freshness/source contracts | `planned` |
| `BATCH-07-C4` | Add stale/rebuild-unavailable/no-write tests. | QUERY-007~011 closure | `planned` |
...
| targeted behavior | projection source/watermark, ordering/dedup, fresh/stale/unavailable, zero-write/no rebuild and non-authoritative guidance | `pending` |
| static boundary | no Query rebuild/repair/event publish; guidance does not create SDK/provider/marketplace truth | `pending` |
...
|---|---|---|
| selector | QUERY-007~011, CONC-016~017 and JOB-003 read inputs expands to exact formal TC identities at execution time | `pending` |
| positive/negative | projection source/watermark, ordering/dedup, fresh/stale/unavailable, zero-write/no rebuild and non-authoritative guidance | `pending` |
...
| commit_message | planned subject is `feat(projection): add derived tool read material` with required groups `Derived projection storage:` and `Reports diagnostics and guidance:` | `pending` |
...
| field | value |
|---|---|
| planned_commit_message | `feat(projection): add derived tool read material`; planned only |
...
| next boundary | must become `commit-08-a` only after project-ledger transition | `pending` |
...
|---|---|---|---|---|
| `BLK-L2T-COMMIT_07_C-ACTIVATION-001` | Activation | `planned` | Project ledger has not advanced through predecessor handoff. | `wait_until_current` |
| `L2T-UP-009` | conditional positive scope | `open_upstream` | SDK tools-client seam remains future-only; guidance cannot claim an existing client. | `keep guidance non-authoritative and future-oriented` |
...
|---|---|---|
| boundary-specific review | Projection source, watermark, typed degraded response and no-write experience apply; query-side rebuild and truth repair are forbidden. | Recheck exact formal/calibration sources before implementation. |

...
| item | contract |
|---|---|
| objective | Implement inbound envelopes, source/version validation, claims, stored receipts and blocked-aware consumer assessment paths. |
| formal authority | formal 03 IF-01/02/04/05 protocols/flows/TX; formal 05 CONSUMER-001/002/004/005; formal 06 AC-016/018/021/029; formal 07 §§6~10 |
| primary selector | CONSUMER-001/002/004/005 and entry TX/CONC branches |
| planned title | `feat(consumer): add inbound tool consumer receipts` |
| required body groups | `Envelope and claim lifecycle:`; `Stored receipts and source assessments:` |
| next boundary | `commit-08-b` |

## Activation Guard

| rule | required observation | current observation | consequence |
|---|---|---|---|
| project ledger | `current_boundary = commit-08-a` | Project ledger has not advanced to this future boundary. | Remain planned; do not edit implementation files. |
| predecessor | real Commit and Handoff Gate for `commit-07-c` | `pending` | Inactive until predecessor is closed. |
...
| `design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md` | exact `commit-08-a` row and batches | `pending` | exact scope and batch authority |
| `design-calibration/07_implementation_plan_step_07_tests_acceptance_gates.md` | exact `commit-08-a` selector/gate row | `pending` | test and evidence direction |
...
| formal/calibration authority | formal 03 IF-01/02/04/05 protocols/flows/TX; formal 05 CONSUMER-001/002/004/005; formal 06 AC-016/018/021/029; formal 07 §§6~10; 03 consumer protocol/flow annexes; module entry-boundary, external-Port and Store annexes | `pending` | exact schema, flow, state and oracle source |
...
|---|---|---|
| allowed_rule | inbound envelope/header/source/version validators | `planned` |
| allowed_rule | consumer claim, typed receipt/rejection/quarantine/gap Store and worker entries | `planned` |
| allowed_rule | IF-01/02/04/05 controlled source calls, replay and blocked tests | `planned` |
| allowed_rule | targeted tests and fixtures owned by this boundary may change with the owning behavior | `planned` |
| forbidden_rule | broker ack/DLQ/retry lifecycle or source-owner truth | `active` |
| forbidden_rule | direct core mutation outside formal application command re-entry | `active` |
| forbidden_rule | raw body logging/persistence, guessed source mapping or hidden fallback | `active` |
...
|---|---|---|---|
| `BATCH-08-A1` | Implement inbound envelope and header validators. | source/version/context checks | `planned` |
| `BATCH-08-A2` | Implement claim and stored receipt lifecycle. | semantic uniqueness/replay | `planned` |
| `BATCH-08-A3` | Implement IF-01/02/04/05 assessment paths. | owner-separated typed mapping | `planned` |
| `BATCH-08-A4` | Add duplicate/blocked/quarantine tests. | CONSUMER subset closure | `planned` |
...
| targeted behavior | invalid header/source/version, claim race, duplicate stored receipt, blocked source, quarantine/gap and no body leakage | `pending` |
| static boundary | worker calls formal application surface only; no broker lifecycle, direct Store mutation or body log | `pending` |
...
|---|---|---|
| selector | CONSUMER-001/002/004/005 and entry TX/CONC branches expands to exact formal TC identities at execution time | `pending` |
| positive/negative | invalid header/source/version, claim race, duplicate stored receipt, blocked source, quarantine/gap and no body leakage | `pending` |
...
| commit_message | planned subject is `feat(consumer): add inbound tool consumer receipts` with required groups `Envelope and claim lifecycle:` and `Stored receipts and source assessments:` | `pending` |
...
| field | value |
|---|---|
| planned_commit_message | `feat(consumer): add inbound tool consumer receipts`; planned only |
...
| next boundary | must become `commit-08-b` only after project-ledger transition | `pending` |
...
|---|---|---|---|---|
| `BLK-L2T-COMMIT_08_A-ACTIVATION-001` | Activation | `planned` | Project ledger has not advanced through predecessor handoff. | `wait_until_current` |
| `L2T-UP-001~002,005~006` | conditional positive scope | `open_upstream` | Authorization and Bus/Observability positive source families remain open. | `retain blocked assessment paths until owner closure` |
...
|---|---|---|
| boundary-specific review | Envelope/context, claim/receipt save-get, replay and owner-separated source mapping apply; broker ack/DLQ is out of scope. | Recheck exact formal/calibration sources before implementation. |

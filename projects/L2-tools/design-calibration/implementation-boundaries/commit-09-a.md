...
| item | contract |
|---|---|
| objective | Implement public bounded Job metadata, scope, journal, checkpoint, frozen plan/target and report carriers. |
| formal authority | formal 03 JF-01~04 protocols/Stores/TX; formal 05 JOB protocol/codec cases; formal 06 AC-023/025/031/038; formal 07 §§6~7 |
| primary selector | JOB-001~004 protocol and codec fixtures |
| planned title | `feat(jobs): add bounded job protocol foundation` |
| required body groups | `Job contracts and journal:`; `Frozen targets and report carriers:` |
| next boundary | `commit-09-b` |

## Activation Guard

| rule | required observation | current observation | consequence |
|---|---|---|---|
| project ledger | `current_boundary = commit-09-a` | Project ledger has not advanced to this future boundary. | Remain planned; do not edit implementation files. |
| predecessor | real Commit and Handoff Gate for `commit-08-b` | `pending` | Inactive until predecessor is closed. |
...
| `design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md` | exact `commit-09-a` row and batches | `pending` | exact scope and batch authority |
| `design-calibration/07_implementation_plan_step_07_tests_acceptance_gates.md` | exact `commit-09-a` selector/gate row | `pending` | test and evidence direction |
...
| formal/calibration authority | formal 03 JF-01~04 protocols/Stores/TX; formal 05 JOB protocol/codec cases; formal 06 AC-023/025/031/038; formal 07 §§6~7; 03 job protocol/flow annexes; application Store and jobs entry annexes | `pending` | exact schema, flow, state and oracle source |
...
|---|---|---|
| allowed_rule | Job metadata/scope/cursor/input/output/report public contracts | `planned` |
| allowed_rule | journal/checkpoint/idempotency and frozen plan/target application shells | `planned` |
| allowed_rule | codec, invalid-scope, duplicate/replay and complete Rustdoc tests | `planned` |
| allowed_rule | targeted tests and fixtures owned by this boundary may change with the owning behavior | `planned` |
| forbidden_rule | generic execute-anything surface, unbounded scan or recursive Command invocation | `active` |
| forbidden_rule | core truth mutation, scheduler/lease ownership or release verdict | `active` |
| forbidden_rule | concrete JF runners beyond protocol/journal foundation | `active` |
...
|---|---|---|---|
| `BATCH-09-A1` | Implement public Job DTOs and complete Rustdoc. | codec/construction closure | `planned` |
| `BATCH-09-A2` | Implement journal/checkpoint/idempotency shell. | stored replay contract | `planned` |
| `BATCH-09-A3` | Implement frozen plan and target schema. | boundedness/immutability | `planned` |
| `BATCH-09-A4` | Add invalid/replay/codec tests. | JOB protocol subset closure | `planned` |
...
| targeted behavior | bounded scope validation, cursor/checkpoint, frozen target, report construction, duplicate stored replay and Rustdoc | `pending` |
| static boundary | no generic execute, unbounded scan, core mutation, scheduler/lease or verdict surface | `pending` |
...
|---|---|---|
| selector | JOB-001~004 protocol and codec fixtures expands to exact formal TC identities at execution time | `pending` |
| positive/negative | bounded scope validation, cursor/checkpoint, frozen target, report construction, duplicate stored replay and Rustdoc | `pending` |
...
| commit_message | planned subject is `feat(jobs): add bounded job protocol foundation` with required groups `Job contracts and journal:` and `Frozen targets and report carriers:` | `pending` |
...
| field | value |
|---|---|
| planned_commit_message | `feat(jobs): add bounded job protocol foundation`; planned only |
...
| next boundary | must become `commit-09-b` only after project-ledger transition | `pending` |
...
|---|---|---|---|---|
| `BLK-L2T-COMMIT_09_A-ACTIVATION-001` | Activation | `planned` | Project ledger has not advanced through predecessor handoff. | `wait_until_current` |
...
|---|---|---|
| boundary-specific review | Bounded plan/output/report, journal, idempotency and Rustdoc experience apply; runners, scheduler and core mutation are excluded. | Recheck exact formal/calibration sources before implementation. |

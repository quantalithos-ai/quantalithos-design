...
| phase | `PH-10` |
| predecessor | `commit-09-b` |
| next_boundary | `commit-10-b` |
...
| item | contract |
|---|---|
| objective | Implement the ten-root, 54-item strict configuration candidate, loader, validator and activation preconditions. |
| formal authority | formal 04 §§3~14; formal 03 §13; formal 05 CFG-001~007/CFG-T/A/F/X; formal 06 config redlines; formal 07 §§6~10 |
| primary selector | CFG-001~007 and all CFG-T/A/F/X branches |
| planned title | `feat(config): add strict tools configuration activation` |
| required body groups | `Strict candidate validation:`; `Profile and cross-field activation:` |
| next boundary | `commit-10-b` |

## Activation Guard

| rule | required observation | current observation | consequence |
|---|---|---|---|
| project ledger | `current_boundary = commit-10-a` | Project ledger has not advanced to this future boundary. | Remain planned; do not edit implementation files. |
| predecessor | real Commit and Handoff Gate for `commit-09-b` | `pending` | Inactive until predecessor is closed. |
...
| `design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md` | exact `commit-10-a` row and batches | `pending` | exact scope and batch authority |
| `design-calibration/07_implementation_plan_step_07_tests_acceptance_gates.md` | exact `commit-10-a` selector/gate row | `pending` | test and evidence direction |
...
| formal/calibration authority | formal 04 §§3~14; formal 03 §13; formal 05 CFG-001~007/CFG-T/A/F/X; formal 06 config redlines; formal 07 §§6~10; 04_config_step_07_config_items.md; Step 5~11 source/profile/loading/failure artifacts; 03 config-binding annex | `pending` | exact schema, flow, state and oracle source |
...
|---|---|---|
| allowed_rule | `tools-infra` typed config candidate, strict parser, source/profile merge and validator | `planned` |
| allowed_rule | ten roots, 54 canonical items, V0~V8 preconditions and B0 input preparation | `planned` |
| allowed_rule | CFG-T/A/F/X, profile isolation, redline, secret-ref and invalid-high tests | `planned` |
| allowed_rule | targeted tests and fixtures owned by this boundary may change with the owning behavior | `planned` |
| forbidden_rule | remote config, hot reload, LKG, env alias or lower-priority fallback | `active` |
| forbidden_rule | secret/raw reference values in logs/digests/reports | `active` |
| forbidden_rule | runtime graph exposure, provider readiness or semantic truth mutation | `active` |
...
|---|---|---|---|
| `BATCH-10-A1` | Implement ten roots and 54 typed configuration items. | strict shape/type/required-set tests | `planned` |
| `BATCH-10-A2` | Implement loader, source merge and validation pipeline. | V0~V4 and conflict tests | `planned` |
| `BATCH-10-A3` | Implement profile/cross-field/capability/redline validation. | V5~V8 and NC sweep | `planned` |
| `BATCH-10-A4` | Add CFG-T/A/F/X and safe-output tests. | strict activation closure | `planned` |
...
| targeted behavior | unknown/missing/type/source/profile/conflict/cross-field/capability/redline/secret-ref/no-fallback cases | `pending` |
| static boundary | exact 10/54 inventory; no aliases/hot reload/LKG/raw secrets; no partial runtime entry | `pending` |
...
|---|---|---|
| selector | CFG-001~007 and all CFG-T/A/F/X branches expands to exact formal TC identities at execution time | `pending` |
| positive/negative | unknown/missing/type/source/profile/conflict/cross-field/capability/redline/secret-ref/no-fallback cases | `pending` |
...
| commit_message | planned subject is `feat(config): add strict tools configuration activation` with required groups `Strict candidate validation:` and `Profile and cross-field activation:` | `pending` |
...
| field | value |
|---|---|
| planned_commit_message | `feat(config): add strict tools configuration activation`; planned only |
...
| next boundary | must become `commit-10-b` only after project-ledger transition | `pending` |
...
|---|---|---|---|---|
| `BLK-L2T-COMMIT_10_A-ACTIVATION-001` | Activation | `planned` | Project ledger has not advanced through predecessor handoff. | `wait_until_current` |
...
|---|---|---|
| boundary-specific review | Config item/source/profile/activation/failure and safe-output rules fully apply; hot reload, secret resolver and readiness do not. | Recheck exact formal/calibration sources before implementation. |

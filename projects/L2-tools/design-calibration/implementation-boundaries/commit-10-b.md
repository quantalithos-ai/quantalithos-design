...
| item | contract |
|---|---|
| objective | Implement runtime composition, Store/UoW/Port capability checks, seven entry bundles and controlled-adapter parity. |
| formal authority | formal 03 §§5.5~5.8/13; formal 04 B0~B8; formal 05 config-assembly; formal 06 dependency/config redlines; formal 07 §§6~10 |
| primary selector | config-assembly, B0~B8 and FOUNDATION-016~018 |
| planned title | `feat(runtime): compose controlled tool adapters` |
| required body groups | `Runtime graph composition:`; `Entry and adapter parity:` |
| next boundary | `commit-11-a` |

## Activation Guard

| rule | required observation | current observation | consequence |
|---|---|---|---|
| project ledger | `current_boundary = commit-10-b` | Project ledger has not advanced to this future boundary. | Remain planned; do not edit implementation files. |
| predecessor | real Commit and Handoff Gate for `commit-10-a` | `pending` | Inactive until predecessor is closed. |
...
| `design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md` | exact `commit-10-b` row and batches | `pending` | exact scope and batch authority |
| `design-calibration/07_implementation_plan_step_07_tests_acceptance_gates.md` | exact `commit-10-b` selector/gate row | `pending` | test and evidence direction |
...
| formal/calibration authority | formal 03 §§5.5~5.8/13; formal 04 B0~B8; formal 05 config-assembly; formal 06 dependency/config redlines; formal 07 §§6~10; 03 infra/entry/external-Port annexes; 04 loading/activation/downstream handoff artifacts | `pending` | exact schema, flow, state and oracle source |
...
|---|---|---|
| allowed_rule | runtime builder, immutable validated config consumption and dependency capability checks | `planned` |
| allowed_rule | Store/UoW/Clock/ID/Port/facade/api/worker/jobs composition | `planned` |
| allowed_rule | controlled/disabled/blocked adapter registration, fake parity and no-partial-graph tests | `planned` |
| allowed_rule | targeted tests and fixtures owned by this boundary may change with the owning behavior | `planned` |
| forbidden_rule | provider readiness claims, non-Core sibling Cargo dependencies or owner-truth adapters | `active` |
| forbidden_rule | fallback execution, half graph/entry exposure or invalid-high fallback | `active` |
| forbidden_rule | new public schema, config key, state or transport selected by composition | `active` |
...
|---|---|---|---|
| `BATCH-10-B1` | Implement builder dependency and capability checks. | B0~B2 prerequisites | `planned` |
| `BATCH-10-B2` | Compose Store/UoW/Clock/ID and Port slots. | B3~B5 atomic graph | `planned` |
| `BATCH-10-B3` | Compose facades and api/worker/jobs entries. | B6~B7 zero partial exposure | `planned` |
| `BATCH-10-B4` | Add controlled/disabled adapter parity and redline checks. | B8/composition closure | `planned` |
...
| targeted behavior | missing capability, controlled/disabled/blocked parity, entry bundle completeness, no partial graph, dependency and profile isolation | `pending` |
| static boundary | only Core compile candidate; all seven entries use one validated immutable graph; no fallback/readiness promotion | `pending` |
...
|---|---|---|
| selector | config-assembly, B0~B8 and FOUNDATION-016~018 expands to exact formal TC identities at execution time | `pending` |
| positive/negative | missing capability, controlled/disabled/blocked parity, entry bundle completeness, no partial graph, dependency and profile isolation | `pending` |
...
| commit_message | planned subject is `feat(runtime): compose controlled tool adapters` with required groups `Runtime graph composition:` and `Entry and adapter parity:` | `pending` |
...
| field | value |
|---|---|
| planned_commit_message | `feat(runtime): compose controlled tool adapters`; planned only |
...
| next boundary | must become `commit-11-a` only after project-ledger transition | `pending` |
...
|---|---|---|---|---|
| `BLK-L2T-COMMIT_10_B-ACTIVATION-001` | Activation | `planned` | Project ledger has not advanced through predecessor handoff. | `wait_until_current` |
| `L2T-UP-001~009` | conditional positive scope | `open_upstream` | Positive external slots, Core exact authority and SDK consumer remain conditional/open. | `compose only controlled/disabled/blocked local seams until exact owner closure` |
...
|---|---|---|
| boundary-specific review | Composition, adapter capability parity, dependency direction and no-partial-graph rules apply; provider readiness and owner truth remain external. | Recheck exact formal/calibration sources before implementation. |

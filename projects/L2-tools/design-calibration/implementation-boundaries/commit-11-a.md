...
| phase | `PH-11` |
| predecessor | `commit-10-b` |
| next_boundary | `commit-11-b` |
...
| item | contract |
|---|---|
| objective | Implement run-scoped test execution, raw/suite reports, all mandatory checks and evidence-index derivation tooling. |
| formal authority | formal 05 §§6~14; formal 06 §§3/10~11; formal 07 §§6~12 |
| primary selector | all case/check/evidence schema fixtures, 234-case manifest and OBS-001~009 |
| planned title | `feat(testing): add run-scoped test evidence tooling` |
| required body groups | `Raw artifacts and suite reports:`; `Checks and evidence derivation:` |
| next boundary | `commit-11-b` |

## Activation Guard

| rule | required observation | current observation | consequence |
|---|---|---|---|
| project ledger | `current_boundary = commit-11-a` | Project ledger has not advanced to this future boundary. | Remain planned; do not edit implementation files. |
| predecessor | real Commit and Handoff Gate for `commit-10-b` | `pending` | Inactive until predecessor is closed. |
...
| `design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md` | exact `commit-11-a` row and batches | `pending` | exact scope and batch authority |
| `design-calibration/07_implementation_plan_step_07_tests_acceptance_gates.md` | exact `commit-11-a` selector/gate row | `pending` | test and evidence direction |
...
| formal/calibration authority | formal 05 §§6~14; formal 06 §§3/10~11; formal 07 §§6~12; 05 test-plan automation/evidence artifacts; 06 acceptance evidence/veto artifacts; 07 Step 7 exact check set | `pending` | exact schema, flow, state and oracle source |
...
|---|---|---|
| allowed_rule | case/dataset manifest and runner over the canonical 234 TC denominator | `planned` |
| allowed_rule | raw case/journal and suite JSON/human report writers | `planned` |
| allowed_rule | all 11 mandatory checks, evidence-index/report builders and redaction/pairing self-tests | `planned` |
| allowed_rule | targeted tests and fixtures owned by this boundary may change with the owning behavior | `planned` |
| forbidden_rule | static pass, empty evidence, hidden filters, cross-run merge or `latest` aliases | `active` |
| forbidden_rule | final verdict, risk acceptance, signoff or business UoW changes | `active` |
| forbidden_rule | raw secret/body/full sensitive reference in output | `active` |
...
|---|---|---|---|
| `BATCH-11-A1` | Implement machine artifact and run-context schemas. | schema/version/path/identity tests | `planned` |
| `BATCH-11-A2` | Implement suite/case/journal writers and reports. | same-run raw-to-report pairing | `planned` |
| `BATCH-11-A3` | Implement all 11 mandatory checks. | negative fixtures and worst-status behavior | `planned` |
| `BATCH-11-A4` | Implement evidence-index/report derivation and redaction tests. | no-static/no-cross-run closure | `planned` |
...
| targeted behavior | manifest identity/denominator, raw/report pairing, failed/missing/cross-run/static/redaction fixtures and all check self-tests | `pending` |
| static boundary | exact 234/22/11/30 inventory; explicit roots; no `latest`, static pass, cross-run or secret output | `pending` |
...
|---|---|---|
| selector | all case/check/evidence schema fixtures, 234-case manifest and OBS-001~009 expands to exact formal TC identities at execution time | `pending` |
| positive/negative | manifest identity/denominator, raw/report pairing, failed/missing/cross-run/static/redaction fixtures and all check self-tests | `pending` |
...
| commit_message | planned subject is `feat(testing): add run-scoped test evidence tooling` with required groups `Raw artifacts and suite reports:` and `Checks and evidence derivation:` | `pending` |
...
| field | value |
|---|---|
| planned_commit_message | `feat(testing): add run-scoped test evidence tooling`; planned only |
...
| next boundary | must become `commit-11-b` only after project-ledger transition | `pending` |
...
|---|---|---|---|---|
| `BLK-L2T-COMMIT_11_A-ACTIVATION-001` | Activation | `planned` | Project ledger has not advanced through predecessor handoff. | `wait_until_current` |
| `L2T-UP-007` | conditional positive scope | `open_upstream` | Immutable source attribution must be bound before any evidence run is eligible. | `keep evidence invalid/not_created until a real baseline exists` |
...
|---|---|---|
| boundary-specific review | Machine schema, explicit roots, same-run pairing, redaction, no-static and failure retention fully apply; business truth and verdict do not. | Recheck exact formal/calibration sources before implementation. |

...
| phase | `PH-11` |
| predecessor | `commit-11-a` |
| next_boundary | `handoff` |
...
| item | contract |
|---|---|
| objective | Implement release-local smoke aggregation, fixed acceptance projections, VETO checks and review-required handoff drafts. |
| formal authority | formal 05 §§9/12~14; formal 06 §§3~14; formal 07 §§6~12 |
| primary selector | VETO-001~013, release-local-smoke, 30 slots, 39 AC, 13 VF and 24 EG directions |
| planned title | `feat(release): add acceptance handoff scaffolding` |
| required body groups | `Release-local aggregation:`; `VETO and review-required handoff:` |
| next boundary | `handoff` |

## Activation Guard

| rule | required observation | current observation | consequence |
|---|---|---|---|
| project ledger | `current_boundary = commit-11-b` | Project ledger has not advanced to this future boundary. | Remain planned; do not edit implementation files. |
| predecessor | real Commit and Handoff Gate for `commit-11-a` | `pending` | Inactive until predecessor is closed. |
...
| `design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md` | exact `commit-11-b` row and batches | `pending` | exact scope and batch authority |
| `design-calibration/07_implementation_plan_step_07_tests_acceptance_gates.md` | exact `commit-11-b` selector/gate row | `pending` | test and evidence direction |
...
| formal/calibration authority | formal 05 §§9/12~14; formal 06 §§3~14; formal 07 §§6~12; 05 release/evidence artifacts; 06 entry/exit/evidence/veto/risk/signoff artifacts; 07 Step 7/11/12 | `pending` | exact schema, flow, state and oracle source |
...
|---|---|---|
| allowed_rule | release-local-smoke aggregation over completed same-run denominator | `planned` |
| allowed_rule | 30 candidate-slot disposition, fixed projections, manifest/seal and VETO/open-issue generators | `planned` |
| allowed_rule | acceptance/risk/review handoff drafts explicitly marked `review_required` | `planned` |
| allowed_rule | targeted tests and fixtures owned by this boundary may change with the owning behavior | `planned` |
| forbidden_rule | final human/agent verdict, automatic risk acceptance, real signature or production readiness | `active` |
| forbidden_rule | missing denominator compensation, static/cross-run evidence or partial projection publish | `active` |
| forbidden_rule | business state mutation, owner closure inference or hidden external success | `active` |
...
|---|---|---|---|
| `BATCH-11-B1` | Implement release-local aggregation and eligibility evaluation. | lower-run/suite/check/slot compatibility | `planned` |
| `BATCH-11-B2` | Implement VETO, open-issue and handoff draft generators. | 39 AC/13 VF/24 EG mapping | `planned` |
| `BATCH-11-B3` | Implement fixed projection, manifest/seal and schema/path checks. | whole-projection and review-required closure | `planned` |
...
| targeted behavior | missing/non-pass lower inputs, slot disposition, manifest-last projection, seal mismatch, VETO, no default verdict/risk/signoff | `pending` |
| static boundary | drafts remain review_required; exact denominators; no partial projection, auto-acceptance or production claim | `pending` |
...
|---|---|---|
| selector | VETO-001~013, release-local-smoke, 30 slots, 39 AC, 13 VF and 24 EG directions expands to exact formal TC identities at execution time | `pending` |
| positive/negative | missing/non-pass lower inputs, slot disposition, manifest-last projection, seal mismatch, VETO, no default verdict/risk/signoff | `pending` |
...
| commit_message | planned subject is `feat(release): add acceptance handoff scaffolding` with required groups `Release-local aggregation:` and `VETO and review-required handoff:` | `pending` |
...
| field | value |
|---|---|
| planned_commit_message | `feat(release): add acceptance handoff scaffolding`; planned only |
...
| next boundary | must become `handoff` only after project-ledger transition | `pending` |
...
|---|---|---|---|---|
| `BLK-L2T-COMMIT_11_B-ACTIVATION-001` | Activation | `planned` | Project ledger has not advanced through predecessor handoff. | `wait_until_current` |
...
|---|---|---|
| boundary-specific review | Release aggregation, projection atomicity, VETO, review ownership and truthfulness apply; final verdict/risk acceptance/signoff remain human/authorized authority. | Recheck exact formal/calibration sources before implementation. |

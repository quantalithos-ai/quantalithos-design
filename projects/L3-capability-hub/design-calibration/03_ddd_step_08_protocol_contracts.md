...
> Fixed access-review reason controlled repair: 2026-08-09; the review-attachment effect is bound to the exact Step 6 §7.6.1 persisted literal/bytes and contracts-only audited-static factory; no request field, protocol, digest field, Port, state or canonical test identity changes
...
| construction / effect | exact rule |
|---|---|
| object calls | generate new review id,derive `AccessGovernanceSeparationMarker::Separated`,call `draft(...)` then `record(...)`;when current review exists call old `supersede(...)`;call identity `attach_review_fact(...)` with `ChangeReason::access_review_fact_recorded()`,whose exact persisted UTF-8 value is `capability-hub.change-reason/access-review-fact-recorded.v1` per Step 6 §7.6.1 |
...
| idempotency / audit | required;digest covers identity + context + risk summary,not the fixed system reason。Acceptance saves new fact / optional superseded fact / identity link,appends one`ReviewFactAttached`record / identity trace / `CapabilityIdentityChanged`capture,then runs affected-material propagation from the same exact `ChangeReason::access_review_fact_recorded()` bytes;result marker remains`Separated`;duplicate replays stored data and does not reconstruct the reason,create another fact or scan material |

The fixed reason is not an inbound DTO field and cannot be overridden by caller, route, config or adapter data. Fresh execution constructs it once through the contracts-owned infallible factory; every record/trace/material bridge copies its validated bytes unchanged. Stored-result or persisted-record replay reads the original serialized value and must not regenerate it from the current constant. A literal/namespace/version change is a persisted protocol compatibility change owned by Step 6/8/9/12/13/16 and cannot be introduced as an implementation-only refactor.

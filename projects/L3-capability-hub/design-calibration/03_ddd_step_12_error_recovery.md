...
| `RecordCapabilityAccessReviewFact` / `command_record_capability_access_review_fact_flow` | missing identity/context/risk -> `MissingRequiredField`;approval/vote/Policy/shared-rules body -> `BodyForbidden`;non-current/retired identity或invalid review state -> `PolicyRejected` | current-review link、owner/version不对称=`ConsistencyDefect`;repository failure=`PortFailure`;review/identity/material race=`OptimisticConflict` | caller提交body-free review fact并使用current identity；不得把review当approval；conflict重读current review link | stable rejection可replay但无review truth；accepted UoW失败全回滚；duplicate不生成review id |

...
|---|---|---|
| project ledger current_boundary equals commit-03-c | pending | Future boundary stays inactive until project ledger advances. |
...
| projects/L3-capability-hub/design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md | commit-03-c row, batches and scope | pending | Exact boundary contract. |
| projects/L3-capability-hub/design-calibration/07_implementation_plan_step_07_tests_acceptance_gates.md | commit-03-c gate row | pending | Exact selector and command contract. |
| projects/L3-capability-hub/design-calibration/07_implementation_plan_step_08_config_environment_dependencies.md | commit-03-c readiness row | pending | Exact prerequisite and unavailable behavior. |
...
| projects/L3-capability-hub/design-calibration/07_implementation_plan_step_11_commit_review_delivery.md | commit-03-c title/body/reviewer/handoff | pending | Planned message only. |
| formal upstream sources | formal 03 C01-C08 and Q01-Q06 flows; DDD Steps 7, 9, 11 and 13; formal 05 command/query/TX cuts; formal 07 sections 3, 6, 7, 8, 10, 11, 12 | pending | Exact schema and truth remain upstream-owned. |

## Allowed Scope

| type | path_or_rule | status |
|---|---|---|
| allowed_rule | identity/registry service inputs, same-UoW orchestration, repository fakes, stored result, minimal facade and focused tests | planned |
| allowed_rule | BATCH-03-C1 service inputs -> BATCH-03-C2 same-UoW service -> BATCH-03-C3 fake/repository parity -> BATCH-03-C4 facade/focused tests | planned |
...
| forbidden_rule | descriptor/relation/exposure families; query mutation; runtime allowlist; provider or marketplace behavior | active |
...
|---|---|---|
| BATCH-03-C1 service inputs -> BATCH-03-C2 same-UoW service -> BATCH-03-C3 fake/repository parity -> BATCH-03-C4 facade/focused tests | each batch is a 100-300 line locally verifiable increment | planned |
...
|---|---|
| boundary_readiness | Local API root, same-UoW stores, clock/ID, stored result and source dependency |
| primary_selector | CMD-001..008 |
| targeted_selector | QUERY-001..006; identity/registry contract rows; TX-001..006 |
| planned_commands | cargo fmt --check; cargo check --workspace; cargo test --workspace -- capability-identity-registry; run_pr_gate.sh --run-id <run_id> --artifact-root <root> --suite service-command-query,repository-transaction --selector CMD-001..008,QUERY-001..006; check_case_manifest.sh --selector CMD-001..008; check_artifact_report_pairing.sh --scope commit-03-c |
| gate_set | GATE-01, GATE-02, GATE-03, GATE-04, GATE-06, GATE-07 |
| raw_report_contract | artifacts/test/<run_id>/raw; reports/runs/<run_id>/suites/service-command-query/identity-registry/; reports/runs/<run_id>/suites/repository-transaction/ |
| evidence_contract | eight Command accepted/rejected/duplicate/race rows; targeted Query zero-write; stored result, same-UoW and trace/capture refs from one run |
| AC_VF_VETO_direction | AC-CH-001/002/006..011/023/025/029; VF-CH-001..003/007/009/010; VETO-CH-001..003/007/009/010; VETO-CH-P-008/009 |
| failure_return | Command acceptance, duplicate/race, Query no-write, same-UoW or stored-result mismatch returns `wait_design`; no partial service slice may be called complete. |
| execution_status | Planned contract only; no command, test, run, artifact, report or evidence instance has been created. |
| planned reviewer | application owner + identity/registry owners |
| planned title | feat(identity-registry): complete the identity and registry service slice |
| required body groups | Accepted service and transaction flow:; Repository and replay parity:; Entry facade and focused verification: |
| next boundary | commit-04-a |
...
| targeted behavior | QUERY-001..006; identity/registry contract rows; TX-001..006 | pending | No test result exists. |
| evidence/provenance | eight Command accepted/rejected/duplicate/race rows; targeted Query zero-write; stored result, same-UoW and trace/capture refs from one run | pending | Planned paths are not evidence. |
...
| commit_message_format | pending | Planned subject: feat(identity-registry): complete the identity and registry service slice |
| commit_body_group | pending | Required groups: Accepted service and transaction flow:; Repository and replay parity:; Entry facade and focused verification: |
...
|---|---|---|---|---|---|
| BLK-CH-03-C-ACTIVATION-001 | activation_gate | planned | Project ledger has not advanced to this future boundary. | Advance the project ledger through the predecessor handoff and set this boundary current. | wait_until_current |

...
|---|---|---|
| project ledger current_boundary equals commit-04-b | pending | Future boundary stays inactive until project ledger advances. |
...
| projects/L3-capability-hub/design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md | commit-04-b row, batches and scope | pending | Exact boundary contract. |
| projects/L3-capability-hub/design-calibration/07_implementation_plan_step_07_tests_acceptance_gates.md | commit-04-b gate row | pending | Exact selector and command contract. |
| projects/L3-capability-hub/design-calibration/07_implementation_plan_step_08_config_environment_dependencies.md | commit-04-b readiness row | pending | Exact prerequisite and unavailable behavior. |
...
| projects/L3-capability-hub/design-calibration/07_implementation_plan_step_11_commit_review_delivery.md | commit-04-b title/body/reviewer/handoff | pending | Planned message only. |
| formal upstream sources | formal 03 adapter Ports/binding; formal 04 external slots and failure; formal 05 adapter/config/redaction cuts; formal 07 sections 3, 6, 7, 8, 10, 11, 12 | pending | Exact schema and truth remain upstream-owned. |

## Allowed Scope

| type | path_or_rule | status |
|---|---|---|
| allowed_rule | descriptor resolver, controlled/disabled adapter, service/config binding and API seam | planned |
| allowed_rule | BATCH-04-B1 resolver -> BATCH-04-B2 controlled/disabled fake -> BATCH-04-B3 service/config binding -> BATCH-04-B4 API seam tests | planned |
...
| forbidden_rule | MCP/A2A/API execution; route/quota/cost/retry truth; external body; generic adapter; Configured fallback | active |
...
|---|---|---|
| BATCH-04-B1 resolver -> BATCH-04-B2 controlled/disabled fake -> BATCH-04-B3 service/config binding -> BATCH-04-B4 API seam tests | each batch is a 100-300 line locally verifiable increment | planned |
...
|---|---|
| boundary_readiness | externalSource/secret/document configured-fake-disabled material; P0 graph must be complete even if selected product is absent |
| primary_selector | CMD-009..012 |
| targeted_selector | QUERY-007..011; STATE-004..006; adapter BIND rows; FOUNDATION-017 |
| planned_commands | cargo fmt --check; cargo check --workspace; cargo test --workspace -- adapter-descriptor; run_pr_gate.sh --run-id <run_id> --artifact-root <root> --suite service-command-query,runtime-binding,configuration-strict --selector CMD-009..012,QUERY-007..011; check_config_catalog.sh --selector adapter; check_redaction.sh --scope adapter; check_dependency_boundary.sh --scope adapter |
| gate_set | GATE-01, GATE-02, GATE-03, GATE-05, GATE-07 |
| raw_report_contract | artifacts/test/<run_id>/raw; reports/runs/<run_id>/suites/service-command-query/descriptor/; reports/runs/<run_id>/suites/runtime-binding/; reports/runs/<run_id>/suites/configuration-strict/; reports/runs/<run_id>/checks/redaction/ |
| evidence_contract | typed descriptor/resolution outcome only; selected unavailable is blocked_dependency; no execution/body; no fallback |
| AC_VF_VETO_direction | AC-CH-003/012..014/026/031..032/035; VF-CH-004/011/012; VETO-CH-004/011/012; VETO-CH-P-005/007 |
| failure_return | Execution/provider route/quota/cost/body leakage or Configured fallback is a veto; selected product absence remains `blocked_dependency` and returns to handoff, not synthetic success. |
| execution_status | Planned contract only; no command, test, run, artifact, report or evidence instance has been created. |
| planned reviewer | adapter owner + config owner |
| planned title | feat(adapter): add the external adapter descriptor service seam |
| required body groups | Descriptor resolution adapters:; Service and configuration binding: |
| next boundary | commit-05-a |
...
| targeted behavior | QUERY-007..011; STATE-004..006; adapter BIND rows; FOUNDATION-017 | pending | No test result exists. |
| evidence/provenance | typed descriptor/resolution outcome only; selected unavailable is blocked_dependency; no execution/body; no fallback | pending | Planned paths are not evidence. |
...
| commit_message_format | pending | Planned subject: feat(adapter): add the external adapter descriptor service seam |
| commit_body_group | pending | Required groups: Descriptor resolution adapters:; Service and configuration binding: |
...
|---|---|---|---|---|---|
| BLK-CH-04-B-ACTIVATION-001 | activation_gate | planned | Project ledger has not advanced to this future boundary. | Advance the project ledger through the predecessor handoff and set this boundary current. | wait_until_current |

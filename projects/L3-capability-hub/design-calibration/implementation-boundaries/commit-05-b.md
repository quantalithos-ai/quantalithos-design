...
| phase | PH-05 |
| design_baseline | planned-after-commit-05-a |
...
| item | contract |
|---|---|
| objective | Complete controlled governance and method relation services with transaction parity. |
| formal authority | 07-实施计划.md §6.4 and exact Step 6/7/8/10/11 rows for commit-05-b |
| allowed scope | relation service inputs, UoW/repositories, controlled resolver/fake and service tests |
| forbidden scope | worker loop, approval mutation, external body and delivery truth |
| batch sequence | BATCH-05-B1 service inputs -> BATCH-05-B2 UoW/repositories -> BATCH-05-B3 controlled resolver/fake -> BATCH-05-B4 service tests |
| primary or targeted selectors | CMD-013..017; QUERY-012..014; INBOUND-001..002; STATE-007..008 |
| readiness prerequisite | relation stores, controlled resolver, I01/I02 source contracts and receipt precursor |
| gate set | GATE-01,GATE-02,GATE-03,GATE-04,GATE-05,GATE-06,GATE-07 |
| planned reviewer | application owner + relation owner |
| planned title | feat(relation-service): add controlled governance and method relation services |
| required body groups | Relation service and persistence:; Controlled seam parity: |
...
|---|---|---|
| project ledger current boundary equals commit-05-b | pending | Future boundary stays inactive until project ledger advances. |
...
| projects/L3-capability-hub/design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md | commit-05-b exact row and batches | pending | Exact boundary contract. |
| projects/L3-capability-hub/design-calibration/07_implementation_plan_step_07_tests_acceptance_gates.md | commit-05-b exact gate and evidence row | pending | Exact gate authority. |
| projects/L3-capability-hub/design-calibration/07_implementation_plan_step_08_config_environment_dependencies.md | commit-05-b readiness row | pending | Exact prerequisite behavior. |
...
| projects/L3-capability-hub/design-calibration/07_implementation_plan_step_11_commit_review_delivery.md | commit-05-b title/body/reviewer/handoff | pending | Planned message only. |
| formal upstream sources | formal 03 relation Ports/TX; formal 04 source bindings; formal 05 relation/inbound cuts | pending | Formal 03/04/05/06 remain schema and oracle authority. |

## Allowed Scope

| type | path_or_rule | status |
|---|---|---|
| allowed_rule | relation service inputs, UoW/repositories, controlled resolver/fake and service tests | planned |
| allowed_rule | BATCH-05-B1 service inputs -> BATCH-05-B2 UoW/repositories -> BATCH-05-B3 controlled resolver/fake -> BATCH-05-B4 service tests; each batch is a 100-300 line locally verifiable increment | planned |
...
| forbidden_rule | worker loop, approval mutation, external body and delivery truth | active |
...
|---|---|---|
| BATCH-05-B1 service inputs -> BATCH-05-B2 UoW/repositories -> BATCH-05-B3 controlled resolver/fake -> BATCH-05-B4 service tests | each batch is a 100-300 line locally verifiable increment | planned |
...
|---|---|
| primary_selector | CMD-013..017 |
| targeted_selector | QUERY-012..014; INBOUND-001..002; STATE-007..008; relation TX/idempotency targeted |
| planned_commands | cargo fmt --check; cargo check --workspace; cargo test --workspace -- governance-method-relation; run_pr_gate.sh --run-id <run_id> --artifact-root <root> --suite service-command-query,entry-inbound,repository-transaction --selector CMD-013..017,QUERY-012..014,INBOUND-001..002; check_artifact_report_pairing.sh --scope commit-05-b; check_redaction.sh --scope relation |
| gate_set | GATE-01,GATE-02,GATE-03,GATE-04,GATE-05,GATE-06,GATE-07 |
| raw_report_contract | suites/service-command-query/governance-relation/; suites/entry-inbound/; suites/repository-transaction/; relation report and receipt candidate raw |
| evidence_contract | accepted seam result, stored receipt precursor, typed ref and UoW order must be same-run; worker loop and external body are out of scope |
| AC_VF_VETO_direction | AC-CH-004,015..017,025,027,031..032; VF-CH-005,006,007,011; VETO-CH-005..007,011; VETO-CH-P-008/009 |
| failure_return | receipt/order/body/no-write failure blocks; worker implementation remains wait_until_current for commit-09-a |
...
|---|---|---|
| targeted selector | CMD-013..017; QUERY-012..014; INBOUND-001..002; STATE-007..008, expanded to exact TC/DS/EV identities at execution time | pending |
...
| commit_message_format | pending | Planned subject: feat(relation-service): add controlled governance and method relation services |
| commit_body_group | pending | Required groups: Relation service and persistence:; Controlled seam parity: |
...
| next_boundary | pending | Must match project ledger after real handoff: commit-06-a |
...
|---|---|---|---|---|---|
| BLK-CH-05-B-ACTIVATION-001 | activation_gate | planned | Project ledger has not advanced to this future boundary. | Advance the project ledger through the predecessor handoff and set this boundary current. | wait_until_current |

...
| item | contract |
|---|---|
| objective | Establish canonical typed reference resolution and sidecar parity. |
| formal authority | 07-实施计划.md §6.4 and exact Step 6/7/8/10/11 rows for commit-07-b |
| allowed scope | typed reference DTO/state, sidecar, resolver/fake and reference symmetry tests |
| forbidden scope | external body, downstream truth, string kind guessing and non-core dependency |
| batch sequence | BATCH-07-B1 typed ref DTO -> BATCH-07-B2 resolution state -> BATCH-07-B3 resolver/fake -> BATCH-07-B4 symmetry tests |
| primary or targeted selectors | CMD-024..026; QUERY-029..033; INBOUND-004..006 |
| readiness prerequisite | reference store/resolver/sidecar and I04-I06 fixtures |
| gate set | GATE-01,GATE-02,GATE-03,GATE-04,GATE-05,GATE-07 |
| planned reviewer | reference owner + dependency reviewer |
| planned title | feat(reference): add canonical capability reference resolution |
| required body groups | Canonical reference contracts:; Reference resolution parity: |
| next boundary | commit-08-a |

## Activation Guard

| rule | status | consequence |
|---|---|---|
| project ledger current boundary equals commit-07-b | pending | Future boundary stays inactive until project ledger advances. |
...
| projects/L3-capability-hub/design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md | commit-07-b exact row and batches | pending | Exact boundary contract. |
| projects/L3-capability-hub/design-calibration/07_implementation_plan_step_07_tests_acceptance_gates.md | commit-07-b exact gate and evidence row | pending | Exact gate authority. |
| projects/L3-capability-hub/design-calibration/07_implementation_plan_step_08_config_environment_dependencies.md | commit-07-b readiness row | pending | Exact prerequisite behavior. |
...
| projects/L3-capability-hub/design-calibration/07_implementation_plan_step_11_commit_review_delivery.md | commit-07-b title/body/reviewer/handoff | pending | Planned message only. |
| formal upstream sources | formal 03 C24-C26/Q29-Q33/I04-I06; reference state and resolver sources | pending | Formal 03/04/05/06 remain schema and oracle authority. |

## Allowed Scope

| type | path_or_rule | status |
|---|---|---|
| allowed_rule | typed reference DTO/state, sidecar, resolver/fake and reference symmetry tests | planned |
| allowed_rule | BATCH-07-B1 typed ref DTO -> BATCH-07-B2 resolution state -> BATCH-07-B3 resolver/fake -> BATCH-07-B4 symmetry tests; each batch is a 100-300 line locally verifiable increment | planned |
...
| forbidden_rule | external body, downstream truth, string kind guessing and non-core dependency | active |
...
|---|---|---|
| BATCH-07-B1 typed ref DTO -> BATCH-07-B2 resolution state -> BATCH-07-B3 resolver/fake -> BATCH-07-B4 symmetry tests | each batch is a 100-300 line locally verifiable increment | planned |
...
|---|---|
| primary_selector | CMD-024..026 |
| targeted_selector | QUERY-029..033; INBOUND-004..006; reference STATE/TX/BIND rows |
| planned_commands | cargo fmt --check; cargo check --workspace; cargo test --workspace -- canonical-reference; run_pr_gate.sh --run-id <run_id> --artifact-root <root> --suite service-command-query,entry-inbound,repository-transaction --selector CMD-024..026,QUERY-029..033,INBOUND-004..006; check_case_manifest.sh --selector CMD-024..026; check_responsibility_boundary.sh --scope reference; check_redaction.sh --scope reference; check_dependency_boundary.sh --scope reference |
| gate_set | GATE-01,GATE-02,GATE-03,GATE-04,GATE-05,GATE-07 |
| raw_report_contract | suites/service-command-query/reference/; suites/entry-inbound/; suites/repository-transaction/; checks/; reference reports |
| evidence_contract | typed reference must preserve subject, kind, digest, reason, version and missing/degraded/invalid semantics; no string kind inference, body or downstream truth |
| AC_VF_VETO_direction | AC-CH-013,017,019,021,025,026,029..032,036; VF-CH-006,007,009,011,012; VETO-CH-006,007,009,011,012; VETO-CH-P-005/008/009 |
| failure_return | kind/source/sidecar/resolver signature gap is wait_design; body or non-core dependency is VETO |
...
|---|---|---|
| targeted selector | CMD-024..026; QUERY-029..033; INBOUND-004..006, expanded to exact TC/DS/EV identities at execution time | pending |
...
| commit_message_format | pending | Planned subject: feat(reference): add canonical capability reference resolution |
| commit_body_group | pending | Required groups: Canonical reference contracts:; Reference resolution parity: |
...
| next_boundary | pending | Must match project ledger after real handoff: commit-08-a |
...
|---|---|---|---|---|---|
| BLK-CH-07-B-ACTIVATION-001 | activation_gate | planned | Project ledger has not advanced to this future boundary. | Advance the project ledger through the predecessor handoff and set this boundary current. | wait_until_current |

...
| phase | PH-10 |
| design_baseline | planned-after-commit-10-b |
...
| item | contract |
|---|---|
| objective | Complete J08 recovery, replay and terminalization behavior. |
| formal authority | 07-实施计划.md §6.4 and exact Step 6/7/8/10/11 rows for commit-10-c |
| allowed scope | J08 recovery/capture binding, stored report, replay and terminalization tests |
| forbidden scope | release verdict, blind retry, recursive entry and core mutation replay |
| batch sequence | BATCH-10-C1 J08 input -> BATCH-10-C2 recovery/capture -> BATCH-10-C3 duplicate/reentry -> BATCH-10-C4 terminal tests |
| primary or targeted selectors | JOB-008 |
| readiness prerequisite | collaboration recovery intent, capture and commit-resolution fixtures |
| gate set | GATE-01,GATE-02,GATE-03,GATE-04,GATE-05,GATE-06,GATE-07 |
| planned reviewer | jobs owner + transaction reviewer |
| planned title | feat(jobs-recovery): add event recovery and replay jobs |
| required body groups | Event recovery flow:; Replay and terminalization: |
...
|---|---|---|
| project ledger current boundary equals commit-10-c | pending | Future boundary stays inactive until project ledger advances. |
...
| projects/L3-capability-hub/design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md | commit-10-c exact row and batches | pending | Exact boundary contract. |
| projects/L3-capability-hub/design-calibration/07_implementation_plan_step_07_tests_acceptance_gates.md | commit-10-c exact gate and evidence row | pending | Exact gate authority. |
| projects/L3-capability-hub/design-calibration/07_implementation_plan_step_08_config_environment_dependencies.md | commit-10-c readiness row | pending | Exact prerequisite behavior. |
...
| projects/L3-capability-hub/design-calibration/07_implementation_plan_step_11_commit_review_delivery.md | commit-10-c title/body/reviewer/handoff | pending | Planned message only. |
| formal upstream sources | formal 03 J08/replay/TX/error recovery; formal 05 recovery/idempotency | pending | Formal 03/04/05/06 remain schema and oracle authority. |

## Allowed Scope

| type | path_or_rule | status |
|---|---|---|
| allowed_rule | J08 recovery/capture binding, stored report, replay and terminalization tests | planned |
| allowed_rule | BATCH-10-C1 J08 input -> BATCH-10-C2 recovery/capture -> BATCH-10-C3 duplicate/reentry -> BATCH-10-C4 terminal tests; each batch is a 100-300 line locally verifiable increment | planned |
...
| forbidden_rule | release verdict, blind retry, recursive entry and core mutation replay | active |
...
|---|---|---|
| BATCH-10-C1 J08 input -> BATCH-10-C2 recovery/capture -> BATCH-10-C3 duplicate/reentry -> BATCH-10-C4 terminal tests | each batch is a 100-300 line locally verifiable increment | planned |
...
|---|---|
| primary_selector | JOB-008 |
| targeted_selector | OUTBOUND-001..010; TX-017..022; recovery/config/idempotency rows |
| planned_commands | cargo fmt --check; cargo check --workspace; cargo test --workspace -- collaboration-recovery-job; run_main_gate.sh --run-id <run_id> --artifact-root <root> --entry jobs --selector JOB-008; check_case_manifest.sh --selector JOB-008; check_artifact_report_pairing.sh --scope commit-10-c; check_redaction.sh --scope job-recovery; check_responsibility_boundary.sh --scope job-no-repair |
| gate_set | GATE-01,GATE-02,GATE-03,GATE-04,GATE-05,GATE-06,GATE-07 |
| raw_report_contract | suites/jobs-lifecycle/recovery/; suites/repository-transaction/jobs/; suites/outbound-collaboration/; recovery/replay reports |
| evidence_contract | duplicate/reentry uses stored plan/journal/report; commit-unknown remains typed; repair handles collaboration intent only, never core mutation or recursive entry |
| AC_VF_VETO_direction | AC-CH-011,018,021,024,025,030,033..037; VF-CH-007,009,010,011; VETO-CH-007,009,010,011; VETO-CH-P-004/005/008/009 |
| failure_return | blind retry, recursive entry, commit-unknown pseudo-success, asymmetric capture or body leak blocks; no release verdict is generated |
...
|---|---|---|
| targeted selector | JOB-008, expanded to exact TC/DS/EV identities at execution time | pending |
...
| commit_message_format | pending | Planned subject: feat(jobs-recovery): add event recovery and replay jobs |
| commit_body_group | pending | Required groups: Event recovery flow:; Replay and terminalization: |
...
| next_boundary | pending | Must match project ledger after real handoff: commit-11-a |
...
|---|---|---|---|---|---|
| BLK-CH-10-C-ACTIVATION-001 | activation_gate | planned | Project ledger has not advanced to this future boundary. | Advance the project ledger through the predecessor handoff and set this boundary current. | wait_until_current |

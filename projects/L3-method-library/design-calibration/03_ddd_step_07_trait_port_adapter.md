...
| facade input | `MethodAssetDistributionHandoffCommandDispatchInput { command_shell, command_source, seam_source: Option<MethodAssetDistributionHandoffSeamSource>, api_entry_context_ref, application_dispatch_ref }`。 |
...
| command source | `MethodAssetDistributionHandoffCommandSource` with exact variants/fields from Step 6 `3B.4A`: `PrepareDistributionRef`;`AdjustDistributionContext`;`MarkDistributionAvailability`. `degraded_decision_ref` is not a source field and only comes from the mapper port. |
| seam source | Optional `MethodAssetDistributionHandoffSeamSource` from Step 6 `3B.4A` is the only source for required slots, target scope, publisher/handoff bindings and boundary markers. `None` disables post-commit publication/handoff for that command. |
| replay/support factory | `MethodAssetDistributionHandoffReplayEnvelopeFactoryInput`;`MethodAssetDistributionHandoffReplayEnvelope`;`MethodAssetDistributionHandoffSupportRefFactory`, including exact `new_distribution_ref(...)`, `new_event_candidate_assembly_ref(...)`, `new_publication_outcome_ref(...)` and `new_handoff_marker_ref(...)` methods from Step 6 `3B.4A`. |
...
The exact service input schemas are closed as follows. Each field is copied without parsing or inference;`seam_source` is canonicalized into the duplicate digest exactly as supplied.

| input | exact fields |
|---|---|
| `PrepareMethodAssetDistributionRefInput` | replay fields `operation_context_ref`, `idempotency_key_ref`, `operation_digest_ref`, `dedup_scope_ref`;source fields `relation_ref`, `requested_distribution_ref`, `distribution_context_ref`, `consumption_context_ref`, `boundary_ref`, `availability_marker`, `candidate_reason_ref`;`seam_source`。 |
| `AdjustMethodAssetDistributionContextInput` | replay fields;source fields `relation_ref`, `distribution_ref`, `previous_context_ref`, `new_context_ref`, `adjustment_reason_ref`, `candidate_reason_ref`, `expected_distribution_version`;`seam_source`。 |
| `MarkMethodAssetDistributionAvailabilityInput` | replay fields;source fields `relation_ref`, `distribution_ref`, `distribution_context_ref`, `availability_marker`, `candidate_reason_ref`;`seam_source`。Mapper output is not part of this input. |

`MethodAssetDistributionHandoffReplayEnvelopeFactoryInput` fields are exactly `command_shell`, `command_source`, `seam_source`, `selector`, `api_entry_context_ref`, `application_dispatch_ref`. `MethodAssetDistributionHandoffReplayEnvelope` fields are exactly `operation_context_ref`, `idempotency_key_ref`, `operation_digest_ref`, `dedup_scope_ref`. Replay-envelope build failures reuse the exact application-owned `MethodAssetReplayEnvelopeBuildError`;no new repository error variant or fake-only error is authorized.

### 3. Port and repository callable surface

| family | current-boundary callable surface | closure rule |
|---|---|---|
| `MethodAssetRelationRepository` | `get_relation_anchor_with_version(relation_ref: MethodAssetRelationRef) -> Result<Option<Versioned<MethodAssetRelationReadAnchor>>, MethodAssetRepositoryError>`。 | Only current-boundary relation method. Full relation load, list/page, save, scan and lifecycle methods are carved out. |
| `MethodAssetDistributionRepository` | `get_distribution_with_version(distribution_ref: MethodAssetDistributionRef) -> Result<Option<Versioned<MethodAssetDistributionRecord>>, MethodAssetRepositoryError>`;`save_distribution(record: MethodAssetDistributionRecord, expected_version: Option<MethodAssetExpectedVersion>, uow: &mut dyn CommandUnitOfWork) -> Result<VersionedRef<MethodAssetDistributionRef>, MethodAssetRepositoryError>`。 | Application-owned body-free record. Prepare creates;adjust/mark update by exact optimistic version. No material body, relation save or runtime truth. |
| `DistributionReadMaterialBuilderPort` | `build_distribution_read_material(input: DistributionReadMaterialBuilderInput) -> DistributionReadMaterialBuildOutcome`。 | Exact body-free input is Step 6 `3B.4A`;no durable material store or package/listing body. |
| `MethodAssetConsumptionAvailabilityResolverPort` | `resolve_distribution_availability(marker: MethodAssetConsumptionAvailabilityMarker) -> MethodAssetConsumptionAvailabilityMarker`。 | Copy-only validation;no runtime/config/error/fake inference. |
| `MethodAssetDegradedDecisionMapperPort` | `map_degraded_decision(marker: MethodAssetConsumptionAvailabilityMarker, diagnostic_ref: Option<MethodAssetInfraSafeDiagnosticRef>) -> Option<MethodAssetDegradedDecisionRef>`。 | Only mapper output may create the degraded decision ref;`Ready` returns `None`. |
| `MethodAssetAdapterAvailabilityPort` | `check_required_distribution_slots(required_slot_refs: MethodAssetAdapterSlotRefSet) -> MethodAssetAdapterAvailabilitySummary`。 | Slot availability only;not config source, health body, material state or runtime truth. `Disabled` must emit its own `MethodAssetInfraSafeDiagnosticRef`;application/service/fake code may copy but may not synthesize it. |
| `MethodAssetCollaborationTargetRegistryPort` | `resolve_distribution_targets(scope_ref: MethodAssetTargetRegistryScopeRef, publisher_binding_ref: MethodAssetPublisherBindingStateRef, handoff_binding_ref: Option<MethodAssetHandoffBindingStateRef>) -> MethodAssetCollaborationTargetSummary`。 | Arguments copy the seam source;publisher binding is required, handoff binding is optional;every branch returns the body-free target set and `Disabled` must emit its own `MethodAssetInfraSafeDiagnosticRef`;no config/topic/URL/secret/target body lookup or local diagnostic synthesis. |
| `MethodAssetEventCandidatePublisherPort` | `publish_event_candidate(input: MethodAssetEventCandidatePublicationInput) -> MethodAssetPublicationOutcome`。 | Input uses a reloaded candidate value and factory-issued publication ref;no current-truth reread or local mint. |
| `MethodAssetCollaborationHandoffPort` | `prepare_distribution_handoff(input: MethodAssetDistributionHandoffInput) -> MethodAssetHandoffOutcome`。 | Exact body-free input is Step 6 `3B.4A`;no report/archive/package/external receipt body. |
| `MethodAssetEventCandidateAssemblyRepository` | `append_event_candidate_assembly(assembly: MethodAssetDistributionEventCandidateAssembly, uow: &mut dyn CommandUnitOfWork) -> Result<VersionedRef<MethodAssetEventCandidateAssemblyRef>, MethodAssetRepositoryError>`;`get_event_candidate_assembly(assembly_ref: MethodAssetEventCandidateAssemblyRef) -> Result<Option<Versioned<MethodAssetDistributionEventCandidateAssembly>>, MethodAssetRepositoryError>`。 | Current-boundary candidate value is Step 6 `3B.4A`;the generic future candidate object is carved out. Append-only reload authority. |
| `MethodAssetPublicationOutcomeRepository` | `save_publication_outcome(outcome: MethodAssetPublicationOutcome, uow: &mut dyn CommandUnitOfWork) -> Result<VersionedRef<MethodAssetPublicationOutcomeRef>, MethodAssetRepositoryError>`;`get_publication_outcome(publication_ref: MethodAssetPublicationOutcomeRef) -> Result<Option<Versioned<MethodAssetPublicationOutcome>>, MethodAssetRepositoryError>`。 | Stores refs/markers only and stays separate from accepted truth UoW. |
| `MethodAssetHandoffMarkerRepository` | `save_handoff_marker(outcome: MethodAssetHandoffOutcome, uow: &mut dyn CommandUnitOfWork) -> Result<VersionedRef<MethodAssetHandoffMarkerRef>, MethodAssetRepositoryError>`;`get_handoff_marker(handoff_ref: MethodAssetHandoffMarkerRef) -> Result<Option<Versioned<MethodAssetHandoffOutcome>>, MethodAssetRepositoryError>`。 | Stores body-free marker outcome only. |
...
Disabled/degraded/unavailable precheck mapping is owned by the application orchestration contract, not by fake-private behavior. The service checks adapter slots, then resolves the body-free target summary once to obtain the exact target set required by the publication ref factory. Adapter `Degraded -> Blocked` and `Unavailable -> Unavailable` copy `marker_ref` as the outcome reason plus the adapter diagnostic;adapter `Disabled -> Blocked` copies its explicit reason and diagnostic. If adapter is `Available`, target `Disabled | Blocked -> Blocked` and target `Unavailable -> Unavailable` copy the target port's reason and diagnostic. These branches mint and persist a safe publication outcome without calling publisher or handoff. Only adapter `Available` plus target `Enabled` may invoke publisher, and only `Published` may invoke handoff.

### 5. Fake / durable parity gate

| fake family | parity requirement |
|---|---|
| relation/distribution repository fake | relation anchor versioned read only;distribution record create/update expected-version conflict, staged UoW visibility and rollback invisibility. |

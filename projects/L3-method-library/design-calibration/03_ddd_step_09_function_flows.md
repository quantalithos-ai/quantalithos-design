...
| `RetireFormalMethodAssetVersionFlow` / `MethodAssetCommandService.retire_formal_version` | Command shell -> formal version retirement | formal version repository;UoW / stored result | load formal version;check exact state guard;mark retired;save | missing / stale / already-retired rejected safely | retired event candidate | consumption material / impact precheck is deferred to later boundaries |

### 3A. `commit-04-b` exact formalization/version command overlay

本节把上表 6 条 PH-04 command flow 收口到 `commit-04-b` 可直接落码的 exact sequence。所有分支都必须先完成 selector/source match、replay envelope build 和 duplicate stored-result lookup;在此之前不得开始 UoW mutation。

| Flow | Exact function-level sequence | Exact callable surface | Safe rejection / stop rule |
|---|---|---|---|
| `EvaluateMethodAssetFormalizationEligibilityFlow` | selector/source match -> build replay envelope -> `find_command_result_by_idempotency(...)` -> load definition/catalog -> exact read each `basis_summary_ref` -> `find_formalization_state_by_definition_catalog(...)` -> `resolve_formalization_basis(...)` -> `build_formalization_eligibility_diagnostic(...)` -> create/update `FormalizationState` with `AssessmentPending` / `Eligible` / `Ineligible` -> `save_formalization_state(...)` -> save stored result -> `commit()` -> on `CommitUnknown` read back stored result first,then formalization state by ref/current lookup。 | `MethodAssetFormalizationVersionSupportRefFactory`;`MethodAssetDefinitionRepository`;`MethodAssetCatalogEntryRepository`;`FormalizationBasisSummaryRepository`;`FormalizationStateRepository`;`FormalizationBasisResolverPort`;`MethodAssetPolicyDiagnosticBuilderPort`;stored-result repository。 | missing definition/catalog/basis/rule,current state version conflict,source mismatch or resolver/diagnostic unavailability returns safe rejected/conflict/manual surface;the flow must not create `FormalMethodAssetVersion`。 |
| `InitiateMethodAssetFormalizationFlow` | selector/source match -> replay envelope -> duplicate lookup -> load definition/catalog -> current-state lookup -> exact read each basis summary -> assert explicit `trigger_marker_ref` -> resolve basis -> create new pending state if absent or update existing state to `AssessmentPending` with new `FormalizationStateReasonSummary` -> `save_formalization_state(...)` -> save stored result -> `commit()` -> `CommitUnknown` read back stored result,then current state lookup。 | same as evaluate flow plus `new_formalization_state_ref(...)` when current-state lookup is absent。 | unsupported implicit trigger,missing basis summary,wrong definition/catalog pairing or current state already `VersionEstablished` returns safe rejected result;the flow must not mint formal version ref。 |
| `EstablishFormalMethodAssetVersionFlow` | selector/source match -> replay envelope -> duplicate lookup -> `get_formalization_state_with_version(...)` -> load definition/catalog -> assert loaded state `state_kind == Eligible` -> assert boundary summary matches loaded state / definition / catalog / basis refs -> `find_current_formal_method_asset_version(...)` must return `None` -> mint new version ref -> `FormalMethodAssetVersion.from_formalization_state(...)` -> `FormalizationState.mark_formalized(...)` -> `save_formal_method_asset_version(...)` and `save_formalization_state(...)` in same UoW -> save stored result -> `commit()` -> `CommitUnknown` read back stored result,then exact version/state reads。 | `FormalizationStateRepository`;`FormalMethodAssetVersionRepository`;definition/catalog repositories;support ref factory;stored-result repository。 | non-eligible state,boundary summary mismatch,current version already exists,missing loaded state or version conflict returns safe rejected/conflict/manual surface;no version may be derived from latest timestamp,publish flag,fingerprint or snapshot。 |
| `RecordFormalVersionSemanticChangeFlow` | selector/source match -> replay envelope -> duplicate lookup -> `get_formal_method_asset_version_with_version(...)` -> exact read each basis summary ref from source -> `build_formal_version_change_diagnostic(...)` -> apply accepted semantic-change marker without rewriting `formal_version_ref` or changing `version_state` away from `Active` -> `save_formal_method_asset_version(...)` -> save stored result -> `commit()` -> `CommitUnknown` read back stored result,then exact version read。 | `FormalMethodAssetVersionRepository`;`FormalizationBasisSummaryRepository`;`MethodAssetPolicyDiagnosticBuilderPort`;stored-result repository。 | missing basis summary,blocking diagnostic,retired/superseded version or version conflict returns safe rejected/conflict surface;the flow must not overwrite old ref meaning or silently create a next version。 |
| `SupersedeFormalMethodAssetVersionFlow` | selector/source match -> replay envelope -> duplicate lookup -> load previous version with version -> load next version with version -> assert same definition lineage and `next_formal_version_ref != previous_formal_version_ref` -> assert previous is not `Retired` and next already carries explicit successor semantics -> call `previous.supersede_with(next_formal_version_ref)` -> `save_formal_method_asset_version(previous, expected_version, uow)` -> save stored result -> `commit()` -> `CommitUnknown` read back stored result,then exact previous/next version reads。 | `FormalMethodAssetVersionRepository`;stored-result repository。 | missing previous/next,definition mismatch,self-supersession,previous retired or version conflict returns safe rejected/conflict/manual surface;the flow must not rewrite the next ref or delete previous history。 |
| `RetireFormalMethodAssetVersionFlow` | selector/source match -> replay envelope -> duplicate lookup -> `get_formal_method_asset_version_with_version(...)` -> require loaded `version_state != Retired` -> `mark_retired(retirement_marker_ref)` / persist `version_state = Retired` -> `save_formal_method_asset_version(...)` -> save stored result -> `commit()` -> `CommitUnknown` read back stored result,then exact version read。 | `FormalMethodAssetVersionRepository`;stored-result repository。 | missing version,version conflict or forbidden transition from `Retired` returns safe rejected/conflict/manual surface;the flow must not inspect future consumption/impact owners,delete formal version truth or synthesize precheck state from scans / fake maps。 |

Current-boundary replay / commit rules:

- Same `(idempotency_key_ref,dedup_scope_ref,operation_digest_ref)` returns previously stored safe result and must not rerun mutation.
- Same key/scope with different digest is a stored conflict/rejected branch,not a new mutation attempt.
- `CommitUnknown` may only read back through the formal stored-result surface and the exact versioned repositories named above;it must not infer outcome from timeout,log line,current truth alone or fake private state.
...
| `RetireFormalMethodAssetVersionFlow` | reserve -> load version -> assert current state is not `Retired` -> mark retired with reason -> save expected version -> stored result | formal version repository | missing / stale / already-retired safe rejected | no version-history deletion;consumption/impact precheck deferred to later boundaries |
...
|---|---|---|---|
| `PrepareMethodAssetDistributionRefFlow` | `PrepareMethodAssetDistributionRefInput` | 1. replay lookup/envelope;2. load relation anchor;3. create/copy distribution ref;4. reject existing record, otherwise build record;5. call builder;6. when seam source exists mint/append candidate;7. save record, optional candidate and stored result in one UoW;8. commit;9. optional post-commit seam processing. | Duplicate replay no-rerun. Relation/context/builder/wrong-kind/seam/body failures reject safely. `seam_source = None` creates no candidate. No durable read material/package/full relation lifecycle. |
| `AdjustMethodAssetDistributionContextFlow` | `AdjustMethodAssetDistributionContextInput` | 1. replay lookup/envelope;2. load relation anchor and distribution record;3. require relation/version/previous-context match;4. apply new context;5. when seam source exists mint candidate;6. save record, optional candidate and stored result in one UoW;7. commit;8. optional post-commit seam processing. | Version/relation/context/scope mismatch safely rejects. `seam_source = None` creates no candidate. No relation save/authorization expansion/runtime state. |
| `MarkMethodAssetDistributionAvailabilityFlow` | `MarkMethodAssetDistributionAvailabilityInput` | 1. replay lookup/envelope;2. load relation anchor and distribution record;3. validate relation/context;4. resolve/copy marker and degraded decision;5. apply marker;6. when seam source exists mint candidate;7. save record by loaded version, optional candidate and stored result in one UoW;8. commit;9. optional post-commit seam processing. | Missing marker/source/mapper output safely rejects. `degraded_decision_ref` is never input. `seam_source = None` creates no candidate. Unavailable/constrained is not delivery/runtime truth. |

### 3. Publication / handoff flow exact overlay

| flow | reload source | exact sequence | branch / replay / side effect rule |
|---|---|---|---|
| `MethodAssetDistributionRefChangedPublicationFlow` | `event_candidate_assemblies.assembly_ref` plus copied `MethodAssetDistributionHandoffSeamSource` | 1. reload exact Step 6 `3B.4A` candidate shell;2. check `required_slot_refs`;3. resolve targets once from exact scope/binding refs to obtain the formal target set;4. apply the exact adapter-first precheck mapping;5. for a non-available adapter or non-enabled target, factory-mint/save `Blocked` or `Unavailable` publication outcome and stop;6. only for adapter `Available` plus target `Enabled`, factory-mint publication ref and call publisher;7. save publication outcome;8. only after `Published`, for each enabled handoff target factory-mint handoff ref, call handoff and save marker outcome. | Adapter `Degraded -> Blocked` and `Unavailable -> Unavailable` copy `marker_ref` plus diagnostic;adapter `Disabled -> Blocked` copies explicit reason plus diagnostic. With adapter `Available`, target `Disabled | Blocked -> Blocked` and `Unavailable -> Unavailable` copy target reason/diagnostic. Safe precheck outcomes call neither publisher nor handoff, never derive diagnostics/refs locally and never roll back accepted command truth. |
| `MethodAssetDistributionAvailabilityChangedPublicationFlow` | reloaded current-boundary candidate shell with copied availability marker plus seam source | 1. verify copied marker and mapper requirement;2. check required slots;3. resolve targets once to obtain the formal target set;4. apply the same adapter-first safe-outcome mapping;5. non-available/disabled/blocked branches mint/save publication outcome without publisher/handoff;6. only the available+enabled branch calls publisher and saves its outcome;7. only `Published` plus matched handoff binding/marker may call/save handoff outcome. | Missing marker/mapper/source stops safely. Port-owned diagnostic is mandatory for every disabled/degraded/unavailable mapping and cannot come from reason text, config, runtime status, exception or fake map. Outcome never stores downstream sync state, subscriber ack, topic, delivery receipt, package body, archive body or report body. |

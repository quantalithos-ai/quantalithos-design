...
|---|---|---|---|
| `MethodAssetDistributionRefChangedPublicationFlow` | `event_candidate_assemblies.assembly_ref` plus copied `MethodAssetDistributionHandoffSeamSource` | 1. reload exact Step 6 `3B.4A` candidate shell;2. check `required_slot_refs`;3. resolve targets from exact scope/binding refs;4. factory-mint publication ref and call publisher with `MethodAssetEventCandidatePublicationInput`;5. save publication outcome;6. for each enabled handoff target, factory-mint handoff ref, call handoff port with `MethodAssetDistributionHandoffInput` and save marker outcome. | Missing/invalid seam source, disabled/blocked/unavailable target, publisher failure or handoff failure records safe outcome/rejection only. It never rereads current truth, derives refs from config/runtime text or rolls back accepted command result. |
| `MethodAssetDistributionAvailabilityChangedPublicationFlow` | reloaded current-boundary candidate shell with copied availability marker plus seam source | 1. verify copied marker;2. check required slots;3. resolve targets;4. mint/call/save publication outcome;5. when handoff binding/marker exists, mint/call/save handoff outcome. | Missing marker or required mapper/source closure stops safely. Outcome never stores downstream sync state, subscriber ack, topic, delivery receipt, package body, archive body or report body. |

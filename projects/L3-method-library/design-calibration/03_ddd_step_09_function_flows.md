...
| `RetireFormalMethodAssetVersionFlow` | selector/source match -> replay envelope -> duplicate lookup -> `get_formal_method_asset_version_with_version(...)` -> `find_consumption_material_refs_by_formal_version(...)` -> `find_pending_impact_summary_refs_by_formal_version(...)` -> require traceable safe refs only -> `mark_retired(retirement_marker_ref)` / persist `version_state = Retired` -> `save_formal_method_asset_version(...)` -> save stored result -> `commit()` -> `CommitUnknown` read back stored result,then exact version read and retirement precheck helper reads。 | `FormalMethodAssetVersionRepository`;`MethodAssetConsumptionMaterialRepository`;`ConsumptionImpactSummaryRepository`;stored-result repository。 | missing version,version conflict,untraceable material/impact helper output or forbidden transition from `Retired` returns safe rejected/conflict/manual surface;the flow must not delete formal version truth,consumption history,trace,audit or impact summaries。 |

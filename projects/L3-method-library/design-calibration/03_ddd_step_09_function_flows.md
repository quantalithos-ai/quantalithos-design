...
| `AdjustMethodAssetDefinitionFlow` / `MethodAssetCommandService.adjust_definition` | Command shell -> existing `MethodAssetDefinition` | definition repository;basis / external summary repository;policy diagnostic builder;UoW / stored result | load definition with version;load basis ref;apply adjustment;save with expected version | missing / stale version rejected;duplicate replay | definition history;definition changed event candidate;audit candidate | stale expected version maps to safe rejection |
| `RetireMethodAssetDefinitionFlow` / `MethodAssetCommandService.retire_definition` | Command shell -> `MethodAssetDefinition` retirement | definition repository;formal version repository;trace / audit family;UoW / stored result | load definition;check existing formal version refs remain traceable;mark retired;save | active formal version conflict becomes rejected unless design later allows reasoned retirement | retired history;definition changed event candidate | retirement does not delete formal version refs |
...
|---|---|---|---|---|
| `EstablishMethodAssetDefinitionFlow` | entry validates definition summary and optional catalog hint -> reserve idempotency -> load accepted external summary refs if present -> create `MethodAssetDefinition` -> UoW save definition -> append definition history / event candidate / stored result -> commit | `MethodAssetDefinition`;definition repository;external summary repository;stored result;UoW | duplicate returns stored accepted surface;invalid external ref rejected body-free | no `MethodContent`;no raw external body;definition ref from domain creation only |
| `AdjustMethodAssetDefinitionFlow` | reserve -> load definition with version -> load basis/external summary refs -> apply adjustment summary -> save with expected version -> record history/effect/stored result | definition repository;basis/external support;policy diagnostic builder | stale version safe rejected;missing definition safe rejected;duplicate replay | no direct formal version mutation;no raw audit log |
| `RetireMethodAssetDefinitionFlow` | reserve -> load definition -> inspect linked formal version refs through version repository family -> domain retire with reason -> save expected version -> stored result/effect | definition repository;formal version repository;trace/audit family | active formal version conflict safe rejected unless later state matrix permits | retirement cannot delete formal version or catalog history |
| `RegisterMethodAssetCatalogEntryFlow` | reserve -> load definition -> check catalog scope lookup -> create catalog entry -> save -> event candidate/stored result | catalog repository;definition repository;IdGenerator;UoW | definition missing rejected;duplicate replay | catalog view/search index cannot be truth source |
...
| `RetireMethodAssetCatalogEntryFlow` | reserve -> load catalog entry -> mark retired with safe reason -> save expected version -> stored result/effect | catalog repository;definition repository optional | missing entry rejected;duplicate replay | retiring catalog does not retire definition |

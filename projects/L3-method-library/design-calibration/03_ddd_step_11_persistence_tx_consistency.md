...
| fresh semantic change / supersede / retire accepted | begin command UoW -> duplicate lookup -> read required version(s) -> apply current-boundary transition -> save mutated version truth -> save command result -> commit | rollback leaves no partial version transition;`CommitUnknown` must read back stored result,then exact version read(s)。 |
...
- fake `retire_formal_version` parity must stay version-repo-only in `commit-04-b`;it must not inspect downstream runtime truth,private queues,logs,ad hoc flags or future-owner material / impact stores before those boundaries are activated。
...
|---|---|---|
| relation read anchor | `MethodAssetRelationRepository.get_relation_anchor_with_version(relation_ref) -> Option<Versioned<MethodAssetRelationReadAnchor>>` under the shared repository result/error surface。 | The Step 6 `3B.4A` anchor is the only current-boundary relation value. Full relation load, list/page, create/save/scan, graph truth and marketplace relation material are excluded. |
| `method_asset_distribution_records` | `MethodAssetDistributionRepository.get_distribution_with_version(distribution_ref)`;`save_distribution(record, expected_version, uow)` with exact Step 7 signatures。 | Application-owned body-free record keyed by `MethodAssetDistributionRef`;prepare creates, adjust updates context by source expected version, mark updates marker by loaded version. No read-material body, relation truth or downstream runtime state. |
| distribution builder output | no standalone repository;only `DistributionReadMaterialBuilderPort.build_distribution_read_material(input: DistributionReadMaterialBuilderInput)`。 | Non-durable transient body-free output. No `distribution_read_materials` table, no material save/list surface and no package/listing body. |
| `event_candidate_assemblies` | Exact typed signatures from Step 7: append/get `MethodAssetDistributionEventCandidateAssembly` under `MethodAssetEventCandidateAssemblyRef` identity through shared `Versioned` / `VersionedRef` / repository error carriers。 | Append-only current-boundary shell from Step 6 `3B.4A`;reload authority for publication/handoff. The generic future candidate object, PH-06 lineage sets, payload and worker schema are excluded. |
...
All newly minted distribution, candidate, publication and handoff refs written by these stores must come from `MethodAssetDistributionHandoffSupportRefFactory`;an already accepted `requested_distribution_ref` is copied without replacement. Relation refs and seam-source refs are copied from validated named wrappers. Distribution record and stored result are always staged/committed/rolled back together;candidate is included in that UoW only when seam source exists. Publication/handoff outcomes use later side-effect UoWs. No repository fake/durable implementation may use row ids, counters, timestamps, config values, typed-ref text or private maps as an alternate identity source.
...
|---|---|
| relation/distribution repository fake | Relation anchor preserves read-only `Versioned<T>` semantics and has no page/save/version-conflict surface;distribution record preserves create/update expected-version conflict, staged UoW visibility and rollback invisibility. |

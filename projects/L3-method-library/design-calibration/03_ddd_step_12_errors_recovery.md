...
| adapter degraded / unavailable / disabled | adapter availability port returns a non-available branch with its formal diagnostic;target registry is resolved once only to supply the formal target set required by outcome identity. | `Degraded -> Blocked`,`Unavailable -> Unavailable`,`Disabled -> Blocked`;copy adapter marker/reason and diagnostic. | publisher and handoff are not invoked;factory-issued outcome shell is persisted;candidate and accepted truth remain durable. | stop without outcome,derive diagnostic from reason/config/status/error,call publisher,or treat disabled as accepted. |
| target disabled / blocked | with adapter `Available`,target registry says disabled/blocked and supplies reason plus formal diagnostic。 | publication_blocked outcome copied from registry reason/diagnostic。 | publisher port is not invoked;candidate remains durable if defined。 | bypass registry,choose alternate target,derive diagnostic locally,turn block into success or failure retry。 |
...
| target / registry unavailable | with adapter `Available`,target registry or target availability summary is unavailable and carries formal reason/diagnostic。 | publication_unavailable with copied reason/diagnostic。 | no publisher/handoff and no candidate rebuild;retryability classification only,algorithm deferred to Step 13。 | expose endpoint/secret/config value,derive diagnostic,loop retry in Step 12,return published。 |

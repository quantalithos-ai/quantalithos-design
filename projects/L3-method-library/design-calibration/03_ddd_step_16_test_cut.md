...
| adapter availability mapping cut | Step 6;Step 7;Step 9;Step 10;Step 11;Step 12;Step 14;Step 15 | 验证 `commit-05-b` adapter `Degraded -> Blocked`,`Unavailable -> Unavailable`,`Disabled -> Blocked`,以及 adapter `Available` 后 target `Disabled | Blocked -> Blocked`,`Unavailable -> Unavailable`。 | 每个分支复制 owning port 的 exact reason/marker + `MethodAssetInfraSafeDiagnosticRef`,通过 support factory mint 并保存 publication outcome;publisher/handoff call count 保持 0;accepted distribution record/candidate/stored result 不回滚。 | 不从 reason/marker text、health payload、topic ack、config、raw error、fake private map 合成 diagnostic,不把 disabled/degraded/unavailable 当 accepted publication/delivery。 |
...
| fake / durable parity cut | Step 6;Step 7;Step 11;Step 13;Step 14;Step 16 | 验证 local fake 与 durable adapter 在 repository/UoW、availability、publisher/handoff outcome、runtime assembly 的语义一致。 | fake 只返回正式 port output,包括两个 `Disabled.diagnostic_ref`;failure injection 不新增 production 不存在的 source;version/cursor/transaction 语义一致。 | 不给 fake 加 test-only diagnostic/ref/map、private source、direct store handle 或 bypass port。 |
...
| post-commit side-effect failure cut | Step 9;Step 11;Step 12;Step 13;Step 15 | 验证 publication/handoff/observability failure after accepted truth/report/candidate committed,并覆盖 disabled precheck outcome persistence。 | 记录 factory-issued safe outcome/port-owned diagnostic;disabled branch 不调用 publisher/handoff;不回滚 accepted truth/report/candidate;不重建 candidate;outcome commit unknown 进入 consistency/manual。 | 不把 publisher/handoff failure 当 Command rollback;不从 current truth、payload、delivery receipt、reason text、config 或 fake map 重建 candidate/diagnostic/outcome。 |

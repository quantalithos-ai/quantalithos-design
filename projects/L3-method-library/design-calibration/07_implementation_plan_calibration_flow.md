...
| 当前设计基线 | `current-design-with-commit-03-b-truth-ref-factory-closure` |
...
6. 读取 `design-calibration/implementation-boundaries/commit-03-b.md`
...
10. 当前 `commit-03-b` 已因 contracts ref-kind owner scope、command shell selector、body-free command source carrier、source-to-service-input assembly、replay envelope/support ref factory、definition/catalog truth-ref factory、dispatch facade、service input carriers、repository/UoW/stored-result surface、repository error surface、definition lifecycle、catalog helper persisted-field coverage、catalog retirement transition、retire formal-version precheck carve-out、fake parity 和 duplicate replay 缺口完成设计闭口而重新激活;恢复实现前必须使用最新 implementation ledger 与 boundary ledger 重跑 Design Gate
```
...
当前 `commit-03-b` definition/catalog accepted service boundary 已由设计侧补齐 implementation-facing contracts ref-kind owner scope、command shell selector、body-free command source carrier、source-to-service-input assembly、replay envelope/support ref factory、definition/catalog truth-ref factory、dispatch facade、service input carriers、repository/UoW/stored-result、repository error surface、definition lifecycle、catalog helper persisted-field coverage、catalog retirement transition、retire formal-version precheck carve-out、fake parity 和 duplicate replay closure,并重新激活到 `read_docs`;

...
| commit-03-b | PH-03 | 建立 definition/catalog accepted service vertical slice | minimal contracts ref-kind registry/export for Step 6 `3B.1` / `3B.1A` labels;service、repo fake、minimal API handler、UoW/stored result;exact dispatch facade、facade I/O structs、selector enum、6 个 service input carriers、definition/catalog repository methods、`Versioned<T>` / expected-version / UoW carrier、stored-result repository methods、application-owned replay/idempotency refs、`MethodAssetRepositoryError`、fake parity | public command DTO body、route / RPC binding、payload fields、formal version、publisher、job、query/material、external summary durable dereference、provider body / URL / artifact archive adapter、未闭口 service input field / error variant / fake-only ref | service-flow-fast slice;infra fake slice |

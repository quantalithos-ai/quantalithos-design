...
| commit-03-b | PH-03 | 建立 definition/catalog accepted service vertical slice | minimal contracts ref-kind registry/export for Step 6 `3B.1` / `3B.1A` labels;service、repo fake、minimal API handler、UoW/stored result;exact dispatch facade、facade I/O structs、body-free command source carrier、selector enum、source-to-service-input assembly map、replay envelope/support ref factory、definition/catalog truth-ref factory、6 个 service input carriers、definition/catalog repository methods、`Versioned<T>` / expected-version / UoW carrier、stored-result repository methods、application-owned replay/idempotency refs、`MethodAssetRepositoryError`、`MethodAssetDefinitionLifecycle` / `definition_lifecycle` transition guard、catalog helper exact signatures、`MethodAssetCatalogEntryStatus` exact `Visible -> Retired` retire mapping / `mark_retired(MethodLibrarySafeMarker)` helper、fake parity;`retire_definition` current-boundary check is definition lifecycle / expected version / safe retirement marker only | public command DTO body、route / RPC binding、payload fields、formal version、formal-version precheck、active formal-version conflict lookup、publisher、job、query/material、external summary durable dereference、provider body / URL / artifact archive adapter、未闭口 service input field / source map / error variant / fake-only ref、private lifecycle side-state、private catalog status map、把 `Pending` / `Hidden` / `Deprecated` 默认为 `Registered`、本地自造 catalog helper 参数、本地构造 replay envelope / opaque refs、本地 mint definition/catalog truth refs | service-flow-fast slice;infra fake slice |
...
| commit-04-b | PH-04 | 建立 formalization/version services and replay | minimal contracts ref-kind registry/export for 6 selector intent labels;application facade、service、repo fake、minimal API handler、UoW/stored result、exact command source carrier、replay envelope/support ref factory、formalization/version truth-ref factory、6 个 service input carriers、formalization state / formal version / basis summary repository 方法、basis resolver / policy diagnostic builder callable surface、retirement precheck helper methods、commit unknown read-back | query/material、publisher/job、history query、downstream runtime truth、fingerprint/snapshot/publish lifecycle、未闭口 history ref family、本地自造 truth ref / replay envelope | service-flow-fast formal slice |
| commit-05-a | PH-05 | 建立 controlled consumption material contracts/domain | consumption material、Definition vs Use guard、availability marker;按 formal `03` §6.3C / Step 6 `4C` 落 exact typed refs、state carriers、body-free support carriers、copy-only availability marker 和 no-downstream-truth tests | actual downstream runtime、application service、repository fake/durable、availability resolver/mapper port、query refresh、handoff / publisher / job / evidence schema | contract-domain-fast consumption slice |
| commit-05-b | PH-05 | 建立 distribution/handoff semantics services | distribution context、handoff shell、availability mapper/fake;按正式 `03` §6.3D、Step 7 closure patch、Step 9 flow overlay 和 Step 11 persistence/UoW overlay 落 application facade、selector/source carrier、replay/support factory、builder/target/publisher/handoff ports、outcome shell、stored-result/UoW 和 fake parity | worker publisher loop、real handoff delivery、real downstream adapter、transport/topic/retry/dead-letter、durable `DistributionReadMaterial` store、relation lifecycle implementation、marketplace transaction、release evidence verdict、raw body / report/archive/package body / external receipt body | service-flow-fast;infra-runtime-fake |
| commit-06-a | PH-06 | 建立 trace/audit/impact/lineage contracts/domain | exact typed refs/wrappers/ref sets、safe-marker reason、body-free support carriers、four pure domain objects/state guards、contracts/domain tests | application/service/repository/fake/durable/resolver/mapper/UoW/replay/persistence/query/refresh/API/worker/publisher/job/report、external body、本地新增 schema/source | contract-domain-fast trace/audit/impact/lineage;targeted redaction scan |
| commit-06-b | PH-06 | 建立 trace/audit/impact/consistency service-store vertical slice | contracts 仅补 7 个正式 selector kind label 的 registry/export/fixture plumbing;domain 仅补两个正式 link helper;application 落正式 `03` §6.3F 的 body-free source/input carriers、selector/source map、facade、四个 repository ports、support/truth-ref factory、stored-result replay、Versioned/expected-version/UoW;infra 落 typed in-memory fake、staged UoW、rollback/CommitUnknown 与 factory parity;直接 service-flow-fast / redaction tests | report generator、evidence index generator、jobs、recovery/replay job、query/material、API/worker/publisher、external/provider body、第五 truth repository、resolver/mapper、未闭口 schema/port/error/marker/ref、私造 opaque/truth refs 或 fake-only semantic map | `service-flow-fast` exact PH-06 flows;fixed redaction-check |
| commit-07-a | PH-07 | 建立 external summary body-free contracts/domain 与 transient adapter seam | formal `03` §6.3G/Step 5~16 exact closure：contracts typed wrappers/kinds、marker wrappers、closed enums/sets/safe summary;domain `ExternalSourceSummary` lifecycle 和 `ExternalBodyBoundaryRule` migration/pure guards;application one-method `ExternalBodyFreeSourceAdapterPort`;infra exact three-private-field fake;direct tests 和 actual-run fixed raw artifacts | service/protocol/repository/UoW/replay/durable dereference/provider body/archive lifecycle/API/worker/publisher/job/query/package-set/report generator;local ref/marker mint、opaque-text parsing 或 fake private semantic map | fixed `contract-domain-fast` external body-free raw set;`redaction-check` |
| commit-07-b | PH-07 | 建立 package/method set peripheral command/store vertical slice | formal `03` §6.3H exact contracts/domain、9 selector/source/input/service flows、2 truth repositories、canonical stored-result/UoW/version/error reuse、complete upstream trait fake parity、factory-only refs/error markers、rollback/CommitUnknown/cross-aggregate invariant、fixed residual/dependency/VETO evidence | marketplace transaction、advanced UX、query/material、API/worker/job/event/publisher、durable adapter、report generator、parallel stored-result/UoW/error family、local marker/ref mint or fake semantic map | fixed `service-flow-fast` package/set raw set;dependency/residual/VETO/redaction targeted outputs |

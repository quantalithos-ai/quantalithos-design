...
| commit-03-b | PH-03 | 建立 definition/catalog accepted service vertical slice | service、repo fake、minimal API handler、UoW/stored result;exact dispatch facade、facade I/O structs、6 个 service input carriers、definition/catalog repository methods、`Versioned<T>` / expected-version / UoW carrier、stored-result repository methods、application-owned replay/idempotency refs、`MethodAssetRepositoryError`、fake parity | formal version、publisher、job、query/material、external summary durable dereference、provider body / URL / artifact archive adapter、未闭口 service input field / error variant / fake-only ref | service-flow-fast slice;infra fake slice |

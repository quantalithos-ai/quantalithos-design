...
| commit-06-a | contract-domain-fast exact kinds/ref sets/body-free carriers/state guards + targeted redaction | EV-ML-CONTRACT-001;EV-ML-REPORT-001 seed;ML-FG-009/011 | `artifacts/test/<run_id>/suites/contract-domain-fast/{cargo-fmt-check.txt,cargo-check-contracts.txt,cargo-check-domain.txt,cargo-test-contracts.txt,cargo-test-domain.txt,redaction-scan.txt}`;optional raw-derived `reports/runs/<run_id>/suites/contract-domain-fast.md` | raw body、source/schema 缺口、static pass 或 `latest` 阻断 |
| commit-06-b | exact `service-flow-fast` trace/impact/audit/lineage/consistency flows;fixed `redaction-check` | EV-ML-SERVICE-001;EV-ML-REDACTION-001 seed;VETO-ML-005/006/009/011 | `artifacts/test/<run_id>/suites/service-flow-fast/{cargo-fmt-check.txt,cargo-check-workspace.txt,cargo-check-application.txt,cargo-check-infra.txt,cargo-test-application.txt,cargo-test-infra.txt,trace-consistency-redline.txt}`;`artifacts/test/<run_id>/redaction-check.txt`;derived reports under `reports/runs/<run_id>/` only | redaction leak、duplicate rerun、CommitUnknown blind retry、raw body or boundary越权不得风险接受 |
| commit-07-a | exact external typed-wrapper/marker/set/safe-summary roundtrips;summary and body-rule state/no-mutation guards;one-method adapter/fake echo/error parity;targeted body-free redlines | EV-ML-CONTRACT-001;EV-ML-REDACTION-001;ML-RL-004;ML-STATE-001/002;VETO-ML-005/011 | `artifacts/test/<run_id>/suites/contract-domain-fast/{cargo-fmt-check.txt,cargo-check-workspace.txt,cargo-check-contracts.txt,cargo-check-domain.txt,cargo-check-application.txt,cargo-check-infra.txt,cargo-test-contracts.txt,cargo-test-domain.txt,cargo-test-application.txt,cargo-test-infra.txt,external-body-free-redline.txt}`;`artifacts/test/<run_id>/redaction-check.txt`;actual-run-derived reports only | provider/document/artifact/archive/evidence body,URL/path/secret/header/status/stack/raw reason/config leak;raw or locally minted ref/marker;fake map/mutation;static pass/`latest`/report generator or any service/repository/provider claim blocks |
| commit-07-b | exact package/set contracts/domain + nine service flows + staged fake parity;duplicate/version/rollback/CommitUnknown/cross-aggregate races;factory-marker/local-mint redlines | EV-ML-SERVICE-001;EV-ML-RISK-001 seed;EV-ML-DEPENDENCY-001 seed;ML-FG-012;ML-RL-004/005;ML-STATE-001/002;ML-TX-001;ML-IDEMP-001;VETO-ML-008 | formal §6.3H.5 fixed `service-flow-fast` raw set plus actual `dependency-boundary.txt`,`peripheral-residual.txt`,`veto-ml-008.txt`,`redaction-check.txt`;only actual-run-derived reports | peripheral blocks/rewrites core,expands consumption authorization,adds marketplace scope,uses non-canonical result/UoW/error or local/fake marker/ref,or leaves partial write after failed commit |

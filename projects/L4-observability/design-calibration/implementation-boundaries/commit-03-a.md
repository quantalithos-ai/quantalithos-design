...
| current_recovery_point | `intake/redaction/correlation UoW flow` |
| activation_rule | `project_execution_ledger.current_boundary == commit-03-a` |
...
打通 safe input -> validation/redaction -> receipt/disposition -> correlation/safe signal 的 application accepted、duplicate、reject、quarantine、delayed flow。

## Activation Guard

| rule | status | consequence |
|---|---|---|
| project ledger current boundary | pending | 必须先推进到 commit-03-a。 |
...
|---|---|---|---|
| `projects/L4-observability/03-详细设计.md` | §7~§8、§11~§14：input、flow、UoW、error/recovery、telemetry | pending | 实现前读取；冲突时暂停并回写真相源。 |
| `projects/L4-observability/05-测试方案.md` | §6：intake/correlation/redaction/UoW cuts | pending | 实现前读取；冲突时暂停并回写真相源。 |
| `projects/L4-observability/design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md` | §5.5、§6、§7 commit-03-a 经验复核 | pending | 实现前读取；冲突时暂停并回写真相源。 |
| `projects/L4-observability/design-calibration/07_implementation_plan_step_09_spikes_risks_open_questions.md` | R06-F-AFFECT-UOW-01、S08-RECOVERY-CLASS-OWNER-01 | pending | 实现前读取；冲突时暂停并回写真相源。 |
| `projects/L4-observability/design-calibration/07_implementation_plan_step_10_rollback_pause_change_control.md` | UoW/recovery failure and pause rules | pending | 实现前读取；冲突时暂停并回写真相源。 |

## Planned Task and Batch Contract

| task | order | action | output | status |
|---|---|---|---|---|
| IMPL-03A-01 | 1 | 闭合 input assembly、redaction port、operation context、digest 和 source map | application inputs/ports | planned |
| IMPL-03A-02 | 2 | 实现 C01~C04 accepted/reject/quarantine/delayed service flow | accepted observation slice | planned |
| IMPL-03A-03 | 3 | 实现 duplicate/conflict/in-flight/commit-unknown probe、UoW 和 idempotency | recovery/idempotency path | planned |
| IMPL-03A-04 | 4 | 绑定 redaction/correlation/telemetry assertions 和 targeted tests | safety test slice | planned |

BATCH-03A-01 input/port/context/digest high-risk; BATCH-03A-02 accepted UoW split by intake/signal; BATCH-03A-03 idempotency/unknown/recovery; BATCH-03A-04 redaction/telemetry <=250 lines

## Allowed Scope

| type | path_or_rule | status |
|---|---|---|
| allowed_rule | application inputs/ports/unit-of-work/idempotency/digest/intake/signal modules及相关 tests | planned |
| allowed_rule | C01~C04、I01/I03 的 observation-side orchestration | planned |
| allowed_rule | typed redaction/correlation and committed accepted write set | planned |

## Forbidden Scope

| type | path_or_rule | status |
|---|---|---|
| forbidden_rule | API route、worker ack、I05 positive decode、external provider、source truth write、Query writer、Job runner | active |
| forbidden_rule | Clone/reload、partial success、默认 retry/action 或从 raw body 推导 digest | active |
...
|---|---|---|---|
| activation gate | 项目台账 current boundary 必须推进到 `commit-03-a` | pending | 设计期未执行；不得推断 pass。 |
| design closure | input fields/source/digest/context/factory can be constructed losslessly | pending | 设计期未执行；不得推断 pass。 |
| flow/UoW tests | accepted write set/order, rollback, duplicate and in-flight paths | pending | 设计期未执行；不得推断 pass。 |
| redaction gate | redaction-before-serialization and forbidden material negative tests | pending | 设计期未执行；不得推断 pass。 |
| correlation gate | correlation id/trace parent/audit/evidence linkage source map | pending | 设计期未执行；不得推断 pass。 |
| commit-unknown | probe and indeterminate handling without default action | pending | 设计期未执行；不得推断 pass。 |
| workspace/scope | application-only scope and build/whitespace checks | pending | 设计期未执行；不得推断 pass。 |
...
| scope ownership | touched paths map only to `commit-03-a` | pending |
...
|---|---|---|---|
| activation_gate | pending | 等待项目级台账推进到 commit-03-a；预创建不授权实现。 | wait_design |
...
| field | value |
|---|---|
| planned_commit_message | planned message from Step 11 for commit-03-a |
...
| next_boundary | next planned boundary after commit-03-a | pending |
| user_owned_changes_untouched | target worktree file list recorded | pending |

## Inherited Affected Binding

`R06-F-AFFECT-UOW-01` 与 `S08-RECOVERY-CLASS-OWNER-01` 保持 open；不得宣称全 flow atomicity 或 recovery owner 已闭合。

GATE-OBS-03、GATE-OBS-08；任何 raw/secret/source write 是 hard blocker。
...
Remain `planned / wait_until_current`. Do not run or record boundary execution until the project ledger advances to `commit-03-a`; then reread this file and the required sources.

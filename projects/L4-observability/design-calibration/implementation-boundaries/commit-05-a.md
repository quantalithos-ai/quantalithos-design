...
| current_recovery_point | `signal projection and derived-event safety` |
| activation_rule | `project_execution_ledger.current_boundary == commit-05-a` |
...
闭合 safe telemetry schema、low-cardinality allowlist、correlation/trace propagation、projection/rollup/freshness source 和 E03/E10/E11/E12 snapshot。

## Activation Guard

| rule | status | consequence |
|---|---|---|
| project ledger current boundary | pending | 必须先推进到 commit-05-a。 |
...
|---|---|---|---|
| `projects/L4-observability/03-详细设计.md` | §5、§7.2~§7.5、§9、§14：signal/projection/telemetry | pending | 实现前读取；冲突时暂停并回写真相源。 |
| `projects/L4-observability/05-测试方案.md` | §6、§10：signal/redaction/metric/trace cuts | pending | 实现前读取；冲突时暂停并回写真相源。 |
| `projects/L4-observability/design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md` | §5.9、§7 commit-05-a 经验复核 | pending | 实现前读取；冲突时暂停并回写真相源。 |
| `projects/L4-observability/design-calibration/07_implementation_plan_step_07_test_acceptance_gates.md` | GATE-OBS-08 telemetry safety | pending | 实现前读取；冲突时暂停并回写真相源。 |
| `standards/document/设计真相源闭环与可落码性标准.md` | observability layering, projection source and redaction | pending | 实现前读取；冲突时暂停并回写真相源。 |

## Planned Task and Batch Contract

| task | order | action | output | status |
|---|---|---|---|---|
| IMPL-05A-01 | 1 | 实现 log/metric/trace safe schema、label allowlist、trace parent propagation | telemetry carriers/policy | planned |
| IMPL-05A-02 | 2 | 实现 signal projection、rollup、read-model marker 和 committed freshness source | projection store/policy | planned |
| IMPL-05A-03 | 3 | 实现 E03/E10/E11/E12 derived immutable snapshot builders | derived event source | planned |
| IMPL-05A-04 | 4 | 实现 recursion/sink failure/retention separation and no-write tests | telemetry safety tests | planned |

BATCH-05A-01 schema/allowlist <=250 lines; BATCH-05A-02 projection/rollup high-risk; BATCH-05A-03 event snapshot <=250 lines; BATCH-05A-04 recursion/sink guard <=200 lines

## Allowed Scope

| type | path_or_rule | status |
|---|---|---|
| allowed_rule | domain signal/projection/telemetry modules、observation-owned infra recorder/projection stores、E03/E10/E11/E12 snapshot tests | planned |
| allowed_rule | safe labels/templates、correlation and committed source markers | planned |

## Forbidden Scope

| type | path_or_rule | status |
|---|---|---|
| forbidden_rule | raw signal body、secret/high-cardinality labels、telemetry-to-command loop、business execution truth、backend retention policy | active |
| forbidden_rule | 从 current truth 重建 event payload 或把 sink failure改写 owner truth | active |
...
|---|---|---|---|
| activation gate | 项目台账 current boundary 必须推进到 `commit-05-a` | pending | 设计期未执行；不得推断 pass。 |
| schema/allowlist | only declared safe fields and low-cardinality labels | pending | 设计期未执行；不得推断 pass。 |
| redaction | redaction-before-serialization and body-free telemetry | pending | 设计期未执行；不得推断 pass。 |
| correlation/trace | correlation id/trace parent propagation has exact source | pending | 设计期未执行；不得推断 pass。 |
| projection source | Fresh/rollup marker requires committed source/fence | pending | 设计期未执行；不得推断 pass。 |
| derived snapshot | E03/E10/E11/E12 never rebuild from current truth | pending | 设计期未执行；不得推断 pass。 |
| sink/no-write | sink failure/recursion/retention separation does not mutate owner truth | pending | 设计期未执行；不得推断 pass。 |
...
| scope ownership | touched paths map only to `commit-05-a` | pending |
...
|---|---|---|---|
| activation_gate | pending | 等待项目级台账推进到 commit-05-a；预创建不授权实现。 | wait_design |
...
| field | value |
|---|---|
| planned_commit_message | planned message from Step 11 for commit-05-a |
...
| next_boundary | next planned boundary after commit-05-a | pending |
| user_owned_changes_untouched | target worktree file list recorded | pending |

## Inherited Affected Binding

受 `03-RPR-S09-PER-FLOW` 和 inherited no-write/owner rules约束；telemetry 只能是 projection，不能形成 authority。

GATE-OBS-08；任何 raw/secret/high-cardinality finding hard-block。
...
Remain `planned / wait_until_current`. Do not run or record boundary execution until the project ledger advances to `commit-05-a`; then reread this file and the required sources.

...
| current_recovery_point | `14 Query diagnostic strict zero-write` |
| activation_rule | `project_execution_ledger.current_boundary == commit-05-b` |
...
打通 Q01~Q14 selector/source map、composite read bundle、visibility/freshness/degraded mapper 和全量 Query zero-write。

## Activation Guard

| rule | status | consequence |
|---|---|---|
| project ledger current boundary | pending | 必须先推进到 commit-05-b。 |
...
|---|---|---|---|
| `projects/L4-observability/03-详细设计.md` | §7.3、§8.3、§10、§12、§15：14 Query and zero-write | pending | 实现前读取；冲突时暂停并回写真相源。 |
| `projects/L4-observability/05-测试方案.md` | §6、§9、§10：query/no-write/diagnostic cuts | pending | 实现前读取；冲突时暂停并回写真相源。 |
| `projects/L4-observability/design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md` | §5.10、§7 commit-05-b 经验复核 | pending | 实现前读取；冲突时暂停并回写真相源。 |
| `projects/L4-observability/design-calibration/07_implementation_plan_step_09_spikes_risks_open_questions.md` | SP-OBS-09-005、03-RPR-S09-PER-FLOW | pending | 实现前读取；冲突时暂停并回写真相源。 |
| `standards/document/设计真相源闭环与可落码性标准.md` | Query/read/no-write and validation truth | pending | 实现前读取；冲突时暂停并回写真相源。 |

## Planned Task and Batch Contract

| task | order | action | output | status |
|---|---|---|---|---|
| IMPL-05B-01 | 1 | 实现 Q01~Q14 selector/request/view/page carrier，显式表达 variants/absence/cardinality | query input/output surface | planned |
| IMPL-05B-02 | 2 | 实现 same-snapshot read bundle/read fence/visibility/freshness mapper | query façade | planned |
| IMPL-05B-03 | 3 | 实现 diagnostic/read-model/peripheral assembler，不引入 writer capability | read assembler | planned |
| IMPL-05B-04 | 4 | 实现 exhaustive write spies、stale/degraded/unknown/absence tests | negative corpus | planned |

BATCH-05B-01 point/page schema split; BATCH-05B-02 read bundle/fence high-risk; BATCH-05B-03 mapper/diagnostic <=300 lines; BATCH-05B-04 negative/write-spy <=250 lines

## Allowed Scope

| type | path_or_rule | status |
|---|---|---|
| allowed_rule | all Query contracts/use/read application modules、API query handlers、read-only infra stores/fakes、query tests | planned |
| allowed_rule | Q01~Q14 visibility/absence/freshness/degraded/diagnostic/peripheral views | planned |

## Forbidden Scope

| type | path_or_rule | status |
|---|---|---|
| forbidden_rule | save/append/mark-stale/refresh/rebuild/reserve calls、source truth write、hidden-to-missing conversion、page fallback | active |
| forbidden_rule | 通过 generic JSON、row existence、错误文本或 first failure 推导结果 | active |
...
|---|---|---|---|
| activation gate | 项目台账 current boundary 必须推进到 `commit-05-b` | pending | 设计期未执行；不得推断 pass。 |
| query identity | 14 exact Query variants/source/absence/page semantics are mapped | pending | 设计期未执行；不得推断 pass。 |
| read fence | same committed snapshot and explicit freshness/visibility/degraded source | pending | 设计期未执行；不得推断 pass。 |
| zero-write | all Query branches prove no writer/UoW/reservation/refresh/rebuild | pending | 设计期未执行；不得推断 pass。 |
| negative corpus | stale/degraded/unknown/hidden/missing/timeout do not collapse | pending | 设计期未执行；不得推断 pass。 |
| API mapping | facade-only query mapping; no direct store or source write | pending | 设计期未执行；不得推断 pass。 |
| workspace/scope | query/read-only scope and build/whitespace checks | pending | 设计期未执行；不得推断 pass。 |
...
| scope ownership | touched paths map only to `commit-05-b` | pending |
...
|---|---|---|---|
| activation_gate | pending | 等待项目级台账推进到 commit-05-b；预创建不授权实现。 | wait_design |
...
| field | value |
|---|---|
| planned_commit_message | planned message from Step 11 for commit-05-b |
...
| next_boundary | next planned boundary after commit-05-b | pending |
| user_owned_changes_untouched | target worktree file list recorded | pending |

## Inherited Affected Binding

`03-RPR-S09-PER-FLOW` 重点要求 Q01~Q14 逐接口 flow proof；不得用 family summary 或默认 missing。

GATE-OBS-03、GATE-OBS-09；任何 writer capability 是 hard blocker。
...
Remain `planned / wait_until_current`. Do not run or record boundary execution until the project ledger advances to `commit-05-b`; then reread this file and the required sources.

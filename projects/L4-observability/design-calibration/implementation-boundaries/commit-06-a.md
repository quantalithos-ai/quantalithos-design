...
| phase | `PH-06` |
| design_baseline | `planned-after-commit-05-b` |
| implementation_repo | `/home/aris/Projects/quantalithos-observability` |
| status | `planned` |
| gate_status | `pending` |
| next_allowed_action | `wait_until_current` |
| current_recovery_point | `report handoff/evidence input and retention protection` |
| activation_rule | `project_execution_ledger.current_boundary == commit-06-a` |
...
闭合 report handoff、EvidenceIndexInput、authenticity/provenance hints、retention/protection marker 和 E06/E07 lifecycle，不生成 verdict。

## Activation Guard

| rule | status | consequence |
|---|---|---|
| project ledger current boundary | pending | 必须先推进到 commit-06-a。 |
...
|---|---|---|---|
| `projects/L4-observability/03-详细设计.md` | §7.2/§7.6、§8.6~§8.7、§11~§13：handoff/retention/report | pending | 实现前读取；冲突时暂停并回写真相源。 |
| `projects/L4-observability/04-配置设计.md` | §10~§11：retention/protection and failure | pending | 实现前读取；冲突时暂停并回写真相源。 |
| `projects/L4-observability/05-测试方案.md` | §6、§10、§13：report/evidence tests | pending | 实现前读取；冲突时暂停并回写真相源。 |
| `projects/L4-observability/06-验收标准.md` | §3~§4、§10~§11：evidence/VF responsibility | pending | 实现前读取；冲突时暂停并回写真相源。 |
| `projects/L4-observability/design-calibration/07_implementation_plan_step_06_tasks_commit_boundaries.md` | §5.11、§7 commit-06-a 经验复核 | pending | 实现前读取；冲突时暂停并回写真相源。 |
| `projects/L4-observability/design-calibration/07_implementation_plan_step_09_spikes_risks_open_questions.md` | S08-JOB-REPORT-REF-OWNER-01、S08-M1-SECONDARY-TYPE-OWNER-01 | pending | 实现前读取；冲突时暂停并回写真相源。 |

## Planned Task and Batch Contract

| task | order | action | output | status |
|---|---|---|---|---|
| IMPL-06A-01 | 1 | 实现 handoff/evidence-index/authenticity/retention/protection public carrier | immutable input contracts | planned |
| IMPL-06A-02 | 2 | 实现 C07~C10/C15~C16 handoff/guard service flow | handoff/protection façade | planned |
| IMPL-06A-03 | 3 | 实现 E06/E07 immutable snapshot/marker lifecycle | event/marker source | planned |
| IMPL-06A-04 | 4 | 实现 no-verdict/no-delete/active-reference-conflict tests | handoff/retention tests | planned |

BATCH-06A-01 handoff/evidence carriers 100~300 lines; BATCH-06A-02 retention/protection <=250 lines; BATCH-06A-03 handoff/retention service split; BATCH-06A-04 no-verdict/no-delete <=200 lines

## Allowed Scope

| type | path_or_rule | status |
|---|---|---|
| allowed_rule | handoff/evidence index/authenticity/retention/protection contracts/domain/application and E06/E07 snapshot source | planned |
| allowed_rule | immutable body-free handoff input、protection/hold marker、candidate report input | planned |

## Forbidden Scope

| type | path_or_rule | status |
|---|---|---|
| forbidden_rule | real evidence alias、final acceptance verdict、source cleanup/delete、external delivery call、J06 positive execution | active |
| forbidden_rule | 让 generator、handoff 或 authenticity hint 代表验收裁决 | active |
...
|---|---|---|---|
| activation gate | 项目台账 current boundary 必须推进到 `commit-06-a` | pending | 设计期未执行；不得推断 pass。 |
| immutable input | handoff/evidence input cannot be mutated or rebuilt from current truth | pending | 设计期未执行；不得推断 pass。 |
| body-free/authenticity | only refs/digests/metadata; no evidence body or verdict | pending | 设计期未执行；不得推断 pass。 |
| retention/protection | hold, active consumer conflict and protection marker semantics are explicit | pending | 设计期未执行；不得推断 pass。 |
| event snapshot | E06/E07 source is committed immutable snapshot | pending | 设计期未执行；不得推断 pass。 |
| report audit | no static passed/latest/alias/signoff | pending | 设计期未执行；不得推断 pass。 |
| workspace/scope | handoff/retention-only scope and build/whitespace checks | pending | 设计期未执行；不得推断 pass。 |
...
| scope ownership | touched paths map only to `commit-06-a` | pending |
...
|---|---|---|---|
| activation_gate | pending | 等待项目级台账推进到 commit-06-a；预创建不授权实现。 | wait_design |
...
| field | value |
|---|---|
| planned_commit_message | planned message from Step 11 for commit-06-a |
...
| next_boundary | next planned boundary after commit-06-a | pending |
| user_owned_changes_untouched | target worktree file list recorded | pending |

## Inherited Affected Binding

`S08-JOB-REPORT-REF-OWNER-01`、`S08-M1-SECONDARY-TYPE-OWNER-01` 保持 open；缺 owner 时只允许 fail closed。

GATE-OBS-06、GATE-OBS-10；handoff input 不拥有最终验收裁决。
...
Remain `planned / wait_until_current`. Do not run or record boundary execution until the project ledger advances to `commit-06-a`; then reread this file and the required sources.

...
|---|---|---|---|---|---|---|---|---|---|---|
| MEM-<PROJECT>-001 | project / phase / commit-boundary | 必读规范 / 提交纪律 / 设计边界 / 工作区安全 / 验证门禁 | <可直接写入永久记忆的一句话规则> | `07-实施计划.md` §3 阅读清单 / 不适用 | `07-实施计划.md` | §3.x | 项目首次开工 / 规范路径变更 / 每次提交前 | until superseded / phase complete | 正式文档优先,暂停并刷新记忆 | 是 / 否 |

Agent 永久记忆生成门禁：

| 检查项 | 通过标准 | 失败处理 |
|---|---|---|
| 种子表存在 | `07-实施计划.md` §3 明确给出种子表 | 不生成永久记忆,暂停并要求补实施计划 |
...
- 永久记忆种子必须覆盖设计修复后的后序任务:先判断提交是否同项目,再显式检查是否需要总结可复用经验,需要时补标准 / SOP / 项目记忆并添加具体示例。
- 项目 owner 的临时执行约束只有在本章明确列入种子表且写明失效条件时,才能进入永久记忆。
...
- 阅读清单、阶段实施前阅读矩阵、Agent 永久记忆种子表、配置、规范和工具前置条件均已列出。
...
|---|---|---|---|---|---|
| PH-xx / commit-xx-a | <文件 / 章节> | <标准章节> | 通过 / 不通过 | <冲突与影响> | <design commit / 不适用> |
